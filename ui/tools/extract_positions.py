...
# THE STAMP'S DEPENDENCIES, DEFINED ONCE AND SHARED. ui/tests/counting_rule.test.js recomputes both
# figures in node from the same raw bytes, so writer and reader agree by construction rather than by
# a mirrored implementation. See ui/tools/positions_identity.py for why the board was the wrong pin.
_pspec = importlib.util.spec_from_file_location(
    "positions_identity", os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                       "positions_identity.py"))
_pid = importlib.util.module_from_spec(_pspec)
_pspec.loader.exec_module(_pid)
...
def main():
    ...
    # ---- THE STAMP NAMES WHAT THIS FILE ACTUALLY DEPENDS ON (2026-08-31) ------------------------
    # It used to name a BOARD (`board` / `expectedBoard`), and that was the wrong dependency: this
    # map is values-free, so a price move, a round advance or a re-bake moves the board md5 and
    # leaves every position code exactly where it was. The stamp said f2df6e0a while the live bundle
    # was c8c2f2b6 and the map was, measured, 804/804 correct — a stamp claiming staleness that did
    # not exist, and an invitation to "fix" it by adding a board pin that would have taken the
    # pocket panel dark on every landing. ui/tools/positions_identity.py carries the full note.
    #
    # `builtAgainstBoard` keeps the trace of which bundle the join ran against, and is labelled in
    # the artifact itself as provenance rather than a dependency, so nobody checks it by mistake.
    payload = {
        "stamp": {
            "rosterSig": _pid.roster_sig([p["key"] for p in players]),
            "rosterSigVersion": _pid.SIG_VERSION,
            "locationsCsvMd5": _pid.csv_md5(LOC_CSV),
            "n": len(by_key),
            "builtAgainstBoard": board_md5[:8],
            "_builtAgainstBoard_note": "PROVENANCE ONLY — which board bundle the name join ran "
                                       "against. NOT a dependency: this map is values-free and does "
                                       "not move when the board does. Do not check it.",
    ...
    body = ("// GENERATED by ui/tools/extract_positions.py — VALUES-FREE position map (item 196(2)).\n"
            ...
            "// the board bundle remains the sole value source.\n"
            "// STAMPED WITH ITS REAL DEPENDENCIES: the ROSTER (the board's player-key set) and the\n"
            "// CSV's own md5. NOT with a board md5 — this map is values-free and does not move when\n"
            "// the board does. Regenerate when a player joins or leaves, or when the CSV is re-cut.\n"
            "window.__CLUB_POSITIONS__ = " + json.dumps(payload, ensure_ascii=False, sort_keys=True) + ";\n")
    with open(OUT, "w", encoding="utf-8") as f:
        f.write(body)
    print("wrote %s — %d players, roster %s, csv %s (values-free; built against board %s)" % (
        os.path.relpath(OUT, ROOT), len(by_key), payload["stamp"]["rosterSig"][:8],
        payload["stamp"]["locationsCsvMd5"][:8], board_md5[:8]))
