...
def main():
    ...
    env = {'PYTHONPATH': '%s:%s' % (WS, VENDOR), 'RL_CONFIG_MODE': 'gate'}
    bd = run('python3 rl_export.py', cwd=WS, env=env, timeout=900)
    built = os.path.join(WS, 'rl_app_data.json')
    built_md5 = md5(built) if os.path.exists(built) else None
    ck('(2) canonical engine build (RL_PVC2/LEGE/LEGF=1) succeeds', bd.returncode == 0 and built_md5 is not None,
       'rc=%d' % bd.returncode)
    ...
    ck('(3) rebuilt board BYTE-IDENTICAL to committed data/rl_build/rl_app_data.json',
       built_md5 == committed_md5, 'rebuilt %s vs committed %s' % (str(built_md5)[:12], committed_md5[:12]))
