...
historical board id: forward_vector_82fcd8bb.json and the identity below are this round's.

Run:  python3 test_forward_lens_82fcd8bb.py      (exit 0 == the committed forward view matches this oracle)
"""
import hashlib
import json
import os
...
FORWARD_BOARD_MD5_GOOD = '82fcd8bb1e552b927299b5702122e321'
FORWARD_REFERENCE = 'forward_vector_82fcd8bb.json'
FORWARD_VECTOR_SHA256 = 'e5b9d5cb8df633468b7dbb8d327ac602ffdf91952484e70622ad6e8fec4260b1'
...
def main():
    ...
    if _lens(doc, '+1').get('sum') != 640116:
        fails.append("lens('+1').get('sum') %r != 640116" % _lens(doc, '+1').get('sum'))
    if _lens(doc, '+2').get('sum') != 203958:
        fails.append("lens('+2').get('sum') %r != 203958" % _lens(doc, '+2').get('sum'))
    ...
    if fails:
        return 1
    print('FORWARD ORACLE OK: board 82fcd8bb active 804 '
          '+1 sum 640116 sheezel 10841 | +2 sum 203958 sheezel 2605 '
          '| seal e5b9d5cb (expect 82fcd8bb/804/640116/203958/0)')
    return 0
