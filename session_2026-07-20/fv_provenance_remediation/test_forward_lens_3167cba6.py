...
historical board id: forward_vector_3167cba6.json and the identity below are this round's.

Run:  python3 test_forward_lens_3167cba6.py      (exit 0 == the committed forward view matches this oracle)
"""
import hashlib
import json
import os
...
FORWARD_BOARD_MD5_GOOD = '3167cba643a6b16e5ef5d904d8957fcd'
FORWARD_REFERENCE = 'forward_vector_3167cba6.json'
FORWARD_VECTOR_SHA256 = 'abb690b680f3f125e1c2316cfdc3264d912dcaf3a9d01bca5a5cc53f13799e20'
...
def main():
    ...
    if _lens(doc, '+1').get('sum') != 637355:
        fails.append("lens('+1').get('sum') %r != 637355" % _lens(doc, '+1').get('sum'))
    if _lens(doc, '+2').get('sum') != 203536:
        fails.append("lens('+2').get('sum') %r != 203536" % _lens(doc, '+2').get('sum'))
    if _lens(doc, '+1').get('sheezel') != 11484:
        fails.append("lens('+1').get('sheezel') %r != 11484" % _lens(doc, '+1').get('sheezel'))
    if _lens(doc, '+2').get('sheezel') != 2739:
        fails.append("lens('+2').get('sheezel') %r != 2739" % _lens(doc, '+2').get('sheezel'))
    ...
    if fails:
        return 1
    print('FORWARD ORACLE OK: board 3167cba6 active 804 '
          '+1 sum 637355 sheezel 11484 | +2 sum 203536 sheezel 2739 '
          '| seal abb690b6 (expect 3167cba6/804/637355/203536/0)')
    return 0
