...
def green1():
    ...
    ok = ok and facts.get('active') == 804 and facts.get('sum_v') == 764021 and facts.get('sheezel') == 9631
    # C5: compare the COMPLETE active-player value vector against the accepted reference — require ZERO movers.
    movers = None
    ref_path = os.path.join(FIX, 'reference_vector_5546f278.json')
    ...
    record('GREEN1_strict_board_5546f278_zero_movers', ok,
           "rc=%s md5=%s active=%s sumv=%s sheezel=%s vector_movers=%s (expect 5546f278/804/764021/9631/0)%s"
...
def red1():
    ...
    record('RED1_stale_ambient_ignored', ok,
           "stale_seeded=%s board=%s (must be 5546f278, never d7a95e8d) files_unchanged=%s"
