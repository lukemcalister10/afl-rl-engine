"""T1 DERIVATION (part 2) — GAP-RECOVERY rate for the demonstrated-backed residual slice.
...
Usage: derive_kpfsh2.py <out.json>"""
import io, contextlib, json, os, sys
import numpy as np
HERE = '/home/user/afl-rl-engine'
sys.path.insert(0, HERE)
import boot_guard
boot_guard.assert_boot('derive_kpfsh2', store_path='/home/claude/rl_workspace/rl_after/rl_model_data.json')
out = sys.argv[1]
...
def LD_top2(p, Y):
    ...
    ls = sorted((adj(p, x['year'], x['avg']) for x in p['scoring']
                 if x['year'] <= Y and x.get('games', 0) >= 12.0 * (_fEy(Y) if x['year'] == Y else 1.0)),
                reverse=True)
    if len(ls) < 2:
        return None
