#!/usr/bin/env python3
"""LEG A (iso evidence-fade) — B3 walk-forward book RE-SEAL. The book is engine-ev()-derived; the iso
evidence-fade + ISO monotonization (RL_ISOFADE, default ON) move ev() for the rows they touch, so the book's
present-year + walk-forward columns move and stable_sha256 advances. Regenerates the candidate matrix EXACTLY
as ship_gates B3 does (s4_matrix_M1v7.py, gate mode, custom S4_MATRIX path so __meta__ is emitted), verifies
its embedded engine/store/config stamps == the candidate, computes the stable-key sha256, rewrites
data/book_stable_seal.json. Store/config UNCHANGED; only head_md5 + stable_sha256 move. Candidate — owner only.
"""
...
_fd, mpath = tempfile.mkstemp(prefix='s4_reseal_isofade_', suffix='.json'); os.close(_fd)
# NB: SGC_* MUST NOT be in the env here — the gate-mode subprocess rejects any SGC_ var (config_manifest gate-seam scan).
...
if not meta:
    print('FAILED: no __meta__ (exit=%s)\n%s' % (r.returncode, r.stderr[-1200:])); sys.exit(1)
...
seal = {
    "_comment": ("Walk-forward book freeze-stamp — RE-SEALED 2026-07-16 at the LEG A iso-evidence-fade candidate "
                 "(iso_corr fades on the v2.10 evidence weight + the ISO table is monotonized non-increasing in "
                 "pick; RL_ISOFADE default ON). The book is engine-ev()-derived; the fade moves ev() for the rows "
                 "it touches, so it is RE-BUILT on the candidate engine (G-BOOK: the formula moved) and re-stamped, "
                 "not re-hashed. Store/config UNCHANGED. RL_ISOFADE=0 reproduces the v2.10 book. Owner tag/main owner-only."),
    "generator": "engine/rl_after/s4_matrix_M1v7.py",
    "head_md5": HEAD, "store_md5": STORE, "n_players": len(by), "stable_sha256": sha,
    "sealed_by": ("LEG A re-seal 2026-07-16 (session_2026-07-16/isofade; engine fc7045d6 -> %s, the iso "
                  "evidence-fade + monotonization; store b1fd0bce UNCHANGED). stable_sha256 %s -> %s because the "
                  "rows the fade touches move in ev() and thus in the book. head_md5 stamp advanced fc7045d6 -> %s. "
                  "Candidate — owner tag/main promote owner-only." % (HEAD, old.get('stable_sha256', '')[:8], sha[:8], HEAD)),
    "sealed_date": "2026-07-16", "config": cfg_full,
}
