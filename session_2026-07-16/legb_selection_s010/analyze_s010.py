...
# ---- A-PAIRS pair_2 (reid/bont, PARITY band) + pair_3 (sanders/bont, sit 0-10% below) on the ON board ----
wln()
wln("## A-PAIRS (scored on the wired-default board; num-SCAR)")
def _num_on(k): return ON[k]['num'] if (k in ON and good(ON.get(k))) else None
_kr, _kb, _ks = 'harley-reid', 'marcus-bontempelli', 'ryley-sanders'
_reid, _bont, _sand = _num_on(_kr), _num_on(_kb), _num_on(_ks)
if _reid and _bont:
    _p2 = (_reid - _bont) / _bont * 100.0
    # band: memo/A_PAIRS historic +/-10% PARITY; owner re-band to +/-15% recorded (register item 266)
    wln("    pair_2 reid/bont (PARITY): reid %d vs bont %d = %+.1f%%  [+/-10%%: %s | +/-15%% (item266): %s]"
        % (_reid, _bont, _p2, 'PASS' if abs(_p2) <= 10 else 'FAIL', 'PASS' if abs(_p2) <= 15 else 'FAIL'))
else:
    wln("    pair_2: reid/bont keys not both present (%s/%s)" % (_reid, _bont))
if _sand and _bont:
    _p3 = (_sand - _bont) / _bont * 100.0
    wln("    pair_3 sanders/bont (sit 0-10%% below): sanders %d vs bont %d = %+.1f%%  [%s]"
        % (_sand, _bont, _p3, 'PASS' if -10 <= _p3 <= 0 else 'FAIL (sanders above bont)'))
else:
    wln("    pair_3: sanders/bont keys not both present (%s/%s)" % (_sand, _bont))

if _reid and _bont:
    print("PAIR2 reid=%d bont=%d pct=%+.2f" % (_reid, _bont, (_reid - _bont) / _bont * 100.0))
if _sand and _bont:
    print("PAIR3 sanders=%d bont=%d pct=%+.2f" % (_sand, _bont, (_sand - _bont) / _bont * 100.0))
_bo = by.get('marcus-bontempelli')
if _bo:
    print("BONT scar %d->%d (%+d) rank %d->%d (%+d)" % (_bo['num_off'], _bo['num_on'], _bo['dscar'], _bo['rank_off'], _bo['rank_on'], _bo['drank']))
_eng_a, _eng_b = 'timothy-english', 'kieren-briggs'
if _eng_a in ON and _eng_b in ON:
    print("EB english=%d briggs=%d off=%.3f on=%.3f" % (OFF[_eng_a]['num'], OFF[_eng_b]['num'],
          OFF[_eng_a]['num']/OFF[_eng_b]['num'], ON[_eng_a]['num']/ON[_eng_b]['num']))
