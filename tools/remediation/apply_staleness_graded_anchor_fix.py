#!/usr/bin/env python3
"""Apply the pre-derived D8 graded-staleness treatment with correct forward-lens anchoring.

The first Round 19 candidate exposed a semantic integration defect: _staleness_grade(p, Y, pos)
looked for real scoring rows in the future evaluation years 2027/2028. Those rows cannot exist at the
Round 19 information cut, so the grade became zero and the old stalled cap was re-applied in both
forward lenses. This bounded correction uses the engine's existing form-anchor year, _fa_year(Y), for
the evidence grade. Present and historical evaluations are unchanged because _fa_year(Y)==Y outside
the Leg-E forward lens; under Leg E both +1 and +2 use the true-now 2026 evidence anchor while the
rest of ev() still evaluates the requested future year.

No knots, scores, player rows, fitted artifacts, or named-player exceptions are changed.
"""
...
def _staleness_grade(p,Y,pos):
    \"\"\"Evidence release for the stalled-one-season population at evidence year Y.\"\"\"
...
BLOCK_OLD = """    if el>=onset and ns<=1:                                   # stalled: essentially no production after the window
        ...
        e=min(e, v0*frac)"""
BLOCK_NEW = """    if el>=onset and ns<=1:                                   # stalled: D8 graded release by true-now evidence
        frac=0.25*max(0.4,1-0.10*(el-onset))*(1.6 if keyruc else 1.0)
        cap=v0*frac
        # Forward lens law: form/evidence is anchored to true-now by Leg E. Asking for a future
        # valuation year must not look for unknowable future scoring rows and silently set grade=0.
        gr=_staleness_grade(p,_fa_year(Y),pos)
...
def main() -> None:
    ...
    boot["_staleness_graded_note"] = (
        "OWNER-AUTHORIZED review candidate 2026-07-22: exact D8 graded-staleness curves; "
        "forward evaluations grade evidence at the existing Leg-E true-now form anchor via _fa_year(Y), "
        "rather than treating absent future score rows as zero evidence. No named-player tuning."
    )
    ...
    print(json.dumps({"engine_before": before, "engine_after": after,
                      "knots_changed": False, "player_overrides": 0,
                      "forward_grade_year": "_fa_year(Y)"}, indent=2))
