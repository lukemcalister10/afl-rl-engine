...
def main(root=None, keep=False, only=None, evidence_dir=None, base=None):
    ...
    if not only or 'ui' in only:
        ...
        # THE WRITER COUNT IS ASSERTED, NOT DESCRIBED. It moved 6 -> 7 when the draft outcome
        # record gained its writer of record (owner UI item 13), and this line is what makes adding
        # a writer a deliberate act: a writer added to steps.ui without being counted here fails the
        # control, and a writer silently REMOVED fails it too — which is the direction that matters,
        # because a missing writer is how the v0 sidecar went stale for a day.
        record('writer6_clean_control',
               rc == 0 and rep6.get('ok') is True and ui_facts.get('writers') == 7
               and ui_facts.get('club_valuation_moved') is False and cv_after == cv_before
               and ui_facts.get('draft_outcomes_moved') is False,
               'exit %s; the ui step ran %s writer(s); the picks bundle regenerated BYTE-IDENTICAL '
               '(%s), %s pick(s) stamped to board %s; the draft outcome record regenerated '
               'byte-identical too (%s selections, pinned to store %s)'
               % (rc, ui_facts.get('writers'), str(cv_after)[:12], ui_facts.get('n_picks'),
                  ui_facts.get('club_valuation_pin'), ui_facts.get('n_selections'),
                  ui_facts.get('draft_outcomes_pin')))
        sb.reset()
