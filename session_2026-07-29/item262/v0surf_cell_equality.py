...
for nsig, osig in pairs.items():
    ...
    if osig not in frozen:
        sys.stderr.write('MISMATCH: reverse-mapped %s not in frozen set\n' % osig[:12]); allsame = False; continue
    diffs = []
    walk(norm(built[nsig]), frozen[osig], '', diffs)
    count(norm(built[nsig]))
    sys.stderr.write('COMPARE %s (new) vs %s (frozen): %s\n'
                     % (nsig[:12], osig[:12], 'IDENTICAL' if not diffs else '%d DIFFERENCES' % len(diffs)))
