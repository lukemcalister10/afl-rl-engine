...
assert seal == struct['seal_sha256_8'] == 'ed5b7fcc', 'SEAL DRIFT %s/%s' % (seal, struct['seal_sha256_8'])
ENT = struct['entrant_pvc']['total']
board_ent = board.get('phantomTotals', {}).get('_meta', {}).get('entrant_layer_pvc')
print('ENTRANT LAYER (sealed ed5b7fcc): %d PVC (draft %d + mech %d)  | board emitted: %s  | MATCH %s'
...
assert board_ent == ENT, ('F5 RECONCILIATION FAIL: board emitted %s != sealed total %d' % (board_ent, ENT))
