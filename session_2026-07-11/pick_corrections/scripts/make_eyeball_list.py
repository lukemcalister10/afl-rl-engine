...
lines.append('- Gate suite: all green except the owner-ruled expected reds **{A2, A3, A12}** (data-caused, unchanged by this build); B4 board-parity PASS; B3 book-seal PASS (candidate re-seal); B5 raise-only intact (0 lowered).')
lines.append('')
...
json.dump({'factor':FACTOR,'rows':rows,'floor_dippers':len(DIP),'worst_dip_scar':worst},
          open(os.path.join(REPO,'session_2026-07-11/pick_corrections/out/eyeball_rows.json'),'w'))
