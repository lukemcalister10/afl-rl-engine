...
bash "$REPO/bootstrap.sh" >/tmp/boot.log 2>&1 && echo "bootstrap OK (engine $(md5sum $WS/_merged_recover.py|cut -c1-8))"
...
python3 rl_export.py 2>&1 | grep -E "EXPORT ATTRIBUTION|PARITY GATE|exported active|NUMÉRAIRE" || true
