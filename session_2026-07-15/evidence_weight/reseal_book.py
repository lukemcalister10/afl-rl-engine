#!/usr/bin/env python3
"""EVIDENCE WEIGHT — B3 walk-forward book RE-SEAL. The book is engine-ev()-derived; replacing the four
evidence regimes with one continuous weight moves ev() for the rows the four regimes touched, so the book's
present-year and walk-forward columns move and stable_sha256 advances. Regenerates the candidate matrix
EXACTLY as ship_gates B3 does (s4_matrix_M1v7.py, gate mode), verifies its embedded engine/store/config
stamps == the candidate, computes the same stable-key sha256, and rewrites data/book_stable_seal.json.
...
_fd, mpath = tempfile.mkstemp(prefix='s4_reseal_evw_', suffix='.json'); os.close(_fd)
...
seal = {
    "_comment": ("Walk-forward book freeze-stamp — RE-SEALED 2026-07-15 at the EVIDENCE-WEIGHT candidate "
                 "(four evidence regimes -> one continuous weight; pedigree fades to the T5 residual 0.11). "
                 "The book is engine-ev()-derived; the evidence weight moves ev() for the rows the four "
                 "regimes touched, so it is RE-BUILT on the candidate engine (G-BOOK: the formula moved) and "
                 "re-stamped, not re-hashed. Store/config UNCHANGED. Owner tag/main promote remain owner-only."),
    "generator": "engine/rl_after/s4_matrix_M1v7.py",
    "head_md5": HEAD, "store_md5": STORE, "n_players": len(by), "stable_sha256": sha,
    "sealed_by": ("EVIDENCE-WEIGHT re-seal 2026-07-15 (session_2026-07-15/evidence_weight; engine "
                  "113a4223 -> %s one-continuous-evidence-weight; store 340a7a32 UNCHANGED). stable_sha256 "
                  "%s -> %s because the rows the four regimes touched move in ev() and thus in the book. "
                  "head_md5 stamp advanced 113a4223 -> %s. Candidate — owner tag/main promote owner-only."
