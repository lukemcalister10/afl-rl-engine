#!/usr/bin/env python3
"""IMPROVER BUILD — B3 walk-forward book RE-SEAL. The book is engine-ev()-derived; the three improver legs
(RL_EO2 kill the _eo min() · RL_LSYM L-SYMMETRY · RL_SAGE29 the S_AGE 29-tail) move ev() for the rows they
touch, so the book's present-year + walk-forward columns move and stable_sha256 advances. Regenerates the
candidate matrix EXACTLY as ship_gates B3 does (s4_matrix_M1v7.py, gate mode), verifies its embedded engine/
store/config stamps == the candidate, computes the same stable-key sha256, rewrites data/book_stable_seal.json.
Store/config UNCHANGED; only head_md5 + stable_sha256 move. Candidate — owner tag/main promote owner-only.
"""
...
_fd, mpath = tempfile.mkstemp(prefix='s4_reseal_improver_', suffix='.json'); os.close(_fd)
...
seal = {
    "_comment": ("Walk-forward book freeze-stamp — RE-SEALED 2026-07-15 at the IMPROVER candidate (three legs: "
                 "_eo two-directional / L-SYMMETRY / the S_AGE 29-tail). The book is engine-ev()-derived; the "
                 "legs move ev() for the rows they touch, so it is RE-BUILT on the candidate engine (G-BOOK: the "
                 "formula moved) and re-stamped, not re-hashed. Store/config UNCHANGED. Owner tag/main owner-only."),
    "generator": "engine/rl_after/s4_matrix_M1v7.py",
    "head_md5": HEAD, "store_md5": STORE, "n_players": len(by), "stable_sha256": sha,
    "sealed_by": ("IMPROVER re-seal 2026-07-15 (session_2026-07-15/improver; engine edc47017 -> %s three improver "
                  "legs; store 340a7a32 UNCHANGED). stable_sha256 %s -> %s because the rows the legs touch move in "
                  "ev() and thus in the book. head_md5 stamp advanced edc47017 -> %s. Candidate — owner tag/main "
                  "promote owner-only." % (HEAD, old.get('stable_sha256', '')[:8], sha[:8], HEAD)),
