...
def _o33_ladder(rho0=0.025,g=0.010,n=14):                   # B-1 family, the derivation's own construction (b2_fit.ladder_of). R-VETLEAN RE-TUNE (#334 c.5316404479, PREREG_B_A1 + its one-iteration rule, both exercised and disclosed): the owner's lean-over-not-under preference ruling moves the point INSIDE the fitted CI box (rho0 CI [0,.08], g CI [.010,.045]). Iteration 1 chose (.050,.0125) off the calibration build; the BUILT cells came in ~.07 under the offline transfer (s* lifts only the projection leg while veteran floors bind, so the anchored instrument's anchor rises more than veteran marks — measured, VETLEAN_ITER2.json). The prereg'd single re-selection with the built-cell calibration lands at (.025,.010) — the max-min frontier point, predicted cells 27:1.105 28:0.987 29:0.946 30:1.175 31:0.941 (floor .945 within the .010 verify tolerance; the 30-cell leans over, ~.15 of headroom under the measured call threshold ~1.32). The W5 harness on the built board is the ruled binding check; the full frontier and both iterations are published (VETLEAN_*.json).
    f,out=1.0,{}
    for j in range(1,n+1):
        f*=(1.0-min(0.60,max(0.0,rho0+g*(j-1)))); out[j]=f
    return out
O33_TALL_LADDER=_o33_ladder()                                # f(1..5)=.975/.9409/.8998/.8524/.7992 (ages 28-32; the fit optimum was .970/.9167/.8433/.7548/.6566 — the re-tune softens the SLOPE inside the CI box, the fitted first step is nearly kept); tail = the family continued (W5 measures nothing past 31 — extrapolation-by-rule, bounded by the frac<0.42 projection stop)
O33_SSTAR_PIN=1.1909                                          # s* PINNED for the R-VETLEAN iter-2 ladder (re-derived 2026-08-17 at base 91e1b07-era tree, RL_O32=1 stage-1 fixed point on the frozen pre-anchor basis: conserves the 55 tall-anchor rows 23-26 to -0.05%, inside the prereg'd 0.2% tolerance; log SSTAR_A2_out.txt. The iter-1 s* 1.3451 and the fit-optimum s* 1.2988 retired with their ladders.)
