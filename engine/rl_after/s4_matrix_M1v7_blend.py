...
def ev_blend(p,Y=2026):
    ...
    par=PR.par_at(pos,min(MA.effpk(p),cp.KMAX),min(max(el,1),6)); pr=bestlvl(p,Y)/max(1,par)
    N=min(max(el,1),6); anchor=dv*RETAIN_RAISED[_sitout_cls(pos)][N-1]   # retain<1 => capped at 1.0 x dv (prorate a penalty, never reward)
    w=_wramp(_cgames(p,Y))
    if ns==0:
    ...
    if el>=onset and ns<=1:
        ...
        frac=0.45*max(0.3,1-0.08*(el-onset))*(1.5 if keyruc else 1.0); e=min(e, dv*frac)
    return round(e)
ev=ev_blend
print("[PART A] +0.1 retain raise + w-blend (G_FULL=44,WSHAPE=1.3) wired into ev; PLACEHOLDER, nothing baked",flush=True)
