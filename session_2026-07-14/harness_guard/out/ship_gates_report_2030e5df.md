...
=== SHIP GATES BOARD — head 2030e5df store 340a7a32 config 69ead79b944d — suite 764a0d91 — 2026-07-14 ===
...
B4        PASS    | PASS    | FAIL     regenerated rl_app_data.json md5=MISSING vs shipped 3dc19fbb (byte-agree gate; export exit=1)  <- MOVED
...
H1        —       | —       | PASS     no code lookalike: 0 files outside the 9 canonical runners reproduce >= 0.50 of one (min 12 shared sig-lines). Tripwire = SSI Guard 3, pointed at code.  <- MOVED
H2        —       | —       | PASS     no masked gate in the live harness (root + hooks; closed session dirs excluded as the record): every live runner sets pipefail and captures gate exit codes. Lint = the item-38 `| tail -8` signature, generalised to the class.  <- MOVED
          columns: CONTROL | PREVIOUS | CURRENT (three-column rule; snapshots data/gates_snapshots/)
VERDICT: FAIL=5  FEATURE=1  HALT=1  PASS=16  PENDING=4  STRUCK=1  (223s)
```
