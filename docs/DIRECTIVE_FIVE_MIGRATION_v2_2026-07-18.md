# DIRECTIVE — THE FIVE-MIGRATION · v2 · seat 13 · 2026-07-18 · SUPERSEDES the same-day v1
### v2 (item 333): the v1 chat mis-based itself (auto-branched from main, then asserted a false
### repo fact without a fresh fetch) and was HALTED + RETIRED with nothing pushed. Job content
### UNCHANGED — v2 adds the GIT ENTRY block and the no-improvised-topology law. Ruled R107.5
### (DECISIONS v123) · released item 328 · authored by seat 13 (item 330).
...
## GIT ENTRY (verbatim; do this before anything else)
Your workspace will open on `main` (or an auto-created branch off it). **That is NOT your base.**
First acts, exactly:
`git fetch origin main claude/legd-pvc-rederivation-act2-l2hqpl`
`git checkout e4177c21934148c19d9cec3c015fee5d28480102`
`git checkout -b claude/five-migration-<fresh-suffix>`
Discard any auto-created branch; push nothing from it. EXPECTED TOPOLOGY, stated so it is never
"discovered": the governance docs (this directive · DECISIONS · HANDOVER · the register) live on
MAIN; the engine candidate lives on the pinned branch stacked PR #105 → #109. Both are true at
once. This is the designed shape, not a conflict. **NEVER merge, rebase, or otherwise join the two
lineages** — main merges are the owner's alone. A store/board/book md5 is a CONTENT CHECK, never a
place to stand (CORE rule 2): there are no "worlds," only branch names + git SHAs. Every
repo-state claim you make must cite a fetch run in THIS session. If any step appears to require a
topology decision this directive does not spell out — or looks "hard to reverse" — that is itself
the HALT signal: stop, push nothing, return the conflict in ≤10 lines. Nothing in this job is
legitimately hard to reverse.
