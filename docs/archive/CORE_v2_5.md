# CORE — AFL SuperCoach RL engine — v2.5 · 2026-07-13 · supersedes v2.4
### v2.5 CLOSES THE v2.9 SEAM. Six folds, no doctrine reversed:
### (1) THE SPEED RULES (S1–S6) get their own section — owner-ruled 2026-07-13, operative in every
###     directive; S6 (audit-sizing) joins them. (2) SILENCE IS A RED — a check that produces no
###     result is a FAILURE, not a pass (born of the 2026-07-13 silent-gate defect: a binding gate
###     crashed behind a `| tail` pipe and the suite reported PASS). (3) THE BASE-PIN RULE for
###     directives (a pin reads "at or after <SHA> + docs/-only diff", never strict equality, while
###     the supervisor's docs pen is live). (4) THE POSITION TAXONOMY is stated here because a
###     previous seat got it wrong and it DRIVES the cohort curves. (5) SSI: the repo file is
###     `SINGLE_SOURCE_INVARIANT.md` (unversioned filename; the version lives in its header) and its
###     repo copy was BEHIND v1.2 until this seam. (6) M1/F11 OBITUARY.
### v2.4 added THE SEAM-PACK RULE (owner-ruled 2026-07-13). v2.3 folded the v2.8-chapter rules:
### TIERED LADDER + one bake per chapter · CONCURRENCY · OWNER ARTIFACTS · FROZEN-THINGS PRINCIPLE ·
### THE REGISTER · research directives carry IMPLICATIONS · supervisor synthesis after each research
### wave · archaeology verified against the CURRENT tree · STOP-and-ask on any missing FEED input ·
### SUPERVISOR TOKEN PEN (docs/-only pushes, SHA cited same turn, audited first by each incoming
### seat) · journals are CUMULATIVE. v2.2 (owner-ruled 2026-07-10, "repo is home"): repo `docs/` is
### the CANONICAL pack home. Earlier changelogs: archive/CHANGELOG.
...
in CONSTRAINTS.
**FILE NOTE (v2.5):** the invariant lives at repo root as `SINGLE_SOURCE_INVARIANT.md` — the FILENAME
carries no version; the VERSION lives in the file's header (v1.2 as of this seam). The repo copy was
behind v1.2 until 2026-07-13; it is now current. Do not create a versioned duplicate filename — that
would be a lookalike of the very document that forbids lookalikes.
...
FRESHNESS CHECK: full-URL ls-remote + the register header + the docs/ list, compared against the manifest
...
SHAs live) → `docs/` OPEN_ITEMS_REGISTER (the only list) → HANDOVER (state) → DECISIONS (open rulings) →
