# ADVERSARIAL REVIEW PROMPT — PVC+FLEX chapter spec v1.1 · 2026-07-16
### FLOW: paste EVERYTHING below the line into a FRESH GPT chat with no prior context. Run once.
### Courier the raw output back to the supervisor seat unedited.
---

You are a hostile technical reviewer red-teaming a design spec before implementation. The system: a
deterministic player-valuation engine for an AFL fantasy dynasty league. ~800 players priced in one
currency (SCAR); draft picks priced by a pick-value curve (PVC) with pick 1 pinned = 3000. One
authored data store; everything else derived and regenerated. Hard gates HALT builds (never warn).

STANDING LAW you must treat as fixed (the spec must be consistent WITH it, not re-argue it):
- No net value strip from young players; cures are lift-led, conserving (value moves, never
  vanishes), and two-directional within the young side. PROTECTED QUANTITY (a just-made ruling): a
  young player's EARNED/PRODUCTION value may never fall; his UNEARNED draft-pick prior may dissolve
  in either direction as evidence accumulates.
- L-AXIS: a mispricing between two named players is cured on the axis where it lives; moving a third
  population to close a named gap is forbidden (carve-out: globally-motivated, globally-measured
  recalibrations are fine).
- L-SYMMETRY: the same weight of evidence to believe improvement as decline; ramps, never one-sided
  cliffs. L-SMOOTH: no value discontinuities; a standing census instrument scores this.
- G-COHORT: young-cohort ratio hard ceiling 1.30, y1–y5. Acceptance (owner-ruled): Tim English must
  price ≥ 1.75× Kieren Briggs (output ratio is 5.77×; today priced 1.56×); proven-27+ output→price
  elasticity β ≥ 0.85 with CI containing 1.0 (today 0.683); the ≤22 slope must RISE from 0.111.
- G-Y0 (just ratified BINDING): the pick curve must equal expected entering-player value at each pick
  "over the wider sample, on average" (population-level; a single class top-heavy in a cheap position
  is NOT a breach). Numéraire: pick 1 = 3000, permanent.
- Position taxonomy: three single-valued store columns (drafted/present/future position); the future
  BLEND is a ruled stream construct (primary + at most one alternate; the dual stream only swaps the
  future replacement bar for the LOWER of the pair; the player is never revalued "as" the alternate).
- Posture lenses (contender/balanced/rebuilder) are display re-weightings; BALANCED is canonical and
  the only board that gates, bakes, or seals. 2027 pick discounts: 10%/15%/5% (bal/cont/reb).

YOUR TASK — attack the spec below on these NINE surfaces, findings only, no praise:
1. Internal contradictions (leg vs leg, leg vs law, acceptance vs law).
2. Acceptance gates that are unmeasurable, under-specified, or GAMEABLE (a build could pass the
   letter while violating the intent — say exactly how).
3. Sequencing/dependency risks in A→B→C→D→E (what breaks if a leg's output differs from assumption).
4. Conservation loopholes: any path where SCAR appears or vanishes rather than moves; any way "the
   young side nets ≥ 0" can be true while the intent is violated.
5. Edge cases of the earned-vs-unearned ruling: constructions where "production credit untouched" is
   claimed but effectively false (interaction of multipliers, ordering of operations, players with
   near-zero evidence, mid-season boundary cases).
6. Posture-lens leakage: any route by which a non-canonical lens could influence gating, baking,
   sealed artifacts, or owner decisions that feed back into pricing.
7. PVC circularity: Leg D derives the pick curve FROM player values that themselves carry (fading)
   pick priors from Leg A — is there a feedback loop, double-count, or degenerate fixed point? How
   would you detect it?
8. G-Y0 population gate: how could it PASS while the curve is materially wrong (sample composition,
   position mix shifts, tolerance abuse)? Name a concrete test that would catch it.
9. Missing failure modes/tests: what would you add to the acceptance set, at most one line each.

OUTPUT FORMAT (strict): a numbered list of findings. Each finding: [SEVERITY: BLOCKER | MAJOR |
MINOR] · the spec section it attacks · the attack in ≤4 sentences · ONE concrete test that would
expose it. If a surface yields nothing, write "Surface N: no finding" on one line. No redesign
proposals beyond a one-line fix sketch. No summary, no compliments.

THE SPEC UNDER REVIEW (verbatim):
...
### FEED the law verbatim (CORE: never a prose restatement of constraint content).

## §0 — WHAT THIS CHAPTER CURES (one paragraph, plain)
The engine underpays its best proven producers and taxes some of them forever for where they were
drafted. Tim English produces 5.77× Kieren Briggs above replacement and is priced 1.56× (item 170).
Two measured mechanisms: (1) the output→price map is CONCAVE AT THE TOP (β = 0.364 peak · 0.683
proven-27+, CI excludes 1 at every band), and (2) `iso_corr` is a PERMANENT, EVIDENCE-BLIND pick tax
(item 132: pick-19 = 0.882 < pick-34 = 1.000, applied every year of a career). This chapter lifts the
compressed proven side ON ITS OWN AXES (L-AXIS, BINDING), implements the ruled flex positional
machinery from the owner's filed registers, re-derives the pick value curve on the cured map, and
rebuilds the lens projection — including the NEW owner-raised posture lenses.

## §1 — THE BINDING LAW (fed VERBATIM to every directive; citations, not restatements)
- **Items 130–131 (owner-worded, operative):** NO net-strip of the young side — a young net-cut is
  wrong-direction AS MEASURED LAW (item 170: star-track young UNDER-priced vs realisation). Every cure
  is LIFT-LED · genuinely CONSERVING (value moves, never vanishes) · TWO-DIRECTIONAL within the young
  side (over-performers never cut) · G-COHORT re-run at spec time.
- **Item 132 (verified in code):** the `iso_corr` cure = an EVIDENCE FADE in the house family on the
  v2.10 evidence-weight `w` — full strength at zero evidence (the debut/V0 leg where the pick IS the
  information survives automatically), dissolving toward 1.0 as evidence saturates. A PURE LIFT on the
  compressed player. Monotonization of the ISO table then matters only where the pick still speaks.
- **L-AXIS (CONSTRAINTS v1.17, BINDING):** a named-pair mispricing is cured on the axis where it
  lives; another cohort's lever is never the instrument. Carve-out + owner-only waivers on the law.
- **R103.5 (override doctrine):** in-stats convictions are MODEL-DEFECT claims the model must reach.
  NAMED WATCH ROWS: **Gawn ↑ · Bontempelli ↑ · Harley Reid (relative band)**.
- **R103.6 (the Reid constraint, operative spec law):** the ≤22 output→price slope is β = 0.111 —
  price rides runway volume, not intensity. The cure prices PROJECTED years through the SAME
  un-compressed map (anti-low-intensity, never anti-young). Young side triple-guarded: G-COHORT hard
  1.30 (waiver expired — v2.10's y5 = 1.3057 would HALT today) · the calibration surface · the Reid row.
- **R103.3 (lens projection law):** the +1/+2 lens shows PROJECTED value with expected production
  credited; the UI toggle stays disabled until this leg lands.
- **Flex spec v1.1 §1.2 + item 20b (ruled blend semantics):** one future position per player = a
...
and evidence-blind (picks 19–33 = −10..−19%, pick-34 = 1.000) · ≤22 slope β 0.111 · star-track young
under-priced 1.44× at top picks rising ~7.7× late, unconditional pedigree over-priced · census-v2
unearned gauge at 125 players / +15,612 SCAR (the chapter's standing disease meter — it must not grow).
...
LENS now=0.34 · bal=0.14 · fut=0.05): **BALANCED = the canonical profile, unchanged — the only board
that gates, bakes, or seals. CONTENDER** = current/+1 weight UP, future discount UP (owner sketch:
0.14 → 0.18–0.20). **REBUILDER** = current weight DOWN, future discount DOWN (0.14 → ~0.10).
Implemented as PARAMETERIZED PROFILES (no new model, no new levers — a lens is a re-weighting of the
same per-season streams); exact preset numbers are §6.2 owner parameters, ratified AFTER a
MOVERS REPORT (top-20 risers/fallers per lens vs balanced) at first render — tuned from evidence, not
a priori. Picks price through the active lens (a rebuilder naturally values future picks more).

## §4 — ORDER, CONCURRENCY, HYGIENE
