#!/usr/bin/env python3
"""ORDER M — THE ND BAND TABLES, IN BOTH WINDOWS. ORDER L's ol_bands.py, REUSED WHOLE.
...
P('=' * 118)
P('ORDER M — ND BAND TABLES, BOTH WINDOWS, ON SIX BOARDS')
P('=' * 118)
...
LABELS = [('M0ETA0', 'ORDER M0 73bf9617 — ORDER K\'s knobs with ETA SET TO ZERO'),
          ('MLOETA0', 'ORDER MLO ba97fb73 — the COOLEST eta=0 point in the whole grid (dose 0, k .15, gu 16)'),
          ('MMIN031', 'ORDER MMIN — the SMALLEST LEGAL ETA anywhere (dose 0, eta 0.31)'),
...
json.dump(OUT, open(os.path.join(HERE, 'BANDS_M.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'BANDS_M_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote BANDS_M.json / BANDS_M_out.txt')
