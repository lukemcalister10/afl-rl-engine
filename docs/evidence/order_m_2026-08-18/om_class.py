#!/usr/bin/env python3
"""ORDER L — THE YEAR-1 CLASS MARK, RECOMPUTED ON THE 2005/06 EXCLUSION BASIS.
...
P('=' * 118)
P('ORDER L — THE YEAR-1 CLASS MARK, WITH AND WITHOUT THE 2005 AND 2006 COHORTS')
P('=' * 118)
...
json.dump(dict(ok_class=OUT, w2=W2, supervisor_ex=SUPERVISOR_EX_OKRULED,
          ...
          open(os.path.join(HERE, 'CLASS_M.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'CLASS_M_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote CLASS_M.json / CLASS_M_out.txt')
