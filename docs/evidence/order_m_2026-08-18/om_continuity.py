#!/usr/bin/env python3
"""ORDER M — G12, CONTINUITY, at ETA = 0 (ORDER K's ok_continuity.py, reused; the ONLY change is eta 0.50 -> 0.00). No cliffs in age, in games, or in pick.
...
DOSE, KAP, GU, ETA, GD, REL = 0.40, 0.20, 8.0, 0.00, 14.0, 1.08   # ORDER M: ETA SET TO ZERO
...
P('ORDER M — G12, CONTINUITY, at ETA = 0. ORDER K ok_continuity.py reused; the ONLY change is eta 0.50 -> 0.00.')
...
json.dump(OUT, open(os.path.join(HERE, 'CONTINUITY_M.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'CONTINUITY_M_out.txt'), 'w').write('\n'.join(L) + '\n')
