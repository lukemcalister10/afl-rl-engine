#!/usr/bin/env python3
"""ORDER L — THE POOL-ARM TABLES, BOTH WINDOWS, WITH AND WITHOUT THE 2005/06 COHORTS.
...
LABELS = [('M0ETA0', "ORDER M0 73bf9617 — ORDER K's knobs with ETA SET TO ZERO"),
          ('MLOETA0', 'ORDER MLO ba97fb73 — the COOLEST eta=0 point in the whole grid'),
...
P('=' * 118)
P('ORDER L — POOL-ARM TABLES, BOTH WINDOWS, WITH AND WITHOUT THE 2005/06 COHORTS')
P('=' * 118)
...
json.dump(OUT, open(os.path.join(HERE, 'ARMS_M.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'ARMS_M_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote ARMS_M.json / ARMS_M_out.txt')
