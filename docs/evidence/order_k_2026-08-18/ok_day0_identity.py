...
OUTP = os.path.join(HERE, 'DAY0_V0_%s.json' % LANE.upper())
...
if LANE == 'on':
    env.update(RL_O36='1', RL_O36_LAM_S1='0.40', RL_O36_TALL='1', RL_O36_FLOORFIX='1',
               RL_O36_KAPPA='0.20', RL_O36_GAMMA='8.0', RL_O36_ETA='0.50', RL_O36_GAMMA_D='14.0',
               RL_O36_LAMBDA='1.08')
else:
    env.update(RL_O35='1')
...
other = os.path.join(HERE, 'DAY0_V0_%s.json' % ('OFF' if LANE == 'on' else 'ON'))
if os.path.exists(other):
    ...
    print('  derived_v0 (THE RAW ENTRY OBJECT) BIT-IDENTICAL : %d of %d   -> %s'
          % (len(same_v0), len(rows), 'PASS' if not diff_v0 else 'FAIL — K6 FIRES: %s' % diff_v0[:10]))
    ...
    json.dump(dict(n_wired=len(rows), derived_v0_identical=len(same_v0),
              ...
              open(os.path.join(HERE, 'DAY0_IDENTITY_K.json'), 'w'), indent=1, sort_keys=True)
    print('\nwrote DAY0_IDENTITY_K.json')
