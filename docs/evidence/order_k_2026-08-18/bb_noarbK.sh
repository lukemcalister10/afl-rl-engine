#!/bin/bash
# ORDER K — the standing no-arb instrument runs. The instruments are ORDER 29's / 31-F's DISCLOSED
...
# matrices: CANDIDATE 31 (O31FFINAL), the LANDING CANDIDATE (O35FINAL = 1f176444) and ORDER K
# (OKRULED = f3101883, the owner's ruled setting with the fixed fade floor).
set -uo pipefail
...
cd "$N"
for L in O31FFINAL O35FINAL OKRULED; do
...
} 2>&1 | tee "$HERE/NOARB_K_out.txt"
cp "$N"/t338ext_O31FFINAL.txt "$N"/t338ext_O35FINAL.txt "$N"/t338ext_OKRULED.txt "$HERE/" 2>/dev/null || true
