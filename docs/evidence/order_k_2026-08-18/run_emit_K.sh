#!/bin/bash
# ORDER K — the walk-forward per-entrant emit. ORDER J's run_emit_o37.sh with the label and the
# staging root re-pointed and RL_O36_FLOORFIX passed through. The ORDER 31-F emitter
# (docs/evidence/candidate_31f/emit_matrix_31f.py, itself the 29C disclosed copy) is BYTE-CARRIED; the
# engine is THIS TREE's wired engine. Same env, same five-var thread pinning, strictly sequential.
set -uo pipefail
LABEL=${OK_LABEL:-OKRULED}
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(cd "$HERE/../../.." && pwd)"
SP=/tmp/claude-0/-home-user-afl-rl-engine/7ac96fea-1199-5b6a-9d77-ded9f53694f7/scratchpad
OUT=$SP/ok/emit_$LABEL
...
# ORDER K: the day-0 guard is RE-BASED on this board, exactly as ORDER J's was and ORDER D's before
# it. LEVER 3 changes the SITTER FADE, and a day-0 sitter price IS entry value x sitter fade, so the
# landing candidate's printed file cannot match by construction. The RAW ENTRY OBJECT derived_v0 is
# byte-identical on all 89 rows; only the printed price moves. Disclosed as a gate result.
export RL_DAY0_FINAL="${RL_DAY0_FINAL:-$HERE/DAY0_K.json}"
echo "  emit starting $(date -u +%H:%M:%S)  (RL_O36=${RL_O36:-unset} lambda_S1=${RL_O36_LAM_S1:-default} floorfix=${RL_O36_FLOORFIX:-default})"
S=$(date +%s)
