#!/usr/bin/env python3
"""ORDER K — THE STANDING TWO-SIDED NO-ARB SUITE, in the format the owner has just re-confirmed.
...
LABELS = [l for l in ('O35FINAL', 'O31FFINAL', 'OKRULED')
          if os.path.exists(SP + '/ok/noarb/table_EXT_%s.json' % l)]
NICE = {'O35FINAL': 'THE LANDING CANDIDATE 1f176444 (Order D) — the base every gate is scored against',
        'O31FFINAL': 'CANDIDATE 31 fe6be9d6 — the earlier board, carried as the owner\'s comparison column',
        'OKRULED': 'ORDER K f3101883 — THE OWNER\'S RULED SETTING (dose 0.40 k 0.20 gu 8 eta 0.50 gd 14 rel 1.08) '
                   'WITH THE FIXED FADE FLOOR'}
...
P('=' * 118)
P('ORDER K — STANDING TWO-SIDED NO-ARB SUITE.  carry charge = 14%/yr.')
...
for lab in [x for x in LABELS if x != 'O35FINAL']:
    P('\n-- THE MOVE, BAND BY BAND (%s minus the landing candidate, in points of yr0->1 appreciation) --' % lab)
    P('  %-16s %12s %12s %12s   %s' % ('band', 'candidate', lab, 'move', 'verdict move'))
    for b in CLASSIC + BANDS5:
        a0 = OUT['nd']['O35FINAL'][b]['apprec01']; a1 = OUT['nd'][lab][b]['apprec01']
        P('  %-16s %+11.2f%% %+11.2f%% %+11.2f   %s -> %s'
          % (b, 100 * a0, 100 * a1, 100 * (a1 - a0),
             OUT['nd']['O35FINAL'][b]['verdict'], OUT['nd'][lab][b]['verdict']))
...
for lab in [x for x in LABELS if x != 'O35FINAL']:
    P('\n-- THE MOVE, ARM BY ARM (primary window; %s minus the landing candidate) --' % lab)
    P('  %-10s %12s %12s %12s   %s' % ('arm', 'candidate', lab, 'move', 'verdict move'))
    for arm in ARM_TYPES + ['ALLPOOL']:
...
json.dump(OUT, open(os.path.join(HERE, 'STANDING_TABLES_K.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'STANDING_TABLES_K_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote STANDING_TABLES_K.json / STANDING_TABLES_K_out.txt')
