#!/usr/bin/env python3
"""ORDER K — the day-0 guard file, regenerated per board.
...
Usage: K_TAG=<tag> J_DOSE=.. J_KAPPA=.. ... python3 o37_day0.py
"""
import os, sys, json, io, contextlib, hashlib

HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(os.path.dirname(os.path.dirname(HERE)))
SP = '/tmp/claude-0/-home-user-afl-rl-engine/7ac96fea-1199-5b6a-9d77-ded9f53694f7/scratchpad/ok'
TAG = os.environ.get('K_TAG', 'tall')
BOARDP = os.path.join(SP, 'bb_%s' % TAG, 'rl_after', 'rl_app_data.json')
OUTNAME = os.environ.get('K_DAY0_OUT', 'DAY0_K_%s.json' % TAG.upper())

os.environ.update(RL_O31='1', RL_O32='1', RL_O36='1', RL_O36_TALL='1',
                  RL_O36_LAM_S1=os.environ.get('K_DOSE', '0.0'),
                  RL_O36_KAPPA=os.environ.get('K_KAPPA', '0.24'),
                  RL_O36_GAMMA=os.environ.get('K_GU', '11.0'),
                  RL_O36_ETA=os.environ.get('K_ETA', '0.41'),
                  RL_O36_GAMMA_D=os.environ.get('K_GD', '14.0'),
                  RL_O36_LAMBDA=os.environ.get('K_REL', '1.08'),
...
DOC = dict(label='ORDER K — the day-0 reference regenerated on board %s (%s)' % (TAG, BOARD_MD5[:8]),
           board='ORDER K %s' % TAG, board_md5=BOARD_MD5,
...
print('ORDER K PRINTED DAY-0 IDENTITY [%s]: %d of %d at tolerance 0 (ND %d, pool %d) on board %s'
      % (TAG, len(out) - len(mism), len(out), nND, nPOOL, BOARD_MD5[:12]))
if mism:
    print('MISMATCHES: %s' % mism[:10])
    sys.exit('ORDER K HALT: the printed day-0 identity does not hold on the written board.')
