...
mkdir -p /home/claude/dob_ws /home/claude/dob_build /home/claude/rl_vendor

# 1. engine + support modules + harness + pipeline + data files
cp -rf "$HERE/engine/rl_after"          /home/claude/dob_ws/
cp -rf "$HERE/engine/forward_valuation" /home/claude/dob_ws/
...
cp -f "$HERE/config_manifest.py"        /home/claude/dob_ws/rl_after/config_manifest.py
...
cp -f "$HERE/fv_provenance.py"          /home/claude/dob_ws/rl_after/fv_provenance.py
cp -f "$HERE/boot_guard.py"             /home/claude/dob_ws/rl_after/boot_guard.py
...
cp -f "$HERE/LTI_REGISTER.md"           /home/claude/dob_ws/rl_after/LTI_REGISTER.md
...
cp -f "$HERE/data/rl_build/rl_app_data.json" /home/claude/dob_build/rl_app_data.json
...
M=$(md5sum /home/claude/dob_ws/rl_after/_merged_recover.py | cut -c1-8)
C=$(md5sum /home/claude/cm_400.pkl | cut -c1-8)
Q=$(md5sum /home/claude/q97m.pkl | cut -c1-8)
S=$(md5sum /home/claude/dob_ws/rl_after/rl_model_data.json | cut -c1-8)
...
RL_REPO="$HERE" python3 "$HERE/boot_guard.py" bootstrap \
  /home/claude/dob_ws/rl_after/rl_model_data.json \
  /home/claude/dob_ws/rl_after/_merged_recover.py \
  /home/claude/cm_400.pkl \
  /home/claude/dob_ws/rl_after/LTI_REGISTER.md || { echo "bootstrap FAILED: seeded workspace does not match the pinned store/register (see above)"; exit 1; }
...
# engine/forward_valuation -> /home/claude/dob_ws/forward_valuation. Verify that COPIED tree against the
...
ws  = fp.fv_identity('/home/claude/dob_ws/forward_valuation')            # the workspace COPY
...
if [ -n "$FVPIN" ] && [ "$FVPIN" != "None" ]; then
  ...
  [ "$FVWS" = "$FVPIN" ] || { echo "bootstrap FAILED: workspace COPY /home/claude/dob_ws/forward_valuation identity ${FVWS:0:12} != pinned ${FVPIN:0:12} — a STALE forward_valuation copy is in the workspace (re-run bootstrap.sh from the intended checkout; the canonical build binds RL_FV to the checkout, but a stale copy must halt loudly)"; exit 1; }
fi
R=$(md5sum /home/claude/dob_ws/rl_after/LTI_REGISTER.md | cut -c1-8)
echo "bootstrap OK"
...
echo "  ENV (with vendor): PYTHONPATH=/home/claude/dob_ws/rl_after:/home/claude/rl_vendor ; RL_FV=<repo>/engine/forward_valuation (canonical FV source)"
