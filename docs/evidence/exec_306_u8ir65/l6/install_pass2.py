#!/usr/bin/env python3
"""#306 L6 PASS 2 — THE L1(b) ENUMERATED SAME-COMMIT INSTALL, executed as ONE ATOMIC ACT.
...
WHAT THIS IS: `install_pass1.py` by the `2a1xa4` seat, RE-POINTED at pass 2 -- the recorded set, nothing
...
pass-specific constants move, and each one is listed here so the diff against pass 1 is readable:
    ...
    derived curve source   pass0_derived_curve1.json  ->  pass1_derived_curve2.json
...
AUTHORISED = 'b61c01b0350cf113deec5b739c5f679f'      # seam 5187094806 order 3 / fired 5187605206
PRE_CURVE = '9f7848f4'                                # what must be installed before this act
OLD_HEAD_STR = '3010.1221'                            # the CURRENT head — the sealed twin token
OLD_PE = 'e1c62f86'                                   # the per_entrant stamp this act replaces
SRC_SURFACE = '6ba4f4c3'                              # the surface the new curve was derived from
...
DERIV = P('docs/evidence/exec_306_2a1xa4/l6/pass1_derived_curve2.json')
...
NEW_PE_MD5 = md5f(P('docs/evidence/exec_306_2a1xa4/l6/pass1_matrix.json'))[:8]
assert NEW_PE_MD5 == 'bab40626', 'per_entrant stamp %s' % NEW_PE_MD5
...
c['numeraire']['_doc'] = (                                    # BY PATH — sealed twin, occurrence 2
    ...
    'CONSTRUCTION. Re-measured at #306 L6 pass 2 on surface %s; the prior value was %s.'
...
c['derived_from'] = (
    ...
    '#306 L6 pass 2: the matrix emitted on the redesigned lens lane at installed curve %s, committed '
...
c['_working_substrate_note'] = (
    '#306 L6 PASS 2: curve 2 (%s) installed as DECLARED WORKING SUBSTRATE under R-H/R-I/N19, derived '
...
u['_doc'] = ('#306 L6 pass 2 working substrate: pick curve payload %s (ladder sum %d, pick64 %d), '
...
rc['pvc_provenance']['_note'] = ('#306 L6 pass 2: curve 2 installed as working substrate from surface '
...
BK = P('docs/evidence/exec_306_u8ir65/l6/_install_backup')
