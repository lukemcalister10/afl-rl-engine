...
WORK = os.path.join(SP, 'd8m', 'noarb')
...
_STORE_PINS = {
    ...
    'cc02567f': "the TREE'S STORE OF RECORD since de9b8eb (the owner's 112 ownership + 23 pick-owner "
                'moves; that commit records "board byte-identical")',
}
...
assert _TREE_STORE == 'cc02567f', 'the tree store is not the D8 base identity: %s' % _TREE_STORE
P('  Every matrix emitted after de9b8eb carries cc02567f and would FAIL CLOSED on the pin. The pin is')
P('  re-pointed per label below and the accepted set is CLOSED to the two identities above; anything')
P('  else still fails closed. PROOF the move is not a price move, measured by this seat, not asserted:')
P('    * the OFF build on THIS tree (store cc02567f) reproduces board a05fe951 BYTE-EXACT — see REPRO_out.txt;')
P('    * D8BASE (store cc02567f) has recs BYTE-IDENTICAL to D7BCAND (store cb38ef11), the matrix the')
P('      on-file D7B tables were rendered from, and to the bake\'s per_entrant_BK.json.')
_CANON = {}
for _l in ('D8CAND', 'D8CAND2', 'D8BASE', 'D7BCAND', 'BK'):
...
assert _CANON['D8BASE'] == _CANON['D7BCAND'] == _CANON.get('BK', _CANON['D7BCAND']), \
    'THE COMPARISON COLUMN IS NOT THE D7B MATRIX — HALT rather than present it as one'
assert _CANON['D8CAND'] == _CANON['D8CAND2'], 'MATRIX DETERMINISM FAILED on the candidate'
assert _CANON['D8CAND'] != _CANON['D8BASE'], 'THE DIAL DID NOTHING — the pass-through did not fire'
P('  -> comparison column CONFIRMED == the D7B matrix; candidate matrix DETERMINISTIC and NON-VACUOUS.')

# ORDER D8: THE PRICED CANDIDATE LEADS THIS LIST AND THE LIVE BOARD SITS DIRECTLY BENEATH IT.
LABELS = [('D8CAND', '*** THE PRICED CANDIDATE 5ea978f7 — the live dial line + RL_O33_TAPEROFF=1 ***'),
          ('D8BASE', 'THE LIVE BOARD a05fe951 — dial unset, same tree. THE COMPARISON COLUMN '
                     '(recs byte-identical to the D7B matrix)')]
...
json.dump(OUT, open(os.path.join(HERE, 'BANDS_NOARB_D8.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'BANDS_NOARB_D8_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote BANDS_NOARB_D8.json / BANDS_NOARB_D8_out.txt')
