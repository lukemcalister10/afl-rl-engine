...
BD = json.load(open(os.path.join(HERE, 'BANDS_NOARB_D8.json')))['nd']
AJ = json.load(open(os.path.join(HERE, 'STANDING_TABLES_NOARB_D8.json')))
ARMS = AJ['arms']
...
CAND, BASE = 'D8CAND', 'D8BASE'
NICE = 'THE PRICED CANDIDATE 5ea978f7 — the live dial line + RL_O33_TAPEROFF=1'
VKEY = 'ALLCOH'
...
h.append('<div class="sub"><b>THE READING RULE, in plain words.</b> A group is fairly priced if it '
         ...
         'board\'s raw years are in <code>BANDS_NOARB_D8_out.txt</code> and '
         '<code>STANDING_TABLES_NOARB_D8_out.txt</code> beside them.</div>')
...
h.append('<div class="sub" style="border-left:3px solid var(--acc);padding-left:10px">'
         '<b>WHAT THIS PAGE IS.</b> The standing no-arb table set, computed on <b>the priced '
         'candidate\'s own walk-forward matrix</b>, with <b>the live board a05fe951 carried beside it '
         'in every cell</b> so the owner can rule with the standing-law tables in hand. '
         '<b>THE BOARD IS PRICED, NOT ADOPTED.</b> The live board has not moved: this seat rebuilt '
         '<code>a05fe951f78482c70520480e184c80ec</code> byte-exact in both dev-shell and canonical '
         'posture before measuring anything, and moved no pin, no store, no config and no register.'
         '</div>')
...
h.append('<div class="sub" style="border-left:3px solid var(--warn);padding-left:10px">'
         ...
         'The candidate is that board <b>plus one thing</b>: <code>RL_O33_TAPEROFF=1</code>, the '
         'ORDER D8 ceiling-only dial, which retires the B-3 post-peak taper. Where the box quotes '
         'the two modern buy-side reds at <b>+21.52%</b> and <b>+15.04%</b>, those are '
         '<b>the LIVE BOARD\'s</b> numbers; the candidate\'s own readings of the same two cells are '
         'printed in the tables and in the ruled-red ledger below and are <b>not</b> the same.</div>')
...
h.append('<div class="sub">Five ND bands plus the classic three (ALL 1-64 / 1-20 / 21-64), in BOTH '
         ...
         '<code>per_entrant_D8CAND.json</code> (<code>b7808db0</code>), with the live board\'s cell '
         'beside each one. Raw: <code>BANDS_NOARB_D8_out.txt</code>. The ND cohort clock is '
...
h.append('<div class="sub">Every pool pathway, both windows, the same standing format, the live '
         ...
         'asserted at run). Raw: <code>STANDING_TABLES_NOARB_D8_out.txt</code>.</div>')
h.append('<div class="sub" style="border-left:3px solid var(--warn);padding-left:10px">'
...
for lab, wkey, name, a01, pt in nd_breach + arm_breach:
    ...
    h.append('<tr class="red"><td class="l">%s</td><td class="l">%s</td><td class="l">%s</td>'
             ...
             % ('CANDIDATE 5ea978f7' if lab == CAND else 'live board a05fe951',
...
h.append('<div class="sub"><b>A breach is a cell whose year-1 appreciation exceeds +14%. '
         ...
         'above. <b>No cell breaches on the candidate that does not also breach on the live board</b>, '
         'and no cell that breaches on the live board stops breaching on the candidate.</div>')
...
h.append('<div class="sub">The registered W2 basis: DRAFT classes 2005-2015 (cohort years 2006-2016). '
         ...
         'Raw: <code>CLASS_D8_out.txt</code>.</div>')
...
for lab, nice in (('OKRULED', 'ORDER K f3101883 — THE INSTRUMENT VALIDATION ROW (published 1.0513 / 1.0324)'),
                  ...
                  (BASE, 'the live board a05fe951 — this seat\'s own dial-unset emit'),
                  (CAND, 'THE PRICED CANDIDATE 5ea978f7')):
    m = CL.get(lab)
    if not m:
...
h.append('</tbody></table></div>')
h.append('<div class="sub">The instrument reproduced ORDER K at <b>1.0513 / 1.0324, difference '
         '0.0000</b> — VALIDATED — before these numbers were read. The live board reads <b>%.4f</b>, '
         'the registered mark, on this seat\'s own emit as well as on the D7b matrix. The candidate '
         'reads <b>%.4f</b>: the class grows more, and stays under the buy rail with %.4f to spare. '
         'Per class, the three cohorts that already sit above 1.14 on the live board (2011, 2012, '
         '2016) still do on the candidate, and <b>no cohort crosses 1.14 that was not already '
         'across</b> — the full range is printed in <code>CLASS_D8_out.txt</code>.</div>'
         % (CL[BASE]['w2'], CL[CAND]['w2'], 1.14 - CL[CAND]['w2']))
...
for t, o, i in [('both boards reproduced byte-exact, dev-shell and canonical', 'REPRO_D8M_out.txt',
                 'run_repro.sh (the D8 seat\'s d8_build.py, byte-copied)'),
                ('the walk-forward emit, candidate (89 of 89)', 'EMIT_D8CAND_out.txt', 'run_emit_D8.sh'),
                ('the emit, determinism repeat', 'EMIT_D8CAND2_out.txt', 'run_emit_D8.sh'),
                ('the walk-forward emit, live board (89 of 89)', 'EMIT_D8BASE_out.txt', 'run_emit_D8.sh'),
                ('the emit driver\'s one declared change, diffed', 'EMIT_D8_DIFF_out.txt', 'diff vs run_emit_CP.sh'),
                ('the ND bands, both windows, both boards', 'BANDS_NOARB_D8_out.txt', 'd8_noarb_bands.py'),
                ('the ND bands, machine-readable', 'BANDS_NOARB_D8.json', 'd8_noarb_bands.py'),
                ('the pool arms, both windows, both boards', 'STANDING_TABLES_NOARB_D8_out.txt', 'd8_noarb_tables.py'),
                ('the pool arms, machine-readable', 'STANDING_TABLES_NOARB_D8.json', 'd8_noarb_tables.py'),
                ('the class cohort mark', 'CLASS_D8_out.txt', 'd8_noarb_class.py'),
                ('the comparison column checked against the on-file D7B tables', 'NOARB_D8_CHECKS_out.txt',
                 'd8_noarb_checks.py'),
                ('the D8 pricing act this seat measures', 'PACKET_D8.md / BUILD_D8_out.txt', 'the D8 build seat')]:
    h.append('<tr><td class="l">%s</td><td class="l"><code>%s</code></td>'
             '<td class="l k">%s</td></tr>' % (esc(t), esc(o), esc(i)))
h.append('</tbody></table></div>')

sub = ('Candidate board <code>5ea978f7b6a073abb2012f10cccbc3e3</code> · base / live board '
       '<code>a05fe951f78482c70520480e184c80ec</code> — <b>both reproduced byte-exact by this seat '
       'before any table was read</b> · engine <code>338a790b</code> · store <code>cc02567f</code> · '
       'candidate matrix <code>per_entrant_D8CAND.json</code> (<code>b7808db0</code>, 2648 records, '
       'byte-identical on a determinism repeat) · comparison matrix '
       '<code>per_entrant_D8BASE.json</code>, whose priced records are byte-identical to the D7b '
       'matrix the standing tables were rendered from · the ORDER 31-F emit reads <b>89 of 89</b> at '
       'tolerance 0 against <code>DAY0_CP.json</code> untouched, on <b>both</b> emits · the year-1 '
       'class mark reads <b>%.4f</b> on the candidate against <b>%.4f</b> on the live board, both '
       'inside the law. <b>PRICED, NOT ADOPTED.</b>' % (CL[CAND]['w2'], CL[BASE]['w2']))
page = '\n'.join(['<title>D8 Ceiling Candidate No-Arb</title>', '<style>%s</style>' % CSS,
                  '<h1>THE NO-ARB TABLES — %s</h1>' % esc(NICE),
                  '<div class="sub">%s</div>' % sub,
                  BOX.html_box(), '\n'.join(h), '<script>%s</script>' % JS])
open(os.path.join(HERE, 'NOARB_D8.html'), 'w').write(page)
print('NOARB_D8.html written  (%d bytes)' % len(page))
