...
if 'D8CAND' in ARMP:
    for wname, _lo, _hi in WINDOWS:
        ...
        for lab, _nice in LABELS:
            ...
            for arm in ARM_TYPES + ['ALLPOOL']:
                a = OUT['arms'].get('D8CAND', {}).get('%s|%s' % (wname, arm))
...
json.dump(OUT, open(os.path.join(HERE, 'STANDING_TABLES_NOARB_D8.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'STANDING_TABLES_NOARB_D8_out.txt'), 'w').write('\n'.join(L) + '\n')
P()
P('written: STANDING_TABLES_NOARB_D8.json · STANDING_TABLES_NOARB_D8_out.txt')
