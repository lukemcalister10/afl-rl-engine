#!/usr/bin/env python3
"""ORDER 22 -- THE ITERATION TRAJECTORY, printed honestly, every round.

  usage: o22_trajectory.py <out.json> <label> [<label> ...]
"""
import sys, json, os

IT = '/tmp/claude-0/-home-user-afl-rl-engine/7ac96fea-1199-5b6a-9d77-ded9f53694f7/scratchpad/o22/iter'
OUT = sys.argv[1]
LABS = sys.argv[2:]
...
P()
P("  So every pathway's raw lambda is pinned by the POOL AGGREGATE, and the pool aggregate is held")
P("  above 1 by ONE pathway that cannot be repriced -- ND>64, whose derived level exceeds the")
P("  signed ND65+ curve cap. Excluding it:")
