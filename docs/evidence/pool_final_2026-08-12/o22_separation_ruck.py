#!/usr/bin/env python3
"""ORDER 22 -- THE SEPARATION LAW ON THE WALK-FORWARD, AND THE `_ruc_prior_cap` CHECK.
...
(b) `_ruc_prior_cap` (D14 build-time check, directive item 5). The cap is
  ...
  usage: o22_separation_ruck.py <ship.json> <final.json> <levels.json> <out.json>
"""
...
NEW['ND65+'] = int(min(float(LV['nd65_measured_k15']), CAP))
