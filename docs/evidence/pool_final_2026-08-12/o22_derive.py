...
phase 1's, carried; FOUR THINGS CHANGE, each ordered:
  ...
  4. THE LEVELS ARE READ FROM THE TREE THAT PRODUCED THE MATRIX, so an iteration reports its own
     inputs. ND65+ is read AS THE ENGINE READS IT -- min(measured_k15, curve[cap_pick]) -- because the
     stored 266.1 is NOT the level in force; the curve cap at pick 64 = 185 binds today.
...
READ-ONLY. Emits nothing. Deterministic.
    ...
    usage: o22_derive.py <matrix.json> <levels.json|CHECKOUT> <out.json> [label] [--basis anchor|v0]
...
P("=" * 122)
P("ORDER 22 DERIVATION   label=%s   matrix=%s (%s)" % (LABEL, os.path.basename(MATRIX), md5(MATRIX)[:8]))
P("=" * 122)
...
P("=" * 122)
P()
P("  THE LEVELS THAT PRODUCED THIS MATRIX (what the engine actually read -- ND65+ is min(stored, curve[%d]=%g)):"
  % (CAPPK, CAP))
P("    flat   " + " · ".join("%s %.4f" % (k, IN_FLAT[k]) for k in sorted(IN_FLAT)))
P("    RD     " + " · ".join("%s %.4f" % (k, IN_RD[k]) for k in sorted(IN_RD)))
P("    ND65+  stored %.4f -> effective %.4f  (%s)" % (IN_ND65, ND65_EFF, 'CAP BINDS' if IN_ND65 > CAP else 'cap does not bind'))
P()
