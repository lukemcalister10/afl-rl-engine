#!/bin/bash
# FULL GATE RUN — G1 never-rises restore, on the final branch bytes.
# NOTE: no RL_V0SURF_PKL — gate mode rejects it as an unpinned model override. The engine resolves
# v0surf by natural precedence to /home/claude/v0surf.pkl, seeded from THIS checkout; Guard 5 asserts
# the LOADED path's md5 == data/expected_boot.json 'v0surf' on every entry, so a wrong surface halts.
set -uo pipefail
REPO=/home/claude/g1b_ws
OUT=$REPO/docs/evidence/g1_never_rises_2026-08-10
...
echo "== engine head in workspace ==" ; md5sum _merged_recover.py
