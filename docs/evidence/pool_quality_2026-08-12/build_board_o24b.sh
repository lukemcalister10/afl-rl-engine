...
# by REPLACING the landed literals (o24b_stage_surface.py) rather than injecting a block, and there is
...
WT=$SP/wt_o24b_$(basename "$OUT" .json)
...
if [ "$SURF" != "nosurface" ]; then
  python3 "$HERE/o24b_stage_surface.py" "$WT" "$SURF" || { echo "SURFACE STAGE FAILED"; exit 1; }
fi
