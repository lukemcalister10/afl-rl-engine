#!/usr/bin/env python3
"""ORDER 24B -- THE U HARVEST, RE-RUN FOR THE QUALITY-CONDITIONED PREMIUM.

Adapted from docs/evidence/pool_dial_2026-08-12/o24_uharvest.py. ORDER 21's population gates, the
depth convention, the entry weights and the `_b_factor == 1.0` proof are CARRIED VERBATIM, and so is
ORDER 24's per-cell current-season participation `phi_c = min(gy/(6*fe), 1)`. Exactly ONE thing is
added, and it is the whole point of ORDER 24B:

    each cell now records its YEAR'S SCORING AVERAGE (`avg_y`), games-weighted across the year's
    scoring rows so a multi-club season reads as one number, so that EVERY HISTORICAL CELL CARRIES
    ITS OWN QUALITY

        q_c = clip( avg_y / par(pathway, d), 0, 1 )

    against the playing par of its own pathway x depth cell -- the par being derived from THIS SAME
    population by o24b_par.py. A playing cell (games > 0) with a missing or zero average records
    `avg_y = None`, which the rule reads as q = 0, per the order.

The old `sitout` flag and ORDER 24's `gy`/`fe` are kept unchanged alongside, so the ORDER 22/23 U and
ORDER 24's U' both reproduce from this same file and all three deliveries compare on one population.

READ-ONLY. The engine is loaded from a staged copy under the scratchpad; the checkout is never written.

  usage: OPENBLAS_NUM_THREADS=1 python3 o24b_uharvest.py <out_cells.json>
"""
...
STAGE = SP + '/eng_stage_o24b/rl_after'
OUTP = sys.argv[1]
...
shutil.rmtree(SP + '/eng_stage_o24b', ignore_errors=True)
...
with contextlib.redirect_stdout(io.StringIO()):
    for p in MA.data:
        ...
        for Y in range(dy + 1, min(lt, 2025) + 1):
            ...
            yrows = [x for x in rows if x['year'] == Y]
            gy = float(sum(x['games'] for x in yrows))
            fe = float(_fEy(Y, p))
            # ORDER 24B: the year's scoring average, GAMES-WEIGHTED across the year's rows so a
            # multi-club season reads as one number. A playing cell with no usable average records
            # None -- the rule reads that as q = 0 and never as a silent par-matching average.
            avg_y = ((sum(float(x.get('avg') or 0.0) * x['games'] for x in yrows) / gy)
                     if gy > 0 else None)
            if avg_y is not None and not (avg_y > 0.0): avg_y = None
...
print()
print("  ORDER 24B -- THE QUALITY FIELD")
_play = [c for c in WC if c['gy'] > 0]
_noavg = [c for c in _play if c['avg_y'] is None]
print("    playing cells (games > 0) ............................................. %d" % len(_play))
print("    playing cells with NO usable average (q = 0 by the rule) .............. %d" % len(_noavg))
print("    non-playing cells (games == 0, phi == 0, no q exists) ................. %d"
      % (len(WC) - len(_play)))
_av = sorted(c['avg_y'] for c in _play if c['avg_y'] is not None)
if _av:
    print("    avg_y over playing cells: min %.2f  p25 %.2f  median %.2f  p75 %.2f  max %.2f"
          % (_av[0], _av[len(_av) // 4], _av[len(_av) // 2], _av[3 * len(_av) // 4], _av[-1]))
    _tg = sum(c['gy'] for c in _play if c['avg_y'] is not None)
    print("    games-weighted whole-pool playing par (all pathways, all depths): %.4f  (games %d)"
          % (sum(c['avg_y'] * c['gy'] for c in _play if c['avg_y'] is not None) / _tg, round(_tg)))
