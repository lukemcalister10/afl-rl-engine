#!/usr/bin/env python3
"""THE D8 ADOPTION, LANDING STEP C — THE RELEASE CONTRACT, THROUGH ITS WRITERS OF RECORD.
...
RCT=load('release_contract_d8', P('release_contract.py'))
...
# (docs/evidence/d8_ceiling_2026-08-20/d8_restamp.py:86-87, which does `json.dump(c,f,indent=2)` then
# `f.write('\n')`). So the body round-trips exactly and the only byte in question is that newline,
...
# PREREG F4 — the things that must not move
if measured['config']!='eed19a75f775aeafe4ee5ea4b3990667192d8f90389ad6b0e8318e91062d14c1':
    raise SystemExit('HALT: PREREG F4 — config_sha256 MOVED to %s. The dial is not a manifest var; '
                     'data/model_config.json must be untouched.'%measured['config'])
if measured['rl_model']!='6fe7c4155866d80e8045bed2d3bf2802': raise SystemExit('HALT: PREREG F4 — rl_model moved')
if measured['fv']!='6e9a370e5970c5aefa859858070f4c3420f0177b4698d6fac90bd08bf1780346': raise SystemExit('HALT: PREREG F4 — fv moved')
if store_md5!='cc02567f80bef39228f25854d121a766': raise SystemExit('HALT: PREREG F4 — store moved')
if measured['engine_head']=='338a790b773cfbbff0e1283794c72efe': raise SystemExit('HALT: engine_head did not move')
print('  PREREG F4: config_sha256 UNMOVED, rl_model UNMOVED, fv UNMOVED, store UNMOVED, engine_head MOVED. OK.')
