...
RE-POINTED 2026-08-13 for the ORDER 29 LANDING's final-board read (supervisor-authorized within the
ruled ORDER 29 spec, which mandates both cohort instruments on the FINAL board; #334 comments
5278653175 / 5279364952). This is ORDER 29's OWN COPY under docs/evidence/landing_29_2026-08-13/noarb/
— the composition act's copy and the instrument of record are both UNTOUCHED. The landing moved the
store (the unflag-three) and re-baked the curve-keyed surface, so the matrix identity moved and had
to be re-pointed:

  EXPECT_STORE   'd9a24282'     -> 'cb38ef11'       (the landed store: the ORDER 29 unflag-three,
                                                     exactly three _pvc_exclude keys deleted)
  EXPECT_V0SURF  '6ef67f07db98' -> '4405cba2b42f'   (the surface re-baked on the ruled tiebroken
                                                     curve — _v0surf_sig hashes the curve, so it
                                                     moves with the landing by construction)
  EXPECT_N       1197           -> 1200             (RE-MEASURED on the landed matrix, never assumed:
                                                     +3 = adam-treloar, dylan-shiel, jeremy-cameron —
                                                     the unflag-three joining the ND teaching
                                                     population at their natural picks)

THE PINS WERE RE-POINTED, NEVER PATCHED AWAY; every assert is byte-identical, only the pinned values
moved. Proven able to fire before this re-point: the ORDER 29 controls seat ran the composition-act
copies against the landed matrix and both HALTED naming both values (NOARB_LANDED_HALT.txt), and the
same unmodified copies reproduced NOARB_MARGINS_V2 to the last digit on the live matrix — the
blocker was the pin, not the pipeline. noarb_table_338.py in this directory is byte-identical to the
instrument of record (md5 0f8220351c64c56ccfa90c60edcdfa5f, asserted at run).

PRIOR ENTRY, kept: RE-POINTED 2026-08-11 for the #334 COMPOSITION ACT's canonical-instrument read, one disclosed act.
The five engine variants compared here (main / FULL / V1 / V2 / V3) were re-emitted by
docs/evidence/composition_2026-08-10/emit_variant.sh against the CURRENT gate store, so both the
store and the curve-keyed year-zero surface signature moved and had to be re-pointed:

  EXPECT_STORE   '37ced3ce'     -> 'd9a24282'       (the current gate store)
  EXPECT_V0SURF  '3e8e50de5103' -> '6ef67f07db98'   (the frozen surface carried by these emits)
  EXPECT_N       1197           -> 1197             (RE-MEASURED on all five matrices, never assumed:
                                                     the ND teaching population is 1197 on every one
                                                     of main/FULL/V1/V2/V3 AND on the stage-5 matrix,
                                                     so the populations are identical and the
                                                     variant comparison is apples-to-apples)

THE PINS WERE RE-POINTED, NEVER PATCHED AWAY, and the asserts are byte-identical to the ones above.
Proven able to fire in this direction before the pins moved: running the untouched noarb_table_338.py
against per_entrant_main.json under the stage-5 pins HALTED with
    AssertionError: matrix store d9a24282 != committed identity 37ced3ce
which is the assert doing its job and naming both values. The stage-5 pins still discriminate the
stage-5 matrix correctly — that reproduction is the step-1 gate recorded in REPRODUCTION.md.

NOTE ON WHAT IS *NOT* RE-POINTED: noarb_table_338.py itself is UNTOUCHED (md5 0f8220351c64c56cc
fa90c60edcdfa5f, byte-identical to both the #338 evidence copy and the stage-5 copy). Every number in
this act's no-arb tables comes out of that unmodified script. Only this harness's pinned identity
values moved.

■ THIS IS A COPY, NOT THE INSTRUMENT. The instrument of record is
  session_2026-07-30/item279_step4/scripts/harness_pvc_REPINNED.py and it is UNTOUCHED by this act.
  This copy exists under the prior no-arb README's recipe, which requires the CURRENT re-pinned harness
  from main to sit beside the tables as `harness_pvc_REPINNED_pass3.py` (the scripts' import name).

RE-POINTED 2026-08-06 for the #338 NO-ARB REISSUE (owner word 2026-08-06), one disclosed act. The
per_entrant lineage that feeds the no-arb tables was re-emitted on the #338 corrected minimum-tenure
basis (docs/evidence/noarb_338_2026-08-06/emit_matrix_338.py, rule from commit 30996f8), against the
CURRENT gate store — so the store this harness pins moved and had to be re-pointed:

  EXPECT_STORE   'f1e8c9fe'     -> '37ced3ce'       (the current gate store; engine head 8f0e3eb1)
  EXPECT_V0SURF  'af556bdca53d' -> 'af556bdca53d'   (UNCHANGED — #338 changed the walk-forward WINDOW
                                                     and the listed-through reconstruction, not the
                                                     year-zero surface; the surface was not re-fit at
                                                     this act and its signature is curve-keyed)
  EXPECT_N       1197           -> 1197             (RE-MEASURED on the #338 matrix, never assumed:
                                                     teaches_curve 1444 -> picks 1..64 -> years
                                                     2004..2022 = 1197. #338 lengthens career WINDOWS;
                                                     it adds and removes no entrant, so the teaching
                                                     population is identically sized and identically
                                                     keyed.)

  THE PINS WERE RE-POINTED, NEVER PATCHED AWAY. The asserts themselves are byte-identical to the
  instrument's. Non-vacuity was proven in BOTH directions BEFORE the pin moved, per this file's
  standing discipline:
      old pin f1e8c9fe  vs  the #338 matrix (store 37ced3ce)  -> AssertionError, names both values
      old pin f1e8c9fe  vs  the #328 matrix (store f1e8c9fe)  -> loads, ND population 1197
  and again AFTER the pin moved, with the two matrices swapped.

  ■ THE ZERO IS UNTOUCHED. `never_established` / `realised_full` / `sofar` are byte-identical to the
    instrument's: a career with no season of >= QUAL_GAMES (6) games still teaches 0.0, and stays in
    the denominator. #338 sits UPSTREAM of that rule — it changes which YEARS are emitted for a
    career, not what a never-established career is worth. On the #338 matrix 262 of the 1197 teaching
    rows are never-established and every one of them still teaches 0.
...
RE-PINNED 2026-08-05 by #306 L6 PASS 0 on the REDESIGNED LENS LANE, one disclosed act under the seam's
pre-fire audit, ruling 1 (#306 comment 5186041140). The loop re-entered under the year-zero redesign;
the lens surface was baked at pass 0, so the shipped signature this harness pins had to be re-read:

  EXPECT_V0SURF  '8291668eff41'  ->  '96d671c952c8'   (surface aaf45964-era pass-4 pin -> the lens lane)
  EXPECT_STORE   '81d24704'      ->  '81d24704'       (UNCHANGED — the store did not move)
  EXPECT_N       1197            ->  1197             (RE-MEASURED on the emitted matrix, NEVER assumed:
                                                       teaches_curve -> picks 1..64 -> years 2004..2022,
                                                       plus a key-set diff against the prior matrix)

  THE PIN IS A PREDICTION THE MATRIX MUST MEET, NOT A STAMP APPLIED TO WHATEVER EMERGED. The value
  above was set BEFORE the emit, from the pass-0 refit log's shipped-config signature, and the freshly
  emitted matrix is then required to satisfy it. An emitted matrix that does not is a HALT-and-file,
  not a pin quietly adjusted to match. (This is how the record itself read at #290 L6 pass 0: "its
  signature is unchanged, which is why the re-pinned harness still loads it without any pin touched.")

  ■ HAZARD CARRIED, AND THE REASON THE MD5 BINDING EXISTS BESIDE THIS PIN (#306 finding F-C, seam
    ruling 2, same comment). THIS ASSERT CANNOT SEE THE REDESIGN. `_v0surf_sig` is keyed on the curve,
    the roster and the gates — NOT on the fitted stack — so the pre-lens surface fb9efdec and the lens
    surface b540833b BOTH hash to signature 96d671c952c8 while differing in bytes. A matrix emitted on
    either satisfies the assert below identically. N22 governs: the signature SELECTS, the md5 GOVERNS.
    The derivation lane therefore records and asserts the FULL md5 of data/v0surf.pkl beside this
    signature. Do not read a green signature assert as evidence of which surface taught the matrix.
    This is the third recorded instance of the two-surfaces-one-signature class (#290 L6 pass 0;
    L-B's pass-0/pass-1 key sharing; this).

  Non-vacuity proven in BOTH directions BEFORE the pin moved and again AFTER, per this file's standing
  discipline; the asserts themselves are untouched and were never relaxed. Code diff against the frozen
  harness e0130cc2 is THREE CONSTANTS AND THIS HEADER — no logic differs.

RE-PINNED 2026-08-05 by #306 L6 PASS 3, one disclosed act under the seam's authorisation
(#306 comment 5187760033). Pass 3 INSTALLED CURVE 3
(b61c01b0 -> 3f5875b5) via the L1(b) enumerated same-commit set, so the shipped v0surf signature moved
again and the surface was re-fit through the one declared lane:

  EXPECT_V0SURF  'f802734d5406'  ->  '1cbbd9b00ff4'   (surface 69571649 -> b683e2ec)
  EXPECT_STORE   '81d24704'      ->  '81d24704'       (UNCHANGED — the store did not move)
  EXPECT_N       1197            ->  1197             (RE-MEASURED on the pass-3 matrix, never assumed)

  The signature discriminates at this pass for the same reason it did at passes 1 and 2 — the curve moved and
...
RE-PINNED 2026-08-05 by #306 L6 PASS 2, one disclosed act under the seam's authorisation
...
RE-PINNED 2026-08-05 by #306 L6 PASS 1, one disclosed act under the seam's authorisation
(#306 comment 5186820918 clause 2). Pass 1 INSTALLED CURVE 1 (e69a3f38 -> 9f7848f4) via the L1(b)
enumerated same-commit set, so the shipped v0surf signature moved and the surface was re-fit through
the one declared lane:

  EXPECT_V0SURF  '96d671c952c8'  ->  '54ef78e3891a'   (surface b540833b -> 6ba4f4c3)
  EXPECT_STORE   '81d24704'      ->  '81d24704'       (UNCHANGED — the store did not move)
  EXPECT_N       1197            ->  1197             (RE-MEASURED on the pass-1 matrix, never assumed)

  NOTE, against the F-C block below: at THIS pass the signature DOES discriminate, because the curve
  moved and the signature is curve-keyed. That is the opposite of pass 0, where the lens changed the
  surface bytes while leaving the signature identical. The full-md5 binding is kept regardless — a
  check that happens to be redundant at one pass is not a check you remove.
...
EXPECT_STORE = 'cb38ef11'        # RE-POINTED 2026-08-13 (ORDER 29 landing): the landed store, d9a24282 -> cb38ef11 (the unflag-three). PRIOR: RE-POINTED 2026-08-11 (#334 composition act): the current gate store, 37ced3ce -> d9a24282. Assert byte-identical; only the pinned value moved.
EXPECT_V0SURF = '4405cba2b42f'   # RE-POINTED 2026-08-13 (ORDER 29 landing): the surface re-baked on the ruled tiebroken curve, 6ef67f07db98 -> 4405cba2b42f. PRIOR: RE-POINTED 2026-08-11 (#334 composition act): the frozen surface these emits carry, 3e8e50de5103 -> 6ef67f07db98. PRIOR NOTE, kept: RE-POINTED 2026-08-07 (#334 stage B stage 3): the DECLARED surface refit around the settled ladder moved the year-zero surface signature af556bdca53d -> 3e8e50de5103. The assert is byte-identical; only the pinned value moved.
                                 # NB: does NOT discriminate fb9efdec from b540833b — see the F-C block above
EXPECT_N = 1200                     # ND teaching rows after the class cut — RE-MEASURED 2026-08-13 on the landed matrix (1197 -> 1200, the unflag-three), not assumed
CLASS_CUT = 2022
K_FOLDS = 5
FOLD_SEED = 20260730
MIN_STRATUM = 20
