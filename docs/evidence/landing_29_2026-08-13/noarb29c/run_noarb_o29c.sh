#!/bin/bash
# ORDER 29C -- BOTH COHORT NO-ARB INSTRUMENTS ON THE TWO BASES, SIDE BY SIDE.
#
...
echo "instrument pins COMPUTED at run (ORDER 29's disclosed copies, unmodified by 29C):"
...
cd "$N"
for pair in "$SP/per_entrant_O29B.json:O29BFINAL" "$SP/per_entrant_O29CFINAL.json:O29CFINAL"; do
  MX="${pair%%:*}"; L="${pair##*:}"
...
bash "$E22/run_noarb_o22.sh" "$SP/per_entrant_O25R4.json" O29CLIVE
echo
python3 "$E22/o22_margins.py" "$HERE/MARGINS_O29C.json" O29CLIVE O29BFINAL O29CFINAL
} 2>&1 | tee "$HERE/NOARB_MARGINS_29C_out.txt"
cp "$N"/t338_O29CFINAL.txt "$N"/allarm_O29CFINAL.txt "$N"/allarm_O29CFINAL.json \
   "$N"/table_O29CFINAL.json "$HERE/" 2>/dev/null
cp "$N"/t338_O29BFINAL.txt "$HERE/t338_O29BFINAL_control.txt" 2>/dev/null
cp "$N"/allarm_O29BFINAL.txt "$HERE/allarm_O29BFINAL_control.txt" 2>/dev/null
cp "$HERE/NOARB_MARGINS_29C_out.txt" "$HERE/MARGINS_O29C.txt"
