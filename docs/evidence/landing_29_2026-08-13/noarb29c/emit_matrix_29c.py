"""ORDER 29C — THE STANDING #338 EMITTER, COPIED, WITH EXACTLY ONE DECLARED CHANGE: THE YEAR-0 COLUMN.

  Source        : docs/evidence/noarb_338_2026-08-06/emit_matrix_338.py, md5 bffde2f786be85037483e9f5f1563068,
                  COPIED BYTE-FOR-BYTE and then edited ONLY at the sites listed under THE 29C DELTA.
                  The standing emitter is FILED HISTORY and is NOT touched by this act; its md5 is
                  re-computed at run and asserted below, so a reader can check the claim rather than
                  take it.
  Brief         : issue #334 comment 5289123976 (ORDER 29C, the re-based no-arb reading).
  Preregistration: docs/evidence/landing_29_2026-08-13/noarb29c/PREREG_29C.md, committed BEFORE this
                  file existed, never edited.

THE DEFECT THIS CURES, owner-caught. The cohort no-arb instruments take their year-0 value from
`rec['v0']`, and the standing emitter writes `v0 = round(v0_start(p), 1)` — the FROZEN FITTED SURFACE
— while years 1…7 are `ev(p, Y)`. ORDER 29B wired `ev()` to the landed entry law and measured the
consequence (`NOARB_MARGINS_29.md` §B3): **0 of 2648 year-0 cells moved**, so the entire published
reading is MIXED-BASIS — a landed-law numerator over a pre-landing denominator. Concretely, the
matrix's RD mean year-0 reads ~772 while the rederived RD cells sit at 206–369, and the 89 currently
wired entrants' actual printed day-0 matches neither. This file produces the LANDED-LAW basis so the
owner's merge decision rests on one ruler instead of two.

THE 29C DELTA — ONE VALUE SITE, PLUS DISCLOSURE THAT WRITES NO RECORD FIELD

  SITE 1 — the emitted record's year-0 value (was emit_matrix_338.py:252):
      -  v0=round(v0_start(p), 1),
      +  v0=round(_landed_v0_engine(p), 1),
    `_landed_v0_engine` is the ORDER 29B entry law, replicated below from the ARTIFACT and proven
    against the board before a single record is written. NOTHING ELSE ON THE RECORD MOVES: `vpath`,
    `yrs`, `anchor`, `cur`, `peak`, `eq`, `pw`, `games_by`, `seasons`, every predicate and every #338
    disclosure are produced by code carried verbatim, so the ONLY difference between this matrix and
    ORDER 29B's is the `v0` column. That is asserted by diffing the two matrices, not asserted by
    this comment.

  The added block below (`THE LANDED ENTRY LAW`) computes the law, PROVES it, and prints a census.
  It adds keys to `meta` — never to a record — because `meta` is the emitter's own identity block and
  is expected to differ between emitters, while a record field differing would break the claim above.

THE LAW, QUOTED FROM THE ENGINE RATHER THAN PARAPHRASED (`_merged_recover.py`, commit 13cbebb):

      day0_v0(p):  p['_pool']                    -> float(MA.pool_v0_of(p))       # '<pathway>|<gfut>'
                   type ND and 1 <= pick <= 64   -> float(nd_v0.posv[MA.gfut(p)][pick])
                   otherwise                     -> None
      ev(day-0 entrant, Y) = day0_v0(p) * _PL_F                       # BOARD -> ENGINE currency
      printed              = int(round(ev / _F)),  _F == _PL_F        # same pick_redenomination.json

  The numéraire `s` is ALREADY INSIDE both published objects — `posv` is `relat_g(p) × curve(p)` on the
  SHIPPED ladder (raw × s), and the pool cells carry `× anchor_factor == s`. So `_PL_F` is the only
  conversion and it is applied exactly once; double-counting the numéraire is the failure mode this
  paragraph exists to prevent. The two owner-Option-A borrowed cells (`PDN|KPF` 92.35874340265629,
  `PDS|KPF` 83.97715038537063) are consumed through `MA.pool_v0_of`, the ONE accessor, exactly as the
  board consumes them — never by reading `cells` directly, so the accessor's halt cannot be bypassed.

WHY A HISTORICAL ENTRANT NEEDS NO RECONSTRUCTED DAY-0 POSITION. The law's position key is `MA.gfut(p)`,
which reads `p['_futpos']`, else `GRP[p['_pos_now']]`, else `GRP[p['pos']]` — ALL STORE COLUMNS, none
scoring-derived. The walk-forward truncates `p['scoring']`; it does not touch those columns, so the key
is invariant across as-of years and a 2004 entrant is keyed from the same field as a 2025 one. The
brief authorised falling back to the store's recorded day-0/drafted position (the delivered-value
lane's `layer1`/LEDGER precedent) if a field were missing; **that fallback is never reached, and the
census printed below is the evidence rather than the assurance.** Any row the law cannot map is
EXCLUDED and COUNTED LOUDLY — never silently defaulted to the position-blind ladder.

THE ROUNDING IS NOT CHANGED. `round(·, 1)` is the standing emitter's convention and it stays, because
the declared change is the VALUE, not the schema. Measured cost, disclosed rather than repaired:
`int(round(matrix_v0 / _PL_F))` reproduces the board's printed day-0 on 87 of 89 rows (`hunter-holmes`
and `cooper-bell` land one point low from the 1-dp cell). The replication proof below is stated on the
law's own UNROUNDED arithmetic, where it is 89 of 89 at tolerance 0.

READ-ONLY on the engine and on the store, exactly as the standing emitter is. Nothing here fits,
decides, or writes a store. The board is not opened.

================================================================================================
EVERYTHING BELOW THIS LINE IS THE STANDING EMITTER'S OWN HEADER, CARRIED VERBATIM.
================================================================================================
