#!/usr/bin/env python3
"""ORDER S — THE ND BAND TABLES, IN BOTH WINDOWS. ORDER R's oq_bands.py, REUSED WHOLE.
...
json.dump(OUT, open(os.path.join(HERE, 'BANDS_S.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'BANDS_S_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote BANDS_S.json / BANDS_S_out.txt')
