#!/usr/bin/env python3
"""ORDER S — THE TWO CENSUSES PLUS THE CHARGE DISTRIBUTION, for one dial line.
...
if 'O40_LAMBDA' in NS:
    print('  ORDER S EFFECTIVE: LAMBDA %.10f (ORDER P %.10f) · THETA_R %.10f · TMAX %.6f'
          % (NS['O40_LAMBDA'], NS['O37_LAMBDA'], NS['O40_THETA_R'], NS['O40_TMAX']))
    print('  CAP FORM %s%s · RECENCY w %.4f%s · MATURE PREMIUM %s'
          % ('SMOOTH COMPRESSION' if NS.get('_O40_CAPFORM') == 'smooth' else 'hard clip',
             ('  C = %.6f (p%s anchor)' % (NS['O40_CAPC'], NS['_O40_CAPPCT']))
             if NS.get('_O40_CAPFORM') == 'smooth' else '',
             NS.get('O40_RECW', 1.0),
             '' if NS.get('_O40REC') else "  (the engine's own flat weighting)",
             'ON at 24+' if NS.get('_O40_PGMAT') else 'off'))
...
json.dump(dict(tag=TAG, dials=DIALS, burn=res, birthday=bres, charge=CH, named=NAMED,
               constants=(dict(BETA_sat=NS['O39_BETA_SAT'], THETA_R=NS['O39_THETA_R'],
                               ...
                               O40_LAMBDA=NS.get('O40_LAMBDA'), O40_THETA_R=NS.get('O40_THETA_R'),
                               O40_TMAX=NS.get('O40_TMAX'), O40_CAPC=NS.get('O40_CAPC'),
                               O40_CAPFORM=NS.get('_O40_CAPFORM'), O40_RECW=NS.get('O40_RECW'),
                               O40_PGMAT=bool(NS.get('_O40_PGMAT')))
