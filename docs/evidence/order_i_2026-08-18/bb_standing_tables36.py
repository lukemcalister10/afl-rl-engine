#!/usr/bin/env python3
"""ORDER I — THE STANDING TWO-SIDED NO-ARB SUITE, in the format the owner has just re-confirmed.
...
committed md5 d59ad550116ebbe3d90ed82becd2c4d5, run whole by bb_noarb36.sh). Pool arms are computed
...
LABELS = ['O35FINAL', 'O36FINAL']
NICE = {'O35FINAL': 'LANDING CANDIDATE 1f176444 (Order D)', 'O36FINAL': 'ORDER I (RL_O36)'}
...
P('=' * 118)
P('ORDER I — STANDING TWO-SIDED NO-ARB SUITE.  carry charge = 14%/yr.')
...
P('  %-16s %12s %12s %12s   %s' % ('band', 'candidate', 'ORDER I', 'move', 'verdict move'))
for b in CLASSIC + BANDS5:
    a0 = OUT['nd']['O35FINAL'][b]['apprec01']; a1 = OUT['nd']['O36FINAL'][b]['apprec01']
    P('  %-16s %+11.2f%% %+11.2f%% %+11.2f   %s -> %s'
      % (b, 100 * a0, 100 * a1, 100 * (a1 - a0),
         OUT['nd']['O35FINAL'][b]['verdict'], OUT['nd']['O36FINAL'][b]['verdict']))
...
P('  %-10s %12s %12s %12s   %s' % ('arm', 'candidate', 'ORDER I', 'move', 'verdict move'))
for arm in ARM_TYPES + ['ALLPOOL']:
    a = OUT['arms']['O35FINAL'].get('PRIMARY|%s' % arm)
    b = OUT['arms']['O36FINAL'].get('PRIMARY|%s' % arm)
...
# ---- entry-year control -----------------------------------------------------------------------------
P('\n-- ENTRY-YEAR CONTROL (S1 must not move the entry year: it is a production correction and a '
  'day-0 row has no production) --')
P('   bound: every ND-band yr0 MEAN within +-0.1% of the landing candidate cell (yr1 is EXPECTED to move)')
br = []
for b in CLASSIC + BANDS5:
    rc = {r['N']: r for r in T338['O35FINAL']['groups'][b]['rows']}
    rn = {r['N']: r for r in T338['O36FINAL']['groups'][b]['rows']}
    for N in (0, 1):
        c = rc[N]['mean_yearN']; n_ = rn[N]['mean_yearN']
        rel = n_ / c - 1.0
        ok = (abs(rel) <= 0.001) if N == 0 else True
        OUT['entry_control']['%s|yr%d' % (b, N)] = dict(control=c, o36=n_, rel=rel, ok=bool(ok))
        if N == 0 and not ok: br.append((b, rel))
        P('   %-16s yr%d: candidate %9.1f  ORDER I %9.1f  %+7.3f%%  %s'
          % (b, N, c, n_, 100 * rel, ('ok' if ok else 'BREACH') if N == 0 else '(yr1 moves by design)'))
P('\nENTRY-YEAR CONTROL: %s' % ('PASS — every yr0 cell inside +-0.1%' if not br else 'BREACHES: %s' % br))
OUT['entry_control']['pass'] = not br

json.dump(OUT, open(os.path.join(HERE, 'STANDING_TABLES_I.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'STANDING_TABLES_I_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote STANDING_TABLES_I.json / STANDING_TABLES_I_out.txt')
