#!/usr/bin/env python3
"""FINAL-CANDIDATE — THE DELIVERY VERIFICATION CHECKLIST. The assembly seat's as_verify.py with
ONLY the six document filenames and the two output names repointed. Every check is byte-identical,
so the score is comparable to the assembly seat's 79/79.
...
# ---------------- 1 · THE NO-ARB PAGE --------------------------------------------------------------
P()
P('1 · THE NO-ARB TABLES  (NOARB_FINAL.html)  — spec: five bands + ALL/1-20/21-64, PLUS the POOL')
P('    ARMS, BOTH windows, both baselines, path test on every breaching cell.')
na = read('NOARB_FINAL.html')
if na is None:
...
json.dump(dict(n=len(RES), passed=npass, failed=nfail, checks=RES),
          open(os.path.join(HERE, 'DELIVERY_VERIFICATION_FC.json'), 'w'), indent=1)
open(os.path.join(HERE, 'DELIVERY_VERIFICATION_FC_out.txt'), 'w').write('\n'.join(L) + '\n')
P()
P('written: DELIVERY_VERIFICATION_FC.json · DELIVERY_VERIFICATION_FC_out.txt')
