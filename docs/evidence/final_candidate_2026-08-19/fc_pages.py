...
CAND = B[CANDTAG]
LEGS = {}
lp = os.path.join(HERE, 'LEGS_FC.json')
...
open(os.path.join(HERE, 'PLAYERS_FINAL.html'), 'w').write(page(
...
#     off THE CANDIDATE'S OWN engine state by fc_v0.py, which self-checks against those three
#     numbers at tolerance 0 before it writes anything.
MX = json.load(open(SP + '/per_entrant_FCBASE.json'))['recs']
MXK = {r['key']: r for r in MX}
_V0 = json.load(open(os.path.join(HERE, 'V0_FC.json')))['v0']
...
h = ['<div class="sub k">Board totals: %s</div>' % HDR,
     ...
     'entry prices <b>v0</b> read off THIS board\'s own engine state (fc_v0.py), not borrowed from '
...
open(os.path.join(HERE, 'YEAR1_FINAL.html'), 'w').write(page(
    'Assembly Year One', 'THE YEAR-1 CLASS — THE CANDIDATE',
    'Cohort %d in pick order, with entry price beside every board.' % TARGET,
    '\n'.join(h)))

print('PLAYERS_FINAL.html  %d rows' % len(rows))
print('YEAR1_FINAL.html    %d rows' % len(Y1))
