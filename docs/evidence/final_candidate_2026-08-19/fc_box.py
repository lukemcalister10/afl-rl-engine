#!/usr/bin/env python3
"""FINAL-CANDIDATE — THE STANDING "WHAT IS IN THIS BOARD AND WHAT IS STILL BROKEN" BOX.
...
IN_IT = [
    ...
    ('THE UNWIND — five games no longer clears the penalty; SEVEN return games do (U0 = 7)',
     'YOUR RULING, and the data supports it: a return of three-to-five games was measured to reverse '
     'only about 17% of the damage, and the measured half-life sits near seven. U0 = 7 is carried '
     'here as OWNER-RULED, DATA-SUPPORTED.'),
    ('THE INJURY CONSOLIDATION — YOUR SHEET IS NOW THE ONLY INJURY TRUTH (new on this board)',
     'The engine used to read TWO injury lists: your sitter annotation and its own long-term-injury '
     'register. They disagreed. On this board the register keeps no live consumption at all — the '
     '37 rows you annotated injured are the whole injured set. 20 rows that were on the register '
     'but NOT on your sheet stop being charged as injured, and 14 rows you annotated but the '
     'register never listed start being charged. The board moves +1,356 across 31 rows and every '
     'one of them is inside that 57-row set. TOBY CONWAY KEEPS HIS SITTING CHARGES: 460 before, '
     '460 after, delta exactly zero.'),
]
...
BROKEN = [
    ...
    ('THE TWO-REGISTER QUESTION IS ANSWERED ON THIS BOARD — and the count was 20, not 21',
     'This was the open item on the last board. It is closed here: your sheet is the only injury '
     'truth and the engine\'s own register has no live consumption. One correction to the earlier '
     'wording, reported as measured rather than as briefed: the register-only population is 20 '
     'rows, not 21. It is listed under what is IN this board above; it is left here so you can see '
     'the open item and its answer in the same place.'),
    ('THE CONTINUITY INSTRUMENT CANNOT SEE THE ABSENCE FADE — MY DEFECT, NOW OPEN',
     ...
     'the last board it therefore reported nine false birthday jumps that were really just the fade '
     'itself. ON THIS BOARD THE BIRTHDAY COULD NOT BE CLEARED AT ALL THROUGH THE FADE, AND THAT IS '
     'SAID PLAINLY RATHER THAN CARRIED OVER FROM LAST TIME. The direct probe needs a comparison '
     'board identical except for the fade; the engine REFUSES to build one, because the unwind rule '
     'shapes a collector that would not be switched on. The only comparison board it will build is '
     'three settings apart, which puts 59 rows in the comparison instead of 9, and the probe\'s own '
     'self-check then fails on 12 of them by exactly one point each — integer rounding, not a real '
     'movement — so THE PROBE REFUSES TO REPORT A BIRTHDAY NUMBER. It is reported as NOT SCORABLE. '
     'What IS measured, on the board with the fade switched off: birthday movement zero, no row '
     'moving at all. The gap is the instrument\'s, it is open, and nothing was adjusted to close it.'),
    ('THE CLASS MARK AND THE NO-ARB TABLES COULD NOT BE PRODUCED FOR THIS BOARD',
     'Both are computed from a walk-forward matrix — every entrant re-priced year by year — and the '
     'builder for that matrix STOPPED rather than produce one. Its safety check compares this '
     'board\'s first-year entry prices against the published set and found three that moved: Sam '
     'Allen, Ollie Murphy and Kobe McDonald, all three of them rows the injury consolidation newly '
     'treats. The check is doing its job: their entry prices really did move, so the matrix would '
     'have been built on a different first-year law than the published one. Re-publishing that '
     'reference is YOUR call, not this seat\'s, so THE CLASS MARK AND THE NO-ARB PAGE ARE MISSING '
     'FOR THIS BOARD and are reported as missing rather than carried over from the previous one. '
     'The same measurement on the board WITHOUT the consolidation reads 1.0671, unchanged.'),
