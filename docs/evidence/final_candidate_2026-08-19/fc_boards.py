...
NICE = {
    ...
    'FC_CAND':    '+ D6 the injury consolidation (RL_O42=1)   = THE CANDIDATE',
    'FC_CAND2':   'the determinism repeat of THE CANDIDATE',
}
...
if not MD5.get('FC_CAND') or not MD5.get('FC_CAND2'):
    P('  DETERMINISM (A-F4): NOT YET SCORABLE — one of the two repeat boards is missing.')
    FAIL.append('determinism not scorable')
elif MD5['FC_CAND'] == MD5['FC_CAND2']:
    P('  DETERMINISM (A-F4): IDENTICAL on the repeat — %s. Did not fire.' % MD5['FC_CAND'][:8])
else:
    P('  DETERMINISM (A-F4): *** FIRED *** %s vs %s' % (MD5['FC_CAND'][:8], MD5['FC_CAND2'][:8]))
...
json.dump(dict(totals=TOT, md5=MD5, lever={t: {kk: vv for kk, vv in LEVER[t].items() if kk != 'rows'}
          ...
          open(os.path.join(HERE, 'BOARDS_FC.json'), 'w'), indent=1, sort_keys=True)
open(os.path.join(HERE, 'BOARDS_FC_out.txt'), 'w').write('\n'.join(L) + '\n')
P()
P('written: BOARDS_FC.json · BOARDS_FC_out.txt · MOVERS_LEDGER.json')
