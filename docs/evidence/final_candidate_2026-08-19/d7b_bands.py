...
WORK = os.path.join(SP, 'asm', 'noarb')
...
# ORDER D7b: THE CANDIDATE NOW HAS A MATRIX AND LEADS THIS LIST. The completion pass could not put it
# here because the emit halted on the ORDER 31-F day-0 guard; D7b wired the third site (o31_D) and the
# emit now reads 89 of 89 on a05fe951 against DAY0_CP.json untouched. FCBASE is retained directly
# beneath it as the candidate MINUS RL_O42 and nothing else — the base, NOT the candidate.
LABELS = [('D7BCAND', '*** THE PARITY CANDIDATE a05fe951 — RL_O42=1 RL_O43=1, THE BOARD UNDER REVIEW ***'),
          ('FCBASE', 'THE D5-FINAL BASE ff936186 — the candidate minus RL_O42 ONLY. THE BASE, NOT THE CANDIDATE.'),
          ('ASMCAND', 'the assembly candidate db1ccef5 — for continuity with the last packet'),
...
json.dump(OUT, open(os.path.join(HERE, 'BANDS_D7B.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'BANDS_D7B_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote BANDS_D7B.json / BANDS_D7B_out.txt')
