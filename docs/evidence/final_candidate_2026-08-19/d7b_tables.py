...
LABELS = [('D7BCAND', '*** THE PARITY CANDIDATE a05fe951 — RL_O42=1 RL_O43=1, THE BOARD UNDER REVIEW ***'),
          ('FCBASE', 'THE D5-FINAL BASE ff936186 — the candidate minus RL_O42 ONLY'),
...
if 'D7BCAND' in ARMP:
    for wname, _lo, _hi in WINDOWS:
        ...
        for lab, _nice in LABELS:
            ...
            for arm in ARM_TYPES + ['ALLPOOL']:
                a = OUT['arms'].get('D7BCAND', {}).get('%s|%s' % (wname, arm))
...
json.dump(OUT, open(os.path.join(HERE, 'STANDING_TABLES_D7B.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'STANDING_TABLES_D7B_out.txt'), 'w').write('\n'.join(L) + '\n')
P()
P('written: STANDING_TABLES_D7B.json · STANDING_TABLES_D7B_out.txt')
