...
# THE CANDIDATE daa16812 CANNOT APPEAR HERE. These tables read a WALK-FORWARD MATRIX and the
# candidate has none — the emit halts on the ORDER 31-F day-0 guard (PACKET_FINAL section 4).
# FCBASE is the candidate MINUS RL_O42 and nothing else. IT IS THE BASE, NOT THE CANDIDATE.
LABELS = [('FCBASE', 'THE D5-FINAL BASE ff936186 — the candidate minus RL_O42 ONLY. '
                     'NOT THE CANDIDATE; the candidate has no matrix.'),
...
json.dump(OUT, open(os.path.join(HERE, 'BANDS_FC.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'BANDS_FC_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote BANDS_ASM.json / BANDS_ASM_out.txt')
