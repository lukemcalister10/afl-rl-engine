...
def bdir(t):
    """FC_* tags are this seat's own builds; everything else is the assembly seat's stack."""
    return FCD if t.startswith('FC_') else ASM

COLS = [('live', 'live', '88ce647f'), ('IDENT_K', 'K', 'f3101883'),
        ('IDENT_P', 'P', '374d4e44'), ('L0_R', 'R', '7f88f509'), ('FC_CAND', 'CANDIDATE', 'daa16812')]
...
for k, vals in ROWS:
    ...
    TR.append(dict(key=k, name=m.get('name'), pos=m.get('pos'), age=m.get('age'), club=m.get('club'),
                   ...
                   R=vals.get('L0_R'), cand=vals.get('FC_CAND'),
                   d_live_K=d(vals.get('live'), vals.get('IDENT_K')),
                   d_K_P=d(vals.get('IDENT_K'), vals.get('IDENT_P')),
                   d_P_R=d(vals.get('IDENT_P'), vals.get('L0_R')),
                   d_R_cand=d(vals.get('L0_R'), vals.get('FC_CAND')),
                   d_live_cand=d(vals.get('live'), vals.get('FC_CAND')),
                   d_K_cand=d(vals.get('IDENT_K'), vals.get('FC_CAND'))))
...
for r in TR:
    ...
    for t, lab, _w in COLS:
        if t not in V:
            continue
        key = {'live': 'live', 'IDENT_K': 'K', 'IDENT_P': 'P', 'L0_R': 'R', 'FC_CAND': 'cand'}[t]
...
open(os.path.join(HERE, 'TRACKER_FINAL.html'), 'w').write('\n'.join(h))

with open(os.path.join(HERE, 'TRACKER_FINAL.csv'), 'w', newline='') as f:
    w = csv.writer(f)
...
open(os.path.join(HERE, 'LEVERS_FINAL.html'), 'w').write('\n'.join(g))

json.dump(dict(totals=TOT, md5=MD5, n_tracked=len(TR),
               lever={t: {kk: vv for kk, vv in LEV[t].items() if kk != 'rows'} for t in LEV}),
          open(os.path.join(HERE, 'TRACKER_FC.json'), 'w'), indent=1, sort_keys=True)
print('TRACKER_FINAL.html  %d moved rows' % len(TR))
print('TRACKER_FINAL.csv')
print('LEVERS_FINAL.html   %d levers' % len(LEV))
for t in TOT:
