#!/usr/bin/env python3
"""FINAL-CANDIDATE — the year-1 class cohort mark. The assembly seat's as_class.py with ONLY the
LABELS list extended (FCBASE, FCCAND) and the output filenames changed. The measurement — the
W2 window, the cohort rule, marks(), the instrument validation against ORDER K — is byte-identical.

FCCAND IS EXPECTED TO BE MISSING: the walk-forward emit for the candidate HALTS on the ORDER 31-F
day-0 replication guard (3 of 89 wired entrants move under RL_O42=1). The label is listed anyway so
the absence is PRINTED as "MATRIX MISSING" rather than silently omitted.
...
LABELS = [('OKRULED', 'ORDER K f3101883'),
          ...
          ('FCCAND', '*** THE FINAL CANDIDATE daa16812 — RL_O42=1 ***')]
# FINAL-CANDIDATE CHANGE: the assembly file DROPPED absent labels here, which would have made the
# halted candidate matrix vanish from the table without trace. FCCAND is kept so its absence PRINTS.
LABELS = [(l, n) for l, n in LABELS
          if l == 'FCCAND' or os.path.exists(os.path.join(SP, 'per_entrant_%s.json' % l))]
...
json.dump(OUT, open(os.path.join(HERE, 'CLASS_FC.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'CLASS_FC_out.txt'), 'w').write('\n'.join(L) + '\n')
