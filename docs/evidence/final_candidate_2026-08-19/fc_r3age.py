...
PRE, mPRE = board('FC_NOR3')       # ramp ON, R3 OFF (and BREAK/UNWIND off — FORCED, see below)
CAND, mCAND = board('FC_CAND')     # THE FINAL CANDIDATE daa16812
...
json.dump(dict(engine=hashlib.md5(open(os.path.join(REPO, 'engine/rl_after/_merged_recover.py'), 'rb').read()).hexdigest(),
          ...
          open(os.path.join(HERE, 'R3_AGE_FC.json'), 'w'), indent=1)
open(os.path.join(HERE, 'R3_AGE_FC_out.txt'), 'w').write('\n'.join(OUT) + '\n')
