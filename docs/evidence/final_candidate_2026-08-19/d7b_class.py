...
fc_class.py carried with TWO declared changes and NOTHING ELSE:
  ...
  (2) the output filenames: CLASS_D7B.json / CLASS_D7B_out.txt.
...
LABELS = [('OKRULED', 'ORDER K f3101883'),
          ...
          ('D7BCAND', '*** THE PARITY CANDIDATE a05fe951 — RL_O42=1 RL_O43=1, THE D7b THIRD SITE ***')]
...
LABELS = [(l, n) for l, n in LABELS
          if l == 'D7BCAND' or os.path.exists(os.path.join(SP, 'per_entrant_%s.json' % l))]
...
json.dump(OUT, open(os.path.join(HERE, 'CLASS_D7B.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'CLASS_D7B_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote CLASS_D7B.json and CLASS_D7B_out.txt')
