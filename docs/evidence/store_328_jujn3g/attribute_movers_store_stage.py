...
# named pair differs.
HELD = ('engine', 'band', 'curve')
def _bad(k):
    av, bv = IDS_B.get(k), IDS_C.get(k)
    return (not av) or (not bv) or (av != bv)
unnamed = [k for k in HELD if _bad(k)]
print('STORE-STAGE COMPLETENESS GATE — held-constant inputs (engine, band, curve):')
for k in HELD:
    av, bv = IDS_B.get(k), IDS_C.get(k)
...
if unnamed:
    ...
    sys.exit(1)
print('  -> only the store and the year-zero surface it forces differ; the cause set is COMPLETE.\n')
...
if OUT:
    json.dump({'held_gate': 'PASS', 'stage': 'store', 'named_pair': ['store', 'surface'],
...
print('\nEVERY MOVER ATTRIBUTED to the store+surface pair; no unnamed axis. PASS.')
sys.exit(0)
