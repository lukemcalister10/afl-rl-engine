#!/bin/bash
# Run tblprobe.py under a channel dial, with the manifest temporarily set to match (gate mode
# requires env == manifest). The manifest is ALWAYS restored, including on failure.
set -uo pipefail
ARM="$1"; shift
...
cp "$REPO/data/model_config.json" "$SP/.man.bak"
cp "$REPO/data/expected_boot.json" "$SP/.boot.bak"
...
cd "$REPO" && python3 "$SP/tblprobe.py" > "$SP/tbl_$ARM.json" 2>&1
echo "$ARM exit=$?"
