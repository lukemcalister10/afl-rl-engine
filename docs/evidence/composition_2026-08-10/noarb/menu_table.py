...
def main():
    ...
    if os.path.exists(SPL):
        ...
        P('=' * 116)
        P("### THE SPLIT COHORT TABLES — ND vs POOL-ROOKIE vs POOL-NON-ROOKIE  (owner spec, ORDER 8)")
        P('=' * 116)
        P('  OWNER: "we have two stories here - ND players... appreciate over time to year 4/5, where they')
        P('  are worth ~50%% more as a ND cohort. And pool players, who do not."  HIS HYPOTHESIS IS')
        P('  CONFIRMED ON HIS OWN INSTRUMENT, and every pre-registered prediction (H8.1-H8.6) HELD.')
        P()
        P('  ratio = SIGMA price at year N / SIGMA year-0 anchor of the same included set.')
        P('  ND>64 sits in Pool-non-rookie (120 rows): past the curve it collapses to the single pool index')
        P('  and is priced off its signed division level, so it is a pool row by the engine\'s own typing.')
        for b in ('main', 'FULL'):
            for w in ('ALL-TIME cohorts', 'COHORTS 2012+'):
                if b not in S or w not in S[b]: continue
                P()
                P('  --- base %s | %s ---' % (b, w))
                P('      %-18s %6s %s' % ('split', 'n', ''.join('%9s' % ('yr%d' % N) for N in range(7))))
                for g in ('ND 1-64', 'Pool-Rookie', 'Pool-non-rookie', 'COMBINED'):
                    d = S[b][w][g]
                    P('      %-18s %6d %s' % (g, d['n'],
                      ''.join('%9.4f' % r['ratio_sum_over_sum'] for r in d['rows'])))
        P()
        P('  TOTALS ARE IN SPLIT_TABLES.txt BESIDE EVERY RATIO, and they must be read together. Under FULL')
        P('  the pool arms\' year-ZERO anchors fall 22-25%% (ND\'s move -0.02%%), so a pool ratio that looks')
        P('  better under FULL is largely its denominator falling, not its delivery rising. That is H8.5,')
        P('  measured: Pool-Rookie SIGMA v0 352,757 -> 264,205; Pool-non-rookie 192,830 -> 149,640.')
        P()
        P('  WHAT IT SETTLES: the arms deliver very differently against their own entry prices - ND 1.5565')
        P('  per unit at year 4, Pool-Rookie 0.7428, Pool-non-rookie 0.6748, about 2.1-2.3 to 1. Neither')
        P('  pool arm is above its entry price even at year 6.')
        P('  WHAT IT CANNOT SETTLE: WHICH price is wrong. "Pool v0 too high" and "ND v0 too low" are')
        P('  observationally identical in a ratio. Both readings stand; the ruling is the owner\'s.')
        P()
        P('  THE CELL THAT JUDGES ITEM B (year-4 delivery per unit of entry price, base main, by DRAFT AGE')
        P('  WITHIN each pool arm) - see B_PROVENANCE_AND_SPLITS.md:')
        P('      draft age        Pool-non-rookie      Pool-Rookie')
        P('      <=18                      0.4869           0.5820')
        P('      19-20                     0.9851           0.6142')
        P('      21+                       0.7708           3.0092')
        P('  ITEM B multiplies EVERY pool entry anchor at draft age 21+ by 2.0478. That gradient is')
        P('  SUPPORTED on the rookie arm (3.01x) and CONTRADICTED on the non-rookie arm (0.77x, BELOW its')
        P('  own 19-20 slice). B\'s derivation pools both arms by construction (is_pool only), so the')
        P('  evidence carrying the lift comes substantially from the arm it is not being questioned on.')
        P('  ON THE NON-ROOKIE ARM THE OWNER\'S "might make sense to be LESS" IS SUPPORTED BY THE DATA.')
        P()
        P('  PLAY-QUALITY PRINCIPLE (owner ruling 5249619354): "we value them on how they play". ITEM B')
        P('  was fitted to D_rt_win = "REALIZED DELIVERY off the seasons and bars" (item_d_derive.py:22-23)')
        P('  - a delivered-VALUE composite that rises with games as well as with level, NOT a play-quality')
        P('  measure. Measured separation on the NON-ROOKIE arm: quality (games-wtd avg) 51.47 / 52.98 /')
        P('  53.33 across draft ages <=18 / 19-20 / 21+ - FLAT, a 3.6%% spread - while participation')
        P('  (career games) is 24.7 / 31.7 / 21.6, the 21+ slice playing the LEAST. B lifts their anchor')
        P('  +105%% anyway. ON THE RULED PRINCIPLE B\'s SHAPE FAILS THE STANDARD ON BOTH ARMS (the rookie')
        P('  arm\'s 21+ edge is +61%% games against +11.7%% quality - participation-dominated).')
        P()
        P('  THE OWNER\'S PAIR, the incoherence in two rows (full waterfalls in B_PROVENANCE_AND_SPLITS.md):')
        P('    Podhajski  2 career games  main 101 -> B +102 -> surprise +116 -> H -152 -> FULL 245  (+142.6%%)')
        P('    Herbert   30 career games  main 1060 -> B +0 ................ -> H -393 -> FULL 627  (-40.8%%)')
        P('    Same demographic. B reaches Podhajski because his price IS his anchor; it reaches Herbert by')
        P('    EXACTLY ZERO because his anchor is already faded out. The ruled machinery doubles the man who')
        P('    has not played and halves the man who has. No menu candidate repairs it.')
        P()
        P('  THE UNION CELL joins MATNONRD in the H re-derivation scope, and it fails DIFFERENTLY:')
        P('    MATNONRD 0.615  does not reproduce (F bent 0.7676), CI [0.115, 1.226] CONTAINS 1.0 - unsupported')
        P('    UNION    0.280  does not reproduce (F bent 0.1670), CI [0.010, 0.639] EXCLUDES 1.0 - supported in')
        P('                    DIRECTION but ruled MILDER than its own evidence, so re-deriving would DEEPEN it')
        P('    NEITHER cell\'s board effect is in any ruled evidence, and NEITHER IS THE COMPOSITION: the three')
        P('    MSD rucks take POOLSIT x UNION x MATNONRD = 0.804 x 0.280 x 0.615 = 0.1384, measured 0.1398')
        P('    (noH 472 -> FULL 66). AN 86%% CUT FROM THREE CELLS COMPOSING, and the composed factor appears')
        P('    NOWHERE in the directive, PHASE2 or the derivation output.')
        P()
        P('  AND THE B-vs-H INCOHERENCE, same instrument (DISC 1.0939, BARN 35, NB 4000) and the same')
        P('  673-row corrected-ruler pool sample: B raises a mature non-rookie pool entrant\'s YEAR-0')
        P('  ANCHOR by 105%% because his band out-delivers the POOL MEAN, and H cuts his FINISHED PRICE by')
        P('  38.5%% because the same demographic under-delivers FAIR. Both ruled. Neither ruling was shown')
        P('  the other\'s effect. NOTHING IS CHANGED HERE - measurement and provenance only.')
        P()
    P('=' * 116)
    P('### HALT-GRADE — AN UNRULED 38.5% CUT ON THE POOL ARM  (full attribution: POOL_ARM_ATTRIBUTION.md)')
    P('=' * 116)
    P('  OWNER CHALLENGE: "a lot of /SSP/MSD players have been slashed - Noble, Hall, McCarthy, Peatling,')
    P('  Herbert, Keane... have players like Noble and Peatling not established themselves on their own')
    P('  production more than their draft arm?"  ANSWERED, AND HE IS RIGHT ON THE MECHANICS.')
    P()
    P('  IT IS ITEM H, AND IT IS ONE CONSTANT. H_MATNONRD = 0.615 is applied at _merged_recover.py:2228')
    P('  to the FINAL production-led price, keyed on three things only: is he pool, is his type not RD,')
    P('  was his DRAFT AGE >= 21. It reads no games, no level, no establishment. Recovered to four')
    P('  decimals on eight independent players: FULL/H-restored = 0.6149 0.6153 0.6143 0.6155 0.6147')
    P('  0.6147 0.6151 0.6163. Not several items composing - literally the one constant.')
    P()
    P('  PRODUCTION BUYS NO PROTECTION. Noble has 158 career games and takes the same 0.615 as a')
    P('  zero-game row. His ITEM A anchor share is EXACTLY 0.000000 - the engine\'s own evidence fade has')
    P('  already retired his entry anchor - so the 38.5% is NOT his draft arm re-asserting itself through')
    P('  the valuation. It is a flat cell multiplier on top of the finished price.')
    P()
    P('  MRAZ IS THE CONTROL AND HE SEPARATES THE TWO CAUSES CLEANLY: his -53.6% is the SURPRISE LAW')
    P('  (+1692 of his 1906 gap), and removing ITEM H moves him by exactly 0.')
    P()
    P('  THE INVERSION IS A SECOND, INDEPENDENT ITEM. ITEM B multiplies the ENTRY ANCHOR by k*_b_shape:')
    P('  x0.4985 at draft age 18, x1.0257 at 19-20, x2.0478 at 21+ (+104.8%). Where a pool entrant has no')
    P('  production his price IS his anchor, so he takes the whole +105% (Perez/Cross +105.3%, Banch')
    P('  +105.5%). Where he HAS production the anchor is already faded to zero weight, so B barely reaches')
    P('  him and H then removes 38.5%. SAME ARM, OPPOSITE DIRECTIONS, TWO SEPARATELY-RULED ITEMS.')
    P('  B\'s conservation assert is real (2.978e-15) but it is over SIGMA ENTRY ANCHOR across the live')
    P('  pool (n=1202), a YEAR-ZERO object - not prices. On the board B is net +3,628.')
    P()
    P('  WHY THIS IS HALT-GRADE, not merely a finding:')
    P('    1. THE ARM IS OUTSIDE EVERY DECIDING FIGURE. The canonical population is n=1197 and it is')
    P('       100%% type ND - ZERO pool rows. The matrix carries 1080 pool-arm rows (691 RD, 106 MSD, 57')
    P('       IRE, 59 UNR, 52 SSP, 51 PDA, 43 PDN, 21 PDS) and not one enters any ruled number.')
    P('    2. THE CUT\'S OWN DERIVATION HALTED. item_h_derive_out.txt: "HALT-NO-SURPRISE... taken AS')
    P('       FILED". The mature-nonRD corrected-ruler CI is [0.115, 1.226] at eff-n 46.2 - IT CONTAINS')
    P('       1.0, so the evidence in front of the owner could not exclude NO CUT AT ALL.')
    P('    3. NO BOARD-EFFECT FIGURE FOR THE POOL ARM APPEARS IN ANY RULED EVIDENCE. Searched the')
    P('       directive, README, PHASE2 and SIDE_BY_SIDE: the factor is named, its board effect on')
    P('       established players never is.')
    P()
    P('  THE SIDE-BY-SIDE CANNOT PROCEED WITH AN UNRULED ARM-WIDE 38.5%% CUT INSIDE IT. Nothing is')
    P('  repaired here and no factor is changed - this is attribution and disclosure, and the ruling is')
    P('  the owner\'s.')
    P()
    P('=' * 116)
