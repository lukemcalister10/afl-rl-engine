#!/usr/bin/env python3
"""ORDER A REPAIR — R2: THE CLASS-LEVEL RESIDUAL, PROPERLY CUT — plus the three named
...
NB = json.load(open(os.path.join(HERE, 'NOARB_32R.json')))
P('')
...
P('FINDING (L2): CONFIRMED for the thin development arms — PDS/PDN (and partially IRE, UNR, PDA)')
P('carry entry cells ABOVE their own arms\' delivered history (K0 far below ND\'s), the K-shrink')
P('borrowing being the located mechanism. Candidate fix: OWN-ARM RE-ANCHOR of those entry cells at')
P('the next v0 refit — RULINGS-MATERIAL, not wired here. The CONTRAST case is SSP: entry too LOW')
P('(K0 far above ND\'s; the +55%% yr0->1 buy-side red) — mature-age entrants convert production')
P('immediately while their cells price development-class entries. Same fix family, opposite sign.')
P('RD is career-fair at entry (K0 ~ ND\'s, mark ~ fair): its weak years 1-3 marks are the')
P('leg-attribution question, not an entry question.')
...
json.dump(dict(order='ORDER A REPAIR R2 — residual attribution + lanes',
               ...
               L2=L2, L3=L3),
          open(os.path.join(HERE, 'ATTRIBUTION_32R.json'), 'w'), indent=1, sort_keys=True, default=float)
open(os.path.join(HERE, 'ATTRIBUTION_32R_out.txt'), 'w').write('\n'.join(OUT) + '\n')
