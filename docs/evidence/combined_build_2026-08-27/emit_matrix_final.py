...
for _row in _d0['rows']:
    ...
    # CANDIDATE WIDENING (2026-08-27): inside the ORDER 46 class the printed law is ONE clause
    # wider — the final-guard construct max(off, min(on, max(off, day0))) — read THROUGH the
    # engine's own o31_pi/day0_v0 names exactly as rl_export's restated assert reads it, so this
    # guard still cannot drift from the law. Out of class the pre-order expression is verbatim.
    if _mb is None:
        _pr = None
    elif G.get('_O46') and G['_o46_class'](_q, MA.BASE_REF):
        _g46 = G['pv_games'](_q, MA.BASE_REF)
        _off = _mb * float(G['o31_pi'](_q, MA.BASE_REF, _g46, _nofloor=True))
        _on = _mb * float(G['o31_pi'](_q, MA.BASE_REF, _g46))
        _d0c = G['day0_v0'](_q)
        _cap = max(_off, float(_d0c)) if _d0c is not None else _off
        _pr = int(round(max(_off, min(_on, _cap))))
    else:
        _pr = int(round(_mb * float(o31_D(_q, MA.BASE_REF))))
