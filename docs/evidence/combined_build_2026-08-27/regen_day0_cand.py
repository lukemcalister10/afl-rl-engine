...
for p in MA.data:
    ...
    # CANDIDATE WIDENING (2026-08-27, the exporter's own restated law, ONE clause wider inside the
    # ORDER 46 class): the identity there is the final-guard construct max(off, min(on, max(off,
    # day0))) — mb stays the INDEPENDENT board-derived v0 leg; only the law widened. Out of class,
    # and with the dial off, the pre-order expression below runs verbatim.
    _Y = MA.BASE_REF
    if G.get('_O46') and G['_o46_class'](p, _Y):
        _g = G['pv_games'](p, _Y)
        _off = mb * float(G['o31_pi'](p, _Y, _g, _nofloor=True))
        _on = mb * float(G['o31_pi'](p, _Y, _g))
        _d0c = G['day0_v0'](p)
        _cap = max(_off, float(_d0c)) if _d0c is not None else _off
        pr = int(round(max(_off, min(_on, _cap))))
    else:
