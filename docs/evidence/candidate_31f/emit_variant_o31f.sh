#!/bin/bash
# ORDER 31-F -- PER-ENTRANT COHORT-BOOK EMIT (24-year walk-forward), THE CANDIDATE'S OWN ENTRY LAW.
#
...
REPO="$(cd "$HERE/../../.." && pwd)"   # candidate_31f is one level shallower than noarb29c
SP=/tmp/claude-0/-home-user-afl-rl-engine/7ac96fea-1199-5b6a-9d77-ded9f53694f7/scratchpad
WT=$SP/wt_o31f_$LABEL
OUT=$SP/o31f/emit_$LABEL
...
cp "$HERE/emit_matrix_31f.py" "$OUT/emit.py"
echo "  emitter pins COMPUTED at run:"
echo "    emit_matrix_31f.py  (ORDER 31-F disclosed copy) $(md5sum "$OUT/emit.py" | cut -c1-32)"
echo "    emit_matrix_29c.py  (the file it was copied from, asserted in-copy) \
$(md5sum "$WT/docs/evidence/landing_29_2026-08-13/noarb29c/emit_matrix_29c.py" | cut -c1-32)"
...
# THE LANE IS ON. Without it the emitter's years 1..7 would be the Step-2 law under a 31-F label; the
# emitter itself HALTS if this is unset, so the claim is checked, not stated.
export RL_O31=1
