#!/usr/bin/env python3
"""ORDER 31-F  --  DAY0_31F_FINAL.json: THIS CANDIDATE'S OWN PRINTED DAY-0 PRICES.
...
BOARDP = os.path.join(SP, 'bb_f2on', 'rl_after', 'rl_app_data.json')
...
DOC = dict(label='ORDER 31-F -- the candidate\'s own printed day-0 prices',
...
json.dump(DOC, open(os.path.join(HERE, 'DAY0_31F_FINAL.json'), 'w'), indent=1, sort_keys=True)
print('ORDER 31-F PRINTED DAY-0 IDENTITY: %d of %d at tolerance 0 (ND %d, pool %d) on board %s'
      % (len(out) - len(mism), len(out), nND, nPOOL, BOARD_MD5))
if mism:
    print('MISMATCHES: %s' % mism[:10])
    sys.exit('ORDER 31-F HALT: the printed day-0 identity does not hold on the written board.')
print('DAY0_31F_FINAL.json written.')
