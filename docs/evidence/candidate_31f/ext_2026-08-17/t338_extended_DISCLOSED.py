...
import harness_repointed as H  # DISCLOSED EDIT 3: the ORDER-29 re-pointed harness copy (02dcf28c), store cb38ef11   # symlink/copy of the committed harness (dots -> underscores)

MATRIX = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, 'per_entrant_338_confirmation.json')
YEARS = list(range(0, 13))  # DISCLOSED EDIT 1 (owner ask 2026-08-17): extend to yr12 to measure the crossing
GROUPS = [('ALL picks 1-64', lambda r: True),
          ('picks 1-20',     lambda r: 1 <= r['pick'] <= 20),
          ('picks 21-64',    lambda r: 21 <= r['pick'] <= 64),
          ('picks 1-10',     lambda r: 1 <= r['pick'] <= 10),   # DISCLOSED EDIT 2 (owner ask 2026-08-17):
          ('picks 11-20',    lambda r: 11 <= r['pick'] <= 20),  # tighter bands
...
def main():
    ...
    jp = os.path.join(HERE, 'noarb_table_338_EXT.json')
