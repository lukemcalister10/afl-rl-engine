#!/usr/bin/env python3
"""ACT A (THE F5 ROUNDING ACT), LANDING STEP A — THE ONE PIN THAT MOVES.
...
WHAT IS ADAPTED, AND THE ONE REAL DIFFERENCE FROM EVERY PRIOR LANDING:
  ...
  **`engine_head` DOES NOT MOVE, and this script asserts that it does not.**
  ...
  `release_contract.json` tracks. The valuation engine is untouched, which is the whole point of the
  act: no player price moves, so the thing that prices players must not move either.
...
WHAT MOVES
  data/expected_boot.json  board  7a3f4fe2... -> md5(data/rl_build/rl_app_data.json)

The board artifact + both .srcmd5 sidecars were installed and committed with the act itself (e6cdec0)
...
Run:  python3 land_f5_pins.py            (dry run)
      python3 land_f5_pins.py --apply
"""
import hashlib, json, os, sys

ROOT = '/home/user/afl-rl-engine'
EXPECT_BOARD = 'c97a4d9f9fa42597f85517c7850d3943'    # BUILT by this seat (dev == canonical), not supplied
OLD_BOARD    = '7a3f4fe23207a29095e6d37408a4b727'    # the R23 board this act supersedes
...
print('=' * 100)
print('ACT A LANDING STEP A — the one pin that moves   (%s)' % ('APPLY' if APPLY else 'DRY RUN'))
print('=' * 100)
