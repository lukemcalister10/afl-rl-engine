#!/usr/bin/env python3
"""ORDER C — THE TWO PREVIEW PAGES (the standing presentation ruling: the owner reads raw data;
self-contained sortable HTML, house conventions carried from o32_pages.py).

  PREVIEW_C_PLAYERS.html  the full player table: rank, player, pathway, pos, age, games,
                          Live · Candidate 31 · repaired C32 · ORDER C, deltas, the two legs.
  PREVIEW_C_YEAR1.html    the year-1 class in DRAFT ORDER with v0: the same four columns.

Data: docs/ledgers/ORDER_C_MOVERS.json — the composed ledger, nothing recomputed here.
"""
import json, os, html
...
def page_head(title, sub, banner):
    return ('<!doctype html>\n<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">\n'
            ...
            '<header><div class="brand">ORDER <b>C</b><span class="sub">%s</span></div>'
            '<div class="spacer"></div>'
            '<div class="stamp">live <b>88ce647f</b> %d · cand 31 <b>fe6be9d6</b> %d · repaired C32 <b>7802ee97</b> %d<br>'
            'ORDER C <b>%s</b> <b>%d</b> · α=%.2f · store cb38ef11 · PRE-NUMERAIRE</div></header>\n'
            % (title, CSS, banner, sub, T['live'], T['c31'], T['c32r'], B['orderC'][:8], T['orderC'], ALPHA))


# ================= PAGE 1: the full player table ==================================================
rows_sorted = sorted(ROWS, key=lambda r: -r['cand'])
H = [page_head('Order C · The Board', 'THE AGE-CONDITIONAL NORMALIZATION · FULL PLAYER TABLE',
               'NOTHING LANDS WITHOUT THE OWNER\'S WORD — PREVIEW OF A CANDIDATE, PRE-NUMERAIRE')]
H.append('<div class="box"><h2>All %d priced rows · click any column to sort · FOUR value columns + the two Order C legs</h2>' % len(rows_sorted))
H.append('<table id="t1"><thead><tr>')
cols = [('#', True), ('player', False), ('path', False), ('pos', False), ('age', True), ('g', True),
        ('live', True), ('cand 31', True), ('C32R', True), ('ORDER C', True),
        ('Δ C32R', True), ('Δ live', True), ('surface', True), ('α leg', True)]
...
for i, r in enumerate(rows_sorted):
    H.append('<tr><td class="num k" data-v="%d">%d</td><td class="l">%s</td><td class="l k">%s</td>'
             '<td class="l k">%s</td><td class="num" data-v="%s">%s</td><td class="num" data-v="%d">%d</td>'
    ...
    H.append(delta_td(r['leg_surface']))
    H.append(delta_td(r['leg_alpha']))
    H.append('</tr>')
H.append('</tbody></table>')
H.append('<div class="note">Legs are REAL BOARD DELTAS from the Order C ladder (repaired C32 → + the '
         'age-conditional surface at the two retained denominators → + the re-derived credit scale '
         'α=%.2f); their sum is Δ vs C32R exactly. Every row aged 24+ is BYTE-UNMOVED from the '
         'repaired C32 (asserted store-wide). Board totals: live %d · C31 %d · C32R %d · Order C %d.'
         '</div></div>' % (ALPHA, T['live'], T['c31'], T['c32r'], T['orderC']))
H.append('<script>%s</script></div>' % SORT_JS)
open(os.path.join(HERE, 'PREVIEW_C_PLAYERS.html'), 'w').write('\n'.join(H))
...
H = [page_head('Order C · Year-1 Class', 'THE YEAR-1 CLASS · DRAFT ORDER',
               'NOTHING LANDS WITHOUT THE OWNER\'S WORD — DAY-0 PRINTS UNMOVED 89/89 (ASSERTED)')]
H.append('<div class="box warn"><h2>Read before the table</h2>'
         '<div class="note">Order C changes NO day-0 print: a gameless row touches neither corrected '
         'denominator and carries no credit, so the 89 draft-day prices are byte-identical to the '
         'repaired Candidate 32\'s (asserted at tolerance 0). What moves in this class are the rows '
         'that have PLAYED — their shown production now judged against their own age\'s measured '
         'expectation and the re-derived credit scale α=%.2f.</div></div>' % ALPHA)
...
cols = [('order', True), ('player', False), ('path', False), ('pick', True), ('pos', False), ('age', True),
        ('g', True), ('v0', True), ('live', True), ('cand 31', True), ('C32R', True), ('ORDER C', True),
        ('Δ C32R', True), ('Δ live', True)]
for i, (nm, num) in enumerate(cols):
...
for i, r in enumerate(Y1):
    H.append('<tr><td class="num k" data-v="%d">%d</td><td class="l">%s</td><td class="l k">%s</td>'
             '<td class="num" data-v="%s">%s</td><td class="l k">%s</td><td class="num" data-v="%s">%s</td>'
             '<td class="num" data-v="%d">%d</td>'
             % (i + 1, i + 1, html.escape(r['name'] or r['key']), r['pathway'] or '?',
                r['pick'] if r['pick'] is not None else '', r['pick'] if r['pick'] is not None else '—',
                r['pos'], r['age'] if r['age'] is not None else '', r['age'] if r['age'] is not None else '—',
                int(r['g']), int(r['g'])))
...
H.append('<div class="note">Draft order: ND by pick, then RD, pathway routes (PDA/PDN/PDS/IRE/UNR), SSP, MSD '
         ...
         'v0 = the row\'s own day-0 entry object (board currency — untouched by this candidate). '
         'Every column sortable; the default order is the draft order.</div></div>')
H.append('<script>%s</script></div>' % SORT_JS)
open(os.path.join(HERE, 'PREVIEW_C_YEAR1.html'), 'w').write('\n'.join(H))
print('written: PREVIEW_C_PLAYERS.html (%d rows) / PREVIEW_C_YEAR1.html (%d rows)' % (len(rows_sorted), len(Y1)))
