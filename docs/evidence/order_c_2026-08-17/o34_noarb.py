#!/usr/bin/env python3
"""ORDER C — THE STANDING NO-ARB DELIVERY, TWO-SIDED, FIVE BANDS + EVERY POOL ARM,
...
NEW = "    jp = os.path.join(HERE, 'noarb_table_338_EXT_%s.json' % os.environ.get('O34_TAG','x'))"
...
for tag, mx in (('31F', SP + '/per_entrant_O31FFINAL.json'), ('32R', SP + '/per_entrant_O32RFINAL.json'),
                ('O34', SP + '/per_entrant_O34FINAL.json')):
    env = dict(ENV, O34_TAG=tag)
...
ARMP = {'32R': arm_paths(SP + '/per_entrant_O32RFINAL.json'),
        'O34': arm_paths(SP + '/per_entrant_O34FINAL.json')}
...
P('')
P('=' * 100)
P('TWO-SIDED FIVE-BAND TABLE (ND, draft clock, extended 338 instrument) — repaired C32 vs ORDER C')
P('=' * 100)
P('A group is fairly priced if it appreciates BETWEEN 0%% and +14%% over its first year.')
P('%-16s | %10s %10s %10s | %10s %10s %10s' % ('band', '32R yr0-1', 'sell', 'buy', 'C yr0-1', 'sell', 'buy'))
BANDS5 = ['picks 1-10', 'picks 11-20', 'picks 21-30', 'picks 31-40', 'picks 41-64']
TB = {}
for g in ['ALL picks 1-64', 'picks 1-20', 'picks 21-64'] + BANDS5:
    rows31 = {r['N']: r['ratio_meanN_over_mean0'] for r in EXT['32R']['groups'][g]['rows']}
    rows32 = {r['N']: r['ratio_meanN_over_mean0'] for r in EXT['O34']['groups'][g]['rows']}
    a31 = rows31[1] / rows31[0] - 1.0
    a32 = rows32[1] / rows32[0] - 1.0
    s31, b31 = flag(a31); s32, b32 = flag(a32)
    TB[g] = dict(c31=a31, r32=a32, path31=rows31, path32=rows32)   # c31/path31 == repaired C32 here; r32/path32 == ORDER C
...
P('five-band spread (widest minus narrowest): repaired C32 %.1f points -> ORDER C %.1f points  [DIAGNOSTIC — reported, not targeted]'
  % (100 * sp31, 100 * sp32))

P('')
P('=' * 100)
P('TWO-SIDED POOL-ARM TABLE (cohort clock, all-arm instrument semantics) — ORDER C, both windows')
P('=' * 100)
...
ARM_TB = {}
for wname, grp in ARMP['O34'].items():
    P('')
    P('--- %s ---' % wname)
...
SC = json.load(open(os.path.join(HERE, 'W2_34_SCORECARD.json')))
...
P('  yr-1-vantage spread BEFORE Order C (repaired C32): k=1 %.3f  k=2 %.3f  k=3 %.3f  k=4 %.3f'
  % (sp31_v1[1], sp31_v1[2], sp31_v1[3], sp31_v1[4]))   # 'C31' label here reads: the repaired C32 baseline
...
json.dump(dict(order='ORDER C — two-sided five-band + pool-arm no-arb + vantage matrix (diagnostic)',
               ext_control_exact=True,
               five_band=TB, five_band_spread_c32r=sp31, five_band_spread_o34=sp32,
               pool_arms=ARM_TB, vantage_matrix_o34=VM, vantage_spread_o34=sp,
               vantage_yr1_spread_c32r=sp31_v1,
               band_fair_yr1=FAIR,
               msd_caption='MSD debut-year rows are excluded from that year and counted (n_pre), never zeroed'),
          open(os.path.join(HERE, 'NOARB_34.json'), 'w'), indent=1, sort_keys=True, default=float)
open(os.path.join(HERE, 'NOARB_34_out.txt'), 'w').write('\n'.join(OUT) + '\n')
print('\nwritten: NOARB_34.json / NOARB_34_out.txt / t338_ext_{31F,32R,O34}_console.txt')
