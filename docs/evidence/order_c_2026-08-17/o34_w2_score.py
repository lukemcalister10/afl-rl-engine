#!/usr/bin/env python3
"""ORDER C — the W2 scorer on the ORDER C matrix, plus the corrected-surface scorecard and the
band-level fair-yr1 benchmarks. The repair's o32r_w2_score.py carried; the changes: the matrix is
per_entrant_O34FINAL.json (its md5 and stamped engine head are READ from the file and substituted
into the committed instrument's identity asserts — same discipline, new identity), and every output
lands in this directory under the _34 names.
"""
...
MXP = SP + '/per_entrant_O34FINAL.json'
MX_MD5 = hashlib.md5(open(MXP, 'rb').read()).hexdigest()
MX_HEAD = json.load(open(MXP))['meta']['engine_head']
...
SUBS = [
    ...
    ("assert md5f(CAND_P) == 'd97f1aee4161ebcf785cd635ed095038', 'matrix md5 mismatch'",
     "assert md5f(CAND_P) == '%s', 'matrix md5 mismatch'" % MX_MD5),
    ("assert A['meta']['engine_head'] == '71d9949a', 'engine head mismatch'",
     "assert A['meta']['engine_head'] == '%s', 'engine head mismatch'" % MX_HEAD),
    ("print('identity OK: matrix md5 d97f1aee store cb38ef11 head 71d9949a n=2648')",
     "print('identity OK: matrix md5 %s store cb38ef11 head %s n=2648')" % (MX_MD5[:8], MX_HEAD)),
    ("with open(os.path.join(HERE, 'RESULTS_W2.json'), 'w') as f:",
     "with open(os.path.join(%r, 'W2_34_RESULTS.json'), 'w') as f:" % HERE),
]
_run = _txt
for a, b in SUBS:
...
open(os.path.join(HERE, 'W2_34_console.txt'), 'w').write(buf.getvalue())

import numpy as np
R = json.load(open(os.path.join(HERE, 'W2_34_RESULTS.json')))
...
# ---- band-level fair yr1: 1.14 x (1 - band yr1 delivered share) -----------------------------------
def nd_band(pk):
...
P('==== ORDER C — W2 SCORECARD, BOTH SURFACES ====')
...
P('')
P('BAND-LEVEL FAIR YR1: fair = 1.14 x (1 - the band\'s own year-1 delivered share)')
...
json.dump(dict(flat=dict(mean_0515=mean_0515, min_class=mn, max_class=mx, slope=slope, W=W_flat,
               ...
               band_fair_yr1=BANDS_OUT, per_class=per, matrix_md5=MX_MD5),
          open(os.path.join(HERE, 'W2_34_SCORECARD.json'), 'w'), indent=1, sort_keys=True, default=float)
open(os.path.join(HERE, 'W2_34_out.txt'), 'w').write('\n'.join(OUT) + '\n')
print('written: W2_34_RESULTS.json / W2_34_SCORECARD.json / W2_34_out.txt')
