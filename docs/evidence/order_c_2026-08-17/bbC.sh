#!/bin/bash
# ORDER C board build. ORDER 31-F's bb31f.sh with ROOT re-pointed to this seat's
# worktree, the scratch directory renamed, and RL_O32 / RL_O32_STAGE passed through. NOTHING ELSE IS
# CHANGED -- same staging, same five-var thread pinning, same RL_V0SURF_PKL override. Builds are
# STRICTLY SEQUENTIAL; PID-unique staging comes from the tag discipline (one tag, one dir, one run).
# ARG1 = tag for the output dir.
set -euo pipefail
export PATH="/root/rl_venv312/bin:$PATH"
ROOT=${RL_ROOT:-/home/user/afl-rl-engine/.claude/worktrees/agent-abd8af3ce56ecb9e3}
SP=${RL_SCRATCH:-/tmp/claude-0/-home-user-afl-rl-engine/7ac96fea-1199-5b6a-9d77-ded9f53694f7/scratchpad/o34}
TAG=${1:-x}
WS=$SP/bb_$TAG
rm -rf "$WS"; mkdir -p "$WS"
...
if [ -n "${RL_O34:-}" ]; then export RL_O34; fi
