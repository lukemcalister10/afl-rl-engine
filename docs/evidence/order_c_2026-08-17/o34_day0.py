...
SP = '/tmp/claude-0/-home-user-afl-rl-engine/7ac96fea-1199-5b6a-9d77-ded9f53694f7/scratchpad/o34'
...
DOC = dict(label='ORDER C -- the candidate\'s own printed day-0 prices',
...
json.dump(DOC, open(os.path.join(HERE, 'DAY0_C_FINAL.json'), 'w'), indent=1, sort_keys=True)
...
# ORDER C acceptance: the 89 prints must be byte-identical to the repaired candidate's own file.
R32 = json.load(open(os.path.join(os.path.dirname(HERE), 'order_a_2026-08-17', 'DAY0_32_FINAL.json')))
r32 = {r['key']: r for r in R32['rows']}
same = all(r['printed'] == r32[r['key']]['printed'] and abs(r['derived_v0'] - r32[r['key']]['derived_v0']) == 0.0
           for r in out) and set(r32) == set(r['key'] for r in out)
print('ORDER C DAY-0 vs REPAIRED C32 PRINTS: %s' % ('IDENTICAL (89/89, printed AND unrounded derived_v0)' if same else 'DIFFER'))
if not same:
    sys.exit('ORDER C HALT: day-0 prints moved.')
print('DAY0_C_FINAL.json written.')
