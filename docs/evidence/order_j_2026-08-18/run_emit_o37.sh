#!/bin/bash
# ORDER J — the walk-forward per-entrant emit. The ORDER 31-F emitter
# (docs/evidence/candidate_31f/emit_matrix_31f.py, itself the 29C disclosed copy) is BYTE-CARRIED; the
# engine is THIS TREE's wired engine with RL_O36=1 (RL_O35/RL_O32/RL_O31 implied and also exported for
# the emitter's own lane assert); the day-0 replication guard re-points at the landing candidate's own
# printed day-0 file. Same env, same five-var thread pinning, strictly sequential.
set -uo pipefail
LABEL=${O37_LABEL:-O37TALL}
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(cd "$HERE/../../.." && pwd)"
SP=/tmp/claude-0/-home-user-afl-rl-engine/7ac96fea-1199-5b6a-9d77-ded9f53694f7/scratchpad
OUT=$SP/o37/emit_$LABEL
...
for V in RL_O36 RL_O36_LAM_S1 RL_O36_TALL RL_O36_KAPPA RL_O36_GAMMA RL_O36_ETA RL_O36_GAMMA_D RL_O36_LAMBDA; do
  if [ -n "${!V:-}" ]; then export $V; fi
done
# ORDER J: the day-0 guard is RE-BASED on this board -- Lever 3 changes the sitter fade, and a
...
export RL_DAY0_FINAL="${RL_DAY0_FINAL:-$HERE/DAY0_J_TALL.json}"
