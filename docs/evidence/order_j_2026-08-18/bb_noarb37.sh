#!/bin/bash
# ORDER J — the standing no-arb instrument runs. The instruments are ORDER 29's / 31-F's DISCLOSED
...
# matrices: the LANDING CANDIDATE (O35FINAL = 1f176444), the OWNER-RULED TALL FACTOR ALONE (O37TALL)
# and the DISCLOSED SUB-GATE REFERENCE (O37REF, which fails J-TOL and is not carried).
set -uo pipefail
...
echo "instrument pins COMPUTED at run (disclosed copies, unmodified by ORDER J):"
...
cd "$N"
for L in O35FINAL O37TALL O37REF; do
...
} 2>&1 | tee "$HERE/NOARB_J_out.txt"
cp "$N"/t338ext_O35FINAL.txt "$N"/t338ext_O37TALL.txt "$N"/t338ext_O37REF.txt "$HERE/" 2>/dev/null || true
