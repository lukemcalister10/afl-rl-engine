#!/usr/bin/env python3
"""ORDER J — THE STANDING TWO-SIDED NO-ARB SUITE, in the format the owner has just re-confirmed.
...
LABELS = [l for l in ('O35FINAL', 'O37TALL', 'O37REF')
          if os.path.exists(SP + '/o37/noarb/table_EXT_%s.json' % l)]
NICE = {'O35FINAL': 'LANDING CANDIDATE 1f176444 (Order D) — the base',
...
P('=' * 118)
P('ORDER J — STANDING TWO-SIDED NO-ARB SUITE.  carry charge = 14%/yr.')
...
json.dump(OUT, open(os.path.join(HERE, 'STANDING_TABLES_J.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'STANDING_TABLES_J_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote STANDING_TABLES_J.json / STANDING_TABLES_J_out.txt')
