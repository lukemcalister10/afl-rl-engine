"""ORDER 23 -- THE DERIVATION, UNCAPPED. Carried from ORDER 22's o22_derive.py.

ORDER 22 header follows verbatim.

ORDER 22 -- THE DERIVATION. Layer 1, layer 2, the reconciliation test, and the derived levels.

DESCENDED FROM `phase1_derive.py` (ORDER 17, branch build/pool-repricing-phase1). The construction is
phase 1's, carried; FOUR THINGS CHANGE, each ordered:

  1. THE STRATA ARE ARM-SPLIT.  `harness_armsplit.structural_values(rows, split=True)` (ORDER 20)
     replaces the pinned `H.structural_values`. The pinned function keys completion strata on
     (pos, tenure) over a population that mixes arms, so a POOL price moves a NATIONAL row's completed
     value and therefore moves nd_profile -- the separation law failing inside the calibration target
     itself. Split on (arm, pos, tenure), a pool move cannot reach a national row at all. CONTROL: with
     split=False the same file reproduces the pinned harness value-for-value; asserted here, not assumed.

  2. THE TARGET IS MEASURED FRESH on whatever matrix is handed in -- never 1.0252 and never 0.9944.

  3. SHRINKAGE IS UNIFORM AT K=15 ACROSS EVERY PATHWAY, not PDS alone.
     Owner verbatim, 2026-08-12 (comment 5260713967): "K=15 was across the board, not PDS."
     w = n/(n+15); RD (n large) is effectively unmoved, the thin extremes are tempered.

  4. THE LEVELS ARE READ FROM THE TREE THAT PRODUCED THE MATRIX, so an iteration reports its own
     inputs. ND65+ is read AS THE ENGINE READS IT -- which under ORDER 23's amendment is the stored
     level itself, the cap having been removed by owner ruling (see the ORDER 23 note below).

Everything else is phase 1's, unchanged: the D3 full-career-profile basis measured through the
harness's own `realised_full`; layer 2 keyed on PATHWAY x POSITION only (no pick axis -- effpk is the
constant 65); cells sampled at n >= 20 derive on own outcomes; thin cells borrow the WHOLE-POOL shape
at K=10; per-pathway RENORMALISATION after borrowing (D4's mandatory guard); RULE 2 for the unsampled
remainder (its own residual group, never the pathway average); reconciliation ENTRY-WEIGHTED IN BOTH
LAYERS at 1e-9 relative.

READ-ONLY. Emits nothing. Deterministic.

    usage: o23_derive.py <matrix.json> <levels.json|CHECKOUT> <out.json> [label] [--basis anchor|v0]

=================================================================================================
THE DENOMINATOR, AND WHY THIS FILE DEPARTS FROM THE DIRECTIVE'S [PROFILE] INSTRUMENT
=================================================================================================
The directive's [PROFILE] measure, and phase 1's, is `SUM structural_value / SUM v0` where `v0` is
the matrix field `v0=round(v0_start(p),1)` (emit_matrix_338.py:252).

**FOR A POOL ROW, `v0_start` IS NOT THE ENTRY PRICE.** The engine splits at the entry anchor:

    entry_anchor(p):  if pool: pool_level(p) * _PL_F * _b_factor(p)     # the SIGNED level -- what this act moves
                      else:    v0_start(p)                              # the ND v0 curve

`v0_start` for a pool entrant is the pool-slot-derived leftover the #326 addendum-6 ruling explicitly
took OUT of his entry price ("for a pool entrant the pool-slot-derived v0_start no longer sets his
entry price"), and `_v0_curve_assert` states of a pool row that "it is priced off its signed division
level (#326 entry anchors) ... and it never reads this surface at all".

MEASURED, NOT ARGUED: cutting every signed pool level by 53-80% moved `SUM v0_start` for the rookie
draft by **-1.5%** and left **622 of 691** rookie rows' `v0` bit-identical. Only rucks moved, and only
because `_ruc_prior_cap` takes the signed level as its basis. **A multiplier derived against
`v0_start` and then written onto the signed level is not a repricing of anything** -- which is ORDER
21's D2 departure, word for word, applied to the level instead of to the retention.

So this file computes the profile on BOTH denominators and REPORTS BOTH:

    basis 'anchor' (DEFAULT, and the one the levels are driven off):
        pool rows      -> entry_anchor = pool_level(division) * _PL_F   [_b_factor == 1.0, asserted]
        national rows  -> v0_start, which IS entry_anchor for them, so the target is unchanged
    basis 'v0' (the directive's literal instrument, kept so the departure is visible and scoreable)

The division of a matrix row is reconstructed by the engine's own rule (`rl_model.py:1432-1446`):
RD -> 'RD:'+pos (the engine asserts `_pool_rd_position(p) == gfut(p)` for every RD row at build time),
pool ND -> 'ND65+', anything else -> its `type`. Verified against the ORDER 21 harvest: 0 mismatches
on 976 matched keys, and `entry_anchor == level * _PL_F` with 0 violations on 3,334 cells.

=================================================================================================
ORDER 23 -- THE ONE AMENDMENT TO ORDER 22'S FILE: THE ND65+ CAP IS GONE
=================================================================================================
Owner ruling, #334 comment 5262928754 (2026-08-12): the signed `nd65_plus` law's min-against-curve[64]
cap is REMOVED, and ND>64 prices at its derived level. ORDER 22 read the level AS THE ENGINE READ IT,
which meant `min(stored, curve[64]) = 185`; ORDER 23's engine reads the stored level itself, so this
file must too, or the derivation would divide the pathway by a price nobody pays. That is the SAME
denominator discipline the file already applies to every other pathway (see the departure note above),
applied to the one pathway a law used to hold apart. NOTHING ELSE IN THIS FILE CHANGES -- same strata,
same fresh target, same uniform K=15, same layer 2, same reconciliation tolerance.
"""
...
EXPECT = {'board': '1dbd1480a34c7823f330273211cbb76a',
...
CAPPK = int(CUR['signed_nd65_plus']['cap_against_curve_pick'])
...
P("=" * 122)
P("ORDER 23 DERIVATION (ND65+ UNCAPPED)   label=%s   matrix=%s (%s)" % (LABEL, os.path.basename(MATRIX), md5(MATRIX)[:8]))
P("=" * 122)
