#!/usr/bin/env python3
"""ORDER 23 -- THE CONSEQUENCE. Carried from ORDER 22's o22_consequence.py; the ONLY changes
are the four board paths and their labels, so the same decomposition is applied to ORDER 23's
own levers (the retention surface is ORDER 23's final one and the repricing lever now includes
the uncapped ND>64 pathway).

ORDER 22 header follows.
...
Reads only artefacts already produced. Writes CONSEQUENCE.json + prints the ledger.
  ...
  usage: o23_consequence.py <out.json>
...
BOARDS = [('LIVE', SP + '/o23/board_C1_control.json'),
          ('VARA', SP + '/o23/board_L1_H.json'),
          ('STAGED22', SP + '/o23/board_L2_RET.json'),
          ('FINAL', SP + '/o23/board_FINAL.json')]
...
P("=" * 122)
P("ORDER 23 -- THE COMPOSED POOL-UPDATE MOVERS LEDGER")
P("=" * 122)
...
P("  ... %d moved rows in total; the full ledger is in CONSEQUENCE_O23.json['ledger']." % len(LED))
P()
