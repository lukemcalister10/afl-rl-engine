#!/usr/bin/env python3
"""ORDER 23 -- RE-DERIVE THE MEAN-PRESERVING UPLIFT U AGAINST A CANDIDATE LEVEL TABLE.
...
CONTROL: run with `CHECKOUT` it must reproduce ORDER 21's published U table EXACTLY.
  ...
  usage: o23_uderive.py <ucells.json> <levels.json|CHECKOUT> <surface_in.json> <surface_out.json|-> [label]
"""
...
# THE ENGINE'S OWN LOOKUP, REPRODUCED: int() truncation, and the ND65+ law AS AMENDED (no cap).
LEV = {k: int(float(v)) for k, v in flat.items()}
# ORDER 23 (owner ruling 5262928754): THE CAP IS REMOVED -- ND65+ takes its derived level.
# SUPERSEDED: LEV['ND65+'] = int(min(nd65, CAP)).
LEV['ND65+'] = int(nd65)
for k, v in rd.items():
...
P("  engine level lookup (int-truncated, ND65+ UNCAPPED by the 2026-08-12 ruling; retired cap curve[%d]=%g):" % (CAPPK, CAP))
...
out['_ORDER23_U'] = ("U RE-DERIVED at the ORDER 23 candidate levels (%s). R is unchanged from "
