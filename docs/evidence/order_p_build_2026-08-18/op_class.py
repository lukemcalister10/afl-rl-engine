#!/usr/bin/env python3
"""ORDER P BUILD — THE YEAR-1 CLASS COHORT MARK, read off the BUILT walk-forward matrices.
...
LABELS = [('O35FINAL', 'the landing candidate 1f176444'),
          ('OKRULED', 'ORDER K f3101883 — the base this order changes one mechanism inside'),
          ('PBUILT', 'ORDER P — THE DECISION BOARD')]
...
OUT = {}
P('=' * 118)
P('ORDER P BUILD — THE YEAR-1 CLASS COHORT MARK, on the BUILT walk-forward matrices')
P('=' * 118)
...
json.dump(OUT, open(os.path.join(HERE, 'CLASS_P.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'CLASS_P_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote CLASS_P.json and CLASS_P_out.txt')
