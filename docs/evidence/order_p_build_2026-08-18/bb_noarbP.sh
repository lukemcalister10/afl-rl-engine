#!/bin/bash
# ORDER P BUILD — the standing no-arb instrument runs. The instruments are ORDER 29's / 31-F's DISCLOSED
...
cd "$N"
for L in O35FINAL OKRULED PBUILT; do
...
} 2>&1 | tee "$HERE/NOARB_P_out.txt"
cp "$N"/t338ext_O35FINAL.txt "$N"/t338ext_OKRULED.txt "$N"/t338ext_PBUILT.txt "$HERE/" 2>/dev/null || true
