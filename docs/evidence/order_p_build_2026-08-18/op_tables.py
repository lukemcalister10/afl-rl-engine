#!/usr/bin/env python3
"""ORDER P BUILD — THE STANDING TWO-SIDED NO-ARB SUITE, in the owner's standing format.
...
NICE = {'O35FINAL': 'THE LANDING CANDIDATE 1f176444 (Order D) — the oldest comparison column',
        ...
        'PBUILT': 'ORDER P — THE DECISION BOARD. The blind eta charge is replaced by '
                  'pi *= exp(-LAMBDA*A(g)*T(s_P)), the pedigree-conditional charge'}
...
P('=' * 118)
P('ORDER P BUILD — STANDING TWO-SIDED NO-ARB SUITE.  carry charge = 14%/yr.')
...
json.dump(OUT, open(os.path.join(HERE, 'STANDING_TABLES_P.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'STANDING_TABLES_P_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote STANDING_TABLES_P.json / STANDING_TABLES_P_out.txt')
