#!/bin/bash
# ORDER P BUILD — the walk-forward per-entrant emit. ORDER K's run_emit_K.sh with RL_O37 passed
...
set -uo pipefail
LABEL=${OP_LABEL:-PBUILT}
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(cd "$HERE/../../.." && pwd)"
SP=/tmp/claude-0/-home-user-afl-rl-engine/7ac96fea-1199-5b6a-9d77-ded9f53694f7/scratchpad
OUT=$SP/op/emit_$LABEL
...
for V in RL_O35 RL_O36 RL_O37 RL_O36_LAM_S1 RL_O36_TALL RL_O36_FLOORFIX RL_O36_KAPPA RL_O36_GAMMA \
         RL_O36_ETA RL_O36_GAMMA_D RL_O36_LAMBDA; do
  if [ -n "${!V:-}" ]; then export $V; fi
done
export RL_DAY0_FINAL="${RL_DAY0_FINAL:-$REPO/docs/evidence/order_k_2026-08-18/DAY0_K.json}"
echo "  emit starting $(date -u +%H:%M:%S)  (RL_O37=${RL_O37:-unset} RL_O36=${RL_O36:-unset} lambda_S1=${RL_O36_LAM_S1:-default})"
S=$(date +%s)
