...
out()
out('  THE ONE NOT-GREEN ROW, STATED PRECISELY (it is not softened, and it is not hidden):')
out('    F6 asked whether the matrix identity moved. IT MOVED: c231fda2 -> 592e9040. The prereg bound')
out('    this seat to re-emit and REPORT THE NEW MARK rather than restate 1.0672, and that is what was')
out('    done — the class instrument was RE-RUN, not reasoned about, and it READ 1.0672.')
out('    MATRIX_DIFF_out.txt is the account of what moved: EXACTLY ONE top-level block (meta), EXACTLY')
out('    TWO fields — engine_head 5f434b95 -> 5ac6780f (which MUST move; the engine source moved at this')
out('    bake) and emitter.workdir (the worktree path, not a priced value). The priced payload `recs` is')
out('    BYTE-IDENTICAL across the two emits: 2648 records, canonical md5 491560370c9f. Not one priced')
out('    record differs. The row stays NOT-GREEN because the identity did move; the mark is measured.')
out()
