#!/usr/bin/env python3
"""THE BAKE (register v780, 2026-08-20) — the year-1 class cohort mark ON THE SHIPPED DEFAULT.

docs/evidence/landing_prep_2026-08-20/lp_class.py carried with TWO declared changes and NOTHING ELSE:
  (a) a BK row is ADDED — the matrix this seat emitted with NO DIAL SET AT ALL, the shipped default.
      The LP row (the landing-prep emit, dial line exported BY HAND) is KEPT beside it on purpose, so
      the two are read against each other rather than one replacing the other.
  (b) the output filenames.
THE MEASUREMENT IS UNTOUCHED, and F6 does not rest on it: the matrix identity is byte-compared in
run_emit_BAKE.sh first. This table is the independent read of the same question.
...
fc_class.py carried with TWO declared changes and NOTHING ELSE:
  ...
  (2) the output filenames: CLASS_BAKE.json / CLASS_BAKE_out.txt.
...
LABELS = [('OKRULED', 'ORDER K f3101883'),
          ...
          ('D7BCAND', 'THE PARITY CANDIDATE a05fe951 — the v776 matrix, CARRIED'),
          ('LP', 'THE PARITY CANDIDATE a05fe951 — the landing-prep emit, dial line exported BY HAND'),
          ('BK', '*** THE SHIPPED DEFAULT a05fe951 — RE-EMITTED THIS SEAT WITH NO DIAL SET AT ALL ***')]
...
LABELS = [(l, n) for l, n in LABELS
          if l == 'BK' or os.path.exists(os.path.join(SP, 'per_entrant_%s.json' % l))]
...
json.dump(OUT, open(os.path.join(HERE, 'CLASS_BAKE.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'CLASS_BAKE_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote CLASS_BAKE.json and CLASS_BAKE_out.txt')
