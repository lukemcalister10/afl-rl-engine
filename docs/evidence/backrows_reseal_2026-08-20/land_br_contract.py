#!/usr/bin/env python3
"""ACT A (THE BACK-ROWS AGE_REF REPAIR), LANDING STEP C — THE RELEASE CONTRACT, THROUGH ITS WRITERS OF RECORD.

ADAPTED from docs/evidence/f5_and_sort_2026-08-20/land_f5_contract.py (itself the R23 adaptation of the
D8/landing-tail pattern). What is adapted: the board id in the must-move assertion is re-pointed at
THIS act's facts. Everything else is carried, because everything else is still true.

**engine_head must NOT move.** This act edits `engine/rl_after/rl_export.py`, the exporter, which no
identity pin tracks. `engine_head` is md5(_merged_recover.py) and the valuation engine is untouched:
25 BACK-HISTORY rows are re-priced by the exporter's own loop, no valuation expression changed, and
all 804 ACTIVE rows are byte-identical. Prior landings before the F5 act asserted engine_head MOVED;
this one asserts the reverse, deliberately, for a stated reason.
...
WHAT IS NOT TOUCHED **BY THIS SCRIPT**: balanced_board_md5 + present_lens_baseline. Their writer of
record is sibling_repin.py, and UNLIKE the F5 act THIS act is expected to MOVE the balanced board —
the residue cured here is present under the balanced posture too, and larger (RL_LEGE=0 leaves
BASE_REF=AGE_REF=2028 entering the back loop). So they are asserted FROZEN here and moved in LANDING
STEP D by their own writer, build-and-compare, never by a value typed into this script. Also untouched:
...
Run:  python3 land_br_contract.py            (dry run)
      python3 land_br_contract.py --apply
"""
...
RCT = load('release_contract_br', P('release_contract.py'))
...
if measured['config'] != 'eed19a75f775aeafe4ee5ea4b3990667192d8f90389ad6b0e8318e91062d14c1':
    raise SystemExit('HALT: config_sha256 moved — the back-rows repair is not a manifest dial')
...
if board_md5 == 'c97a4d9f9fa42597f85517c7850d3943': raise SystemExit('HALT: the board did NOT move')
if board_md5 != '68be10c79d0ee096455754e084bcf757': raise SystemExit('HALT: the board is not this act''s board')
