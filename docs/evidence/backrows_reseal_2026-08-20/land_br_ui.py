#!/usr/bin/env python3
"""ACT A (THE BACK-ROWS AGE_REF REPAIR), LANDING STEP E — THE UI BUNDLES. BOTH WRITERS. THE THRICE-PROVEN TRAP.

CARRIED BYTE-FOR-BYTE IN LOGIC from docs/evidence/f5_and_sort_2026-08-20/land_f5_ui.py (itself carried
from docs/evidence/r23_advance_2026-08-20/land_e_ui.py). Only this docstring differs. The script is parameterless — it reads the board pin, the round and every identity
...
MEASURED HERE, NOT ASSUMED: the BEFORE lines below print whether stamp.release is present at entry.
The sibling repin (step D) regenerates board_view through extract_board_view, so if it dropped the
block the trap was live in this very transaction and the print says so rather than the docstring
claiming it.

ONE THING TO READ CAREFULLY IN THE OUTPUT: stamp.balanced_board_md5 is asserted against
expected_boot.balanced_board_md5, which STEP D moved (3970156c -> 556ad70d) through its own writer of
record. stamp.release.balanced_board_md5 is asserted against release_lineage's FROZEN present-lens
baseline instead, which must never move. Those two legitimately differ, and each is checked against
the source it is actually assembled from.
...
spec=importlib.util.spec_from_file_location('rm_br',P('engine','rl_after','ingestion','round_movers.py'))
rm=importlib.util.module_from_spec(spec); sys.modules['rm_br']=rm; spec.loader.exec_module(rm)
