...
P('=' * 118)
P('ORDER N — ND BAND TABLES, BOTH WINDOWS. ORDER M file, only the board list changed.')
P('=' * 118)
...
LABELS = [('NDERIV', 'ORDER N — THE DERIVED CHARGE, PRICED OFFLINE (ESTIMATE, NOT A BUILD)'),
...
json.dump(OUT, open(os.path.join(HERE, 'BANDS_N.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'BANDS_N_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote BANDS_N.json / BANDS_N_out.txt')
