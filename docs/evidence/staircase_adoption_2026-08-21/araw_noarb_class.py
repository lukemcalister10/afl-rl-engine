# ==================================================================================================
# A-RAW CARRY. This file is docs/evidence/staircase_fix_2026-08-20/sfx_noarb_class.py, byte-carried by
...
# ==================================================================================================
#!/usr/bin/env python3
"""ORDER 44 MEASUREMENT — the year-1 class cohort mark ON BOTH CONSERVED CANDIDATES (F4).

d8_noarb_class.py carried with TWO declared changes and NOTHING ELSE:
  (1) the LABELS list: SFXBASE (THE LIVE R23 BOARD 68be10c7, dial unset, this seat's own emit) and
      the two conserved candidates SFXACON (ratchet+conserve) and SFXBCON (smooth+conserve) are
      appended beneath the standing list. D7BCAND is KEPT so the registered 1.0672 stands beside
      them rather than being quoted from memory, and D8BASE/D8CAND are kept as the previous act's
      rows. The two candidates are exempt from the absent-label filter, so a missing candidate
      matrix PRINTS as missing instead of vanishing.
  (2) the output filenames (CLASS_SFX.json / CLASS_SFX_out.txt).

THE MEASUREMENT IS BYTE-IDENTICAL — the W2 window, the cohort rule, marks(), the floor 1.03 and the
buy rail 1.14 are untouched, and the instrument still self-validates against ORDER K's published
marks before any candidate number is read. THIS IS PREREG_STAIRCASE.md F4.
...
LABELS = [('OKRULED', 'ORDER K f3101883'),
          ...
          ('D8CAND', 'ORDER D8 5ea978f7 — the previous act\'s priced candidate'),
          ('SFXBASE', 'THE LIVE R23 BOARD 68be10c7 — dial unset, THIS SEAT\'s own emit on the EDITED tree'),
          ('SFXARAW', '*** ORDER 44 VARIANT A RAW — RL_O44_LVLMONO=ratchet — THE ADOPTED ARM ***'),
...
LABELS = [(l, n) for l, n in LABELS
          if l in ('SFXARAW', 'SFXBRAW', 'SFXACON', 'SFXBCON') or os.path.exists(os.path.join(SP, 'per_entrant_%s.json' % l))]
...
json.dump(OUT, open(os.path.join(HERE, 'CLASS_ARAW.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'CLASS_ARAW_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote CLASS_SFX.json and CLASS_SFX_out.txt')
