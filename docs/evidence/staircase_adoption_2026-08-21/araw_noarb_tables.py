# ==================================================================================================
# A-RAW CARRY. This file is docs/evidence/staircase_fix_2026-08-20/sfx_noarb_tables.py, byte-carried by
...
# ==================================================================================================
#!/usr/bin/env python3
"""ORDER 44 MEASUREMENT — THE POOL-ARM NO-ARB TABLES ON BOTH CONSERVED CANDIDATES.

d8_noarb_tables.py carried with THREE declared changes and NOTHING ELSE:
  (1) LABELS: TWO candidates rather than one — SFXACON (ratchet+conserve) and SFXBCON
      (smooth+conserve) — with SFXBASE (THE LIVE R23 BOARD 68be10c7, dial unset, same tree, same
      seat) beneath them as THE COMPARISON COLUMN. The owner is choosing BETWEEN the variants, so
      both stand on one page against one base.
  (2) THE MOVE BLOCK RUNS ONCE PER CANDIDATE (CANDS, not a single hardcoded key), against every
      other label. D8 hardcoded 'D8CAND' because it had exactly one candidate; this act has two and
      a move table for only one of them would be the choice made silently.
  (3) the output filenames (STANDING_TABLES_NOARB_SFX.json / _out.txt).

THE MEASUREMENT, THE ARMS, THE WINDOWS, THE MSD YEAR-1 EXCLUSION AND THE OWNER'S PATH TEST ARE
UNTOUCHED, and the three instrument md5s are still asserted at run.
ORIGINAL D8 HEADER FOLLOWS.

ORDER D8 MEASUREMENT — THE POOL-ARM NO-ARB TABLES ON THE PRICED CANDIDATE 5ea978f7.

d7b_tables.py (the standing pool-arm instrument) carried with THREE declared changes and NOTHING ELSE:
  (1) LABELS: D8CAND (the priced candidate 5ea978f7 = the live dial line + RL_O33_TAPEROFF=1) at the
      head, with D8BASE (THE LIVE BOARD a05fe951, dial unset, same tree, same seat) beneath it as THE
      COMPARISON COLUMN. D8BASE's recs are BYTE-IDENTICAL to per_entrant_D7BCAND.json's — the matrix
      the on-file D7B pool-arm tables were rendered from — so the comparison column IS the standing-law
      table, re-emitted rather than re-typed. The candidate key in the MOVE block follows.
  (2) the output filenames (STANDING_TABLES_NOARB_D8.json / _out.txt).
  (3) the instrument scratch dir re-pointed at this seat's own dir.

THE MEASUREMENT, THE ARMS, THE WINDOWS, THE MSD YEAR-1 EXCLUSION AND THE OWNER'S PATH TEST ARE
UNTOUCHED. The cohort clock is the all-arm instrument's own: MSD keys on the DRAFT YEAR ITSELF,
everyone else on draft year + 1. The three instrument md5s are asserted at run.
...
ORDER D7b — THE POOL-ARM NO-ARB TABLES ON THE PARITY CANDIDATE a05fe951.

as_tables.py carried with TWO declared changes: the candidate label is D7BCAND (with FCBASE added as
the nearest baseline), and the output filenames (STANDING_TABLES_D7B.*). THE MEASUREMENT, the arms,
the windows, the MSD exclusion and the path test are UNTOUCHED.
...
LABELS = [('SFXARAW', '*** ORDER 44 VARIANT A RAW — RL_O44_LVLMONO=ratchet — THE ADOPTED ARM ***'),
          ('SFXBRAW', 'ORDER 44 variant B raw — RL_O44_LVLMONO=smooth (measured, NOT adopted)'),
...
CANDS = ('SFXARAW', 'SFXBRAW', 'SFXACON', 'SFXBCON')          # DECLARED CHANGE (2): the move block runs once per candidate
...
P('=' * 122)
for CAND in [c for c in CANDS if c in ARMP]:
    P()
    P('  ### CANDIDATE %s ###' % CAND)
    for wname, _lo, _hi in WINDOWS:
        ...
        for lab, _nice in LABELS:
            ...
            for arm in ARM_TYPES + ['ALLPOOL']:
                a = OUT['arms'].get(CAND, {}).get('%s|%s' % (wname, arm))
                ...
                OUT['moves'].setdefault('%s|%s|%s' % (CAND, wname, lab), {})[arm] = dict(
...
json.dump(OUT, open(os.path.join(HERE, 'STANDING_TABLES_NOARB_ARAW.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'STANDING_TABLES_NOARB_ARAW_out.txt'), 'w').write('\n'.join(L) + '\n')
P()
P('written: STANDING_TABLES_NOARB_SFX.json · STANDING_TABLES_NOARB_SFX_out.txt')
