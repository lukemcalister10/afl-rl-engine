# ==================================================================================================
# B-RAW CARRY. This file is docs/evidence/staircase_fix_2026-08-20/sfx_noarb_class.py, byte-carried by
...
LABELS = [(l, n) for l, n in LABELS
          if l in ('SFXBRAW', 'SFXACON', 'SFXBCON') or os.path.exists(os.path.join(SP, 'per_entrant_%s.json' % l))]
...
json.dump(OUT, open(os.path.join(HERE, 'CLASS_BRAW.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'CLASS_BRAW_out.txt'), 'w').write('\n'.join(L) + '\n')
