"""ORDER 25 (carried from ORDER 20C) — does the F2 book<->board parity gate break on the landed board?
...
SP = '/tmp/claude-0/-home-user-afl-rl-engine/7ac96fea-1199-5b6a-9d77-ded9f53694f7/scratchpad/o25'

book = {v['key']: v.get('cur') for v in json.load(open(os.path.join(ROOT, 'engine/rl_after/s4_matrix.json'))).values()
        if v.get('key')}

for label, path in (('psi CONTROL board e2bf7347', os.path.join(SP, 'board_CONTROL_psi.json')),
                    ('LANDED   board 88ce647f', os.path.join(ROOT, 'data/rl_build/rl_app_data.json'))):
