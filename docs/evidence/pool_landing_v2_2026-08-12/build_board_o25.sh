#!/bin/bash
# ORDER 25 -- BUILD A VARIANT BOARD ON THE FULL LANDING STAGE. Sibling of emit_variant_o25.sh and
# of docs/evidence/pool_quality_2026-08-12/build_board_o24b.sh; the only addition is the level stage.
#
# THE CHECKOUT'S board, engine, curve artifact and self-test ARE NEVER WRITTEN.
#
# Usage: build_board_o25.sh <outfile> <surface.json|nosurface> <levels.json|nolevels> [git-ref]
set -uo pipefail
OUT="$1"; SURF="$2"; LEV="$3"; REF="${4:-HEAD}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(cd "$HERE/../../.." && pwd)"
Q="$REPO/docs/evidence/pool_quality_2026-08-12"
SP=/tmp/claude-0/-home-user-afl-rl-engine/7ac96fea-1199-5b6a-9d77-ded9f53694f7/scratchpad
WT=$SP/wt_o25_$(basename "$OUT" .json)
...
if [ "$LEV" != "nolevels" ]; then
  python3 "$HERE/o25_stage.py" "$WT" "$LEV" | sed -n '1,20p' || { echo "LEVEL STAGE FAILED"; exit 1; }
fi
