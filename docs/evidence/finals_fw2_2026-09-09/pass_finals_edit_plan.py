"""FW2 AS A STORE EDIT — emit the edit list AND the movers it predicts, from one engine load.
...
SO THIS EMITS AN `act_kind: store-edit` SPEC. 183 players, two fields each:
...
OUT_DIR = os.path.join(REPO, 'docs', 'evidence', 'finals_fw2_2026-09-09')
SEASON = 2026
...
def run(ns):
    ...
    # THE FEED'S SHORT FORMS, WRITTEN OUT. Two names arrive abbreviated in the source file and match
    # no store row; both are unambiguous once expanded, and both are listed here rather than fixed in
    # the CSV so the file stays byte-identical to what the owner sent.
    #   Bailey Williams needs NO entry: the store carries exactly one (bailey-williams-wb, Western
    #   Bulldogs), which is the one the owner named, so the general resolver already lands on him.
    OVERRIDE = {'alex n-bullen': 'alex-neal-bullen',      # Alex Neal-Bullen (Adelaide)
                'hugo h-kahan': 'hugo-hall-kahan'}        # Hugo Hall-Kahan (Adelaide)
    ...
    for name, score in parsed['rows']:
        ...
        if len(cands) != 1:
            raise SystemExit('FW2 name %r resolves to %d row(s)' % (name, len(cands)))
    ...
    out = {'season': SEASON, 'scores_file': 'scores/FW2.csv', 'round_decimals': decimals,
           ...
           'control_pass': True}
    json.dump(out, open(os.path.join(OUT_DIR, 'FW2_EDIT_PLAN.json'), 'w'), indent=1)
    T('FW2 edit plan: %d edits over %d players; %d movers (%d played, %d did not); pool %d -> %d (%+d)'
