#!/usr/bin/env python3
"""ORDER R — THE YEAR-1 CLASS COHORT MARK, read off the BUILT walk-forward matrices.
...
LABELS = [('OKRULED', 'ORDER K f3101883'),
          ...
          ('QB1', 'FIX B1  p5  b0     A off — THE CONTROL 1b1817f3'),
          ('QAB1', 'FIX A+B1 p5 b0     A ON  — THE CONTROL cbbb94d4'),
          ('R15', 'ORDER R p15 b0     A off'),
          ('R20', 'ORDER R p20 b0     A off'),
          ('R15A', 'ORDER R p15 b0     A ON'),
          ('R20A', 'ORDER R p20 b0     A ON'),
          ('Rb1', 'ORDER R p5  0.111  A off'),
          ('Rb2', 'ORDER R p5  0.105  A off'),
          ('R15b1', 'ORDER R p15 0.111  A off'),
          ('R20b2', 'ORDER R p20 0.105  A off — SOFTEST'),
          ('R20b2A', 'ORDER R p20 0.105  A ON  — softest + FIX A')]
...
OUT = {}
P('=' * 118)
P('ORDER R — THE YEAR-1 CLASS COHORT MARK, on the BUILT walk-forward matrices')
P('=' * 118)
...
json.dump(OUT, open(os.path.join(HERE, 'CLASS_R.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'CLASS_R_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote CLASS_R.json and CLASS_R_out.txt')
