#!/usr/bin/env python3
"""ORDER R — THE OWNER'S PATH TEST, APPLIED TO EVERY BREACHING CELL.
...
The rule is written on PREREG_R.md BEFORE any table was read, and it is restated here so it cannot
be bent to a result:
...
T = json.load(open(os.path.join(HERE, 'STANDING_TABLES_R.json')))
BJ = json.load(open(os.path.join(HERE, 'BANDS_R.json')))['nd']
CHARGE = 0.14
...
P('=' * 118)
P("ORDER R — THE OWNER'S PATH TEST ON EVERY BREACHING CELL")
P('=' * 118)
...
# THE ND BANDS COME FROM BANDS_R.json, which carries BOTH WINDOWS. Reading only the primary window
# would hide the modern-window breaches, which are the ones this order moves. ALLCOH only: the
# EX0506 arm is a sensitivity and is reported in BANDS_R_out.txt, not ruled on here.
rows = []
...
P('  counts are in PATHTEST_R.json under n_included.')
P()
...
json.dump(dict(carry=CARRY, charge=CHARGE, cells=OUTR),
          open(os.path.join(HERE, 'PATHTEST_R.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'PATHTEST_R_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote PATHTEST_R.json / PATHTEST_R_out.txt')
