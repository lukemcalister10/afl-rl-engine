...
LABELS = [('OKRULED', 'ORDER K f3101883 — THE CURRENT CANDIDATE'),
...
P('  %-6s %-6s %12s %12s %12s   %s' % ('class', 'draft', 'ORDER K', 'landing', 'cand 31', 'in the mark?'))
for y in CLASSES:
    row = []
    for lab, _ in LABELS:
...
P()
P('  n by class (ORDER K): %s'
  % '  '.join('%d:%d' % (y, OUT['OKRULED']['n_per_class'][y]) for y in CLASSES))
...
json.dump(dict(ok_class=OUT, w2=W2, supervisor_ex=SUPERVISOR_EX_OKRULED,
          ...
          open(os.path.join(HERE, 'CLASS_L.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'CLASS_L_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote CLASS_L.json / CLASS_L_out.txt')
