#!/usr/bin/env python3
"""ORDER S — CONTINUITY, ON THE EFFECTIVE CONSTANTS, PLUS THE SEASON-TURN AXIS.

ORDER R's or_continuity.py with TWO substantive changes:
  (1) the cap function T is read from THE ENGINE'S OWN o38_T rather than re-implemented, so the
      ORDER S COMPRESSION is swept as the board was actually built and not as a clip lookalike;
      LAMBDA and THETA_R come from the ORDER S effective constants for the same reason;
  (2) A NEW AXIS — THE SEASON TURN. The prereg's claim is that a geometric-in-years-back recency
      weight is EXACTLY invariant to the calendar turn, because at a turn every played season's
      exponent rises by one and the common factor cancels in the normalisation. That is VERIFIED
      here through the engine's own o37_surplus, by evaluating it at Y and at Y+1 on unchanged data,
      not assumed. It is the cliff S1 must not create.
...
G0 = NS['O37_G0']
# ORDER S: the EFFECTIVE level, cap and slope. With every S dial unset these ARE ORDER R's, which
# with every R dial unset ARE ORDER P's, bit for bit (asserted at load as S-F0 / R1).
LAM = NS.get('O40_LAMBDA', NS.get('O39_THETA_R') and NS['O37_LAMBDA'] or NS['O37_LAMBDA'])
THR = NS.get('O40_THETA_R', NS.get('O39_THETA_R', NS['O37_THETA_R']))
S0 = NS['O37_S0']
TMAX = NS.get('O40_TMAX', NS.get('O39_TMAX', NS['O37_TMAX']))
ETA, GD = NS['O32_ETA'], NS['O32_GAMMA_D']
A = lambda g: 1.0 - math.exp(-g / G0)
ON38 = bool(NS.get('_O38'))
# THE ENGINE'S OWN cap function, so the COMPRESSION is swept rather than a clip lookalike.
T = NS['o38_T'] if ON38 else (lambda s: min(max(1.0 - THR * (s - S0), 0.0), TMAX))
OLDF = lambda g: max(0.0, 1.0 - ETA * ((g / GD) * math.exp(1.0 - g / GD))) if g > 0 else 1.0
CAPFORM = NS.get('_O40_CAPFORM', '')
P('=' * 112)
P('ORDER S — CONTINUITY.  tag=%s  dials=%s' % (TAG, DIALS))
...
P('  CAP FORM %s%s · RECENCY w %.4f%s · MATURE PREMIUM %s'
  % ('SMOOTH COMPRESSION' if CAPFORM == 'smooth' else 'hard clip',
     ('  C = %.6f (p%s anchor)' % (NS['O40_CAPC'], NS['_O40_CAPPCT'])) if CAPFORM == 'smooth' else '',
     NS.get('O40_RECW', 1.0), '' if NS.get('_O40REC') else '  (the engine\'s own flat weighting)',
     'ON at %d+' % NS['O37_AGE_GATE'] if NS.get('_O40_PGMAT') else 'off'))
...
# ---- NEW AXIS 3: THE SEASON TURN ------------------------------------------------------------------
P()
P('== NEW AXIS — THE CALENDAR TURN. The cliff the recency weighting must not create. ==')
P('   At a calendar turn every played season ages by one year. If the weighting were, say, a step')
P('   function of years-back, a season crossing a step boundary would move the whole surplus and the')
P('   whole charge on 1 January for no reason but the date. THE CLAIM ON THE PREREG is that a')
P('   GEOMETRIC weight is EXACTLY invariant to this, because at a turn every exponent rises by one')
P('   and the common factor w cancels in the normalisation.')
P('   IT IS VERIFIED HERE THROUGH THE ENGINE\'S OWN o37_surplus, at Y=2026 and at Y=2027 on')
P('   UNCHANGED DATA — the store has no 2027 season, so the ONLY thing that differs between the two')
P('   calls is that every season is one year further back.')
SURP = NS['o37_surplus']
turn = []
for p in MA.players:
    if not p.get('_by'):
    ...
    f0 = math.exp(-LAM * A(g) * T(s0v)) if g > 0 else 1.0
    f1 = math.exp(-LAM * A(g) * T(s1v)) if g > 0 else 1.0
    turn.append(dict(key=p['key'], name=p.get('player'), s0=s0v, s1=s1v, ds=abs(s1v - s0v),
                     f0=f0, f1=f1, df=abs(f1 - f0), nseas=len([x for x in (p.get('scoring') or [])
                                                               if (x.get('games') or 0) > 0])))
if turn:
    ws = max(t['ds'] for t in turn)
    wf = max(t['df'] for t in turn)
    P('   rows tested: %d' % len(turn))
    P('   LARGEST MOVE IN THE SURPLUS ACROSS THE TURN      : %.6e points a game' % ws)
    P('   LARGEST MOVE IN THE CHARGE FACTOR ACROSS THE TURN: %.6e' % wf)
    P('   the bar is the step ORDER P created on the 23->24 age axis, 0.4653.')
    P('   S1-F2 (any step in the charge factor above 1e-9 at fixed data): %s'
      % ('*** FIRED — the weighting IS a new cliff ***' if wf > 1e-9
         else 'does not fire — the turn is EXACTLY invariant'))
    if wf > 1e-9:
        for t in sorted(turn, key=lambda z: -z['df'])[:5]:
            P('     %-26s %d seasons  s %.6f -> %.6f   f %.6f -> %.6f'
              % (str(t['name'])[:26], t['nseas'], t['s0'], t['s1'], t['f0'], t['f1']))
    OUT['season_turn'] = dict(n=len(turn), worst_s=ws, worst_f=wf, fired=bool(wf > 1e-9))
else:
    P('   no row produced a surplus at both clocks.')
    OUT['season_turn'] = dict(n=0)
P()
P('   WHAT THIS AXIS DOES NOT COVER, said plainly: the ARRIVAL of a new season at a turn moves the')
P('   surplus, and it should — that is new evidence, not a weighting artefact. This axis holds the')
P('   data fixed on purpose so the WEIGHTING is the only thing that can move.')
