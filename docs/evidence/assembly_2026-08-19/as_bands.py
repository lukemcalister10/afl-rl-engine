...
LABELS = [('ASMCAND', '*** THE ASSEMBLY CANDIDATE — THE BOARD UNDER REVIEW ***'),
...
json.dump(OUT, open(os.path.join(HERE, 'BANDS_ASM.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'BANDS_ASM_out.txt'), 'w').write('\n'.join(L) + '\n')
