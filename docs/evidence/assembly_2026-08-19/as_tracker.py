...
COLS = [('live', 'live', '88ce647f'), ('IDENT_K', 'K', 'f3101883'),
        ('IDENT_P', 'P', '374d4e44'), ('L0_R', 'R', '7f88f509'), ('V754_CAND', 'CANDIDATE', None)]
...
for k, vals in ROWS:
    ...
    TR.append(dict(key=k, name=m.get('name'), pos=m.get('pos'), age=m.get('age'), club=m.get('club'),
                   ...
                   R=vals.get('L0_R'), cand=vals.get('V754_CAND'),
                   d_live_K=d(vals.get('live'), vals.get('IDENT_K')),
                   d_K_P=d(vals.get('IDENT_K'), vals.get('IDENT_P')),
                   d_P_R=d(vals.get('IDENT_P'), vals.get('L0_R')),
                   d_R_cand=d(vals.get('L0_R'), vals.get('V754_CAND')),
                   d_live_cand=d(vals.get('live'), vals.get('V754_CAND')),
                   d_K_cand=d(vals.get('IDENT_K'), vals.get('V754_CAND'))))
...
for r in TR:
    ...
    for t, lab, _w in COLS:
        if t not in V:
            continue
        key = {'live': 'live', 'IDENT_K': 'K', 'IDENT_P': 'P', 'L0_R': 'R', 'V754_CAND': 'cand'}[t]
