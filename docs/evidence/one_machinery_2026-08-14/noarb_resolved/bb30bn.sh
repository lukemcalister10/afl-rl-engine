#!/bin/bash
# ORDER 30B-N board build. VERBATIM bb30bp.sh (itself verbatim bb30b.sh) with ROOT re-pointed to THIS
# seat's worktree, the scratch dir re-pointed to o30bn, and ONE pass-through added for the new declared
# default-off dial RL_O30B_RESOLVED. NOTHING ELSE CHANGED -- same staging, same five-var thread pinning,
# same RL_V0SURF_PKL override (packet 29 3.3). ENGINE RUNS ARE STRICTLY SEQUENTIAL, FOREGROUND ONLY.
...
ROOT=${RL_ROOT:-/home/user/afl-rl-engine/.claude/worktrees/agent-a14698ead2bf8585d}
SP=${RL_SCRATCH:-/tmp/claude-0/-home-user-afl-rl-engine/7ac96fea-1199-5b6a-9d77-ded9f53694f7/scratchpad/o30bn}
TAG=${1:-x}
GR=${2:-UNSET}
RLM=${3:-}
WS=$SP/bb_$TAG
...
# ORDER 30B-N: the RESOLVED candidate's dial. DEFAULT-OFF; unset => not exported, so the dial-off build
# exercises the code default and must reproduce 9298203135202a0c707bb0977ba38c31 byte-exact.
if [ -n "${RL_O30B_RESOLVED:-}" ]; then export RL_O30B_RESOLVED; fi
