...
CANDS = ('SFXACON', 'SFXBCON')          # DECLARED CHANGE (2): the move block runs once per candidate
...
json.dump(OUT, open(os.path.join(HERE, 'STANDING_TABLES_NOARB_SFX.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'STANDING_TABLES_NOARB_SFX_out.txt'), 'w').write('\n'.join(L) + '\n')
P()
