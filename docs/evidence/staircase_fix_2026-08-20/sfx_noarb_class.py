...
LABELS = [(l, n) for l, n in LABELS
          if l in ('SFXACON', 'SFXBCON') or os.path.exists(os.path.join(SP, 'per_entrant_%s.json' % l))]
...
json.dump(OUT, open(os.path.join(HERE, 'CLASS_SFX.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'CLASS_SFX_out.txt'), 'w').write('\n'.join(L) + '\n')
