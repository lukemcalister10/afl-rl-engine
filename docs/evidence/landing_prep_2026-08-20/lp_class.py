...
fc_class.py carried with TWO declared changes and NOTHING ELSE:
  ...
  (2) the output filenames: CLASS_LP.json / CLASS_LP_out.txt.
...
LABELS = [('OKRULED', 'ORDER K f3101883'),
          ...
          ('LP', '*** THE PARITY CANDIDATE a05fe951 — RE-EMITTED THIS SEAT ON THE RE-KEYED BRANCH ***')]
...
LABELS = [(l, n) for l, n in LABELS
          if l == 'LP' or os.path.exists(os.path.join(SP, 'per_entrant_%s.json' % l))]
...
json.dump(OUT, open(os.path.join(HERE, 'CLASS_LP.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'CLASS_LP_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote CLASS_LP.json and CLASS_LP_out.txt')
