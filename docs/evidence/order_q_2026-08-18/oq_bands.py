#!/usr/bin/env python3
"""ORDER Q — THE ND BAND TABLES, IN BOTH WINDOWS. ORDER P's op_bands.py, REUSED WHOLE.
...
LABELS = [('OKRULED', 'ORDER K f3101883'),
          ...
          ('QA', 'ORDER Q FIX A — monotonised in entry price. PRICED, NOT ADOPTED'),
          ('QB1', 'ORDER Q FIX B1 — the age-24 gate deleted. PRICED, NOT ADOPTED'),
          ('QB2', 'ORDER Q FIX B2 — the 23-26 ramp, invented endpoint. PRICED, NOT ADOPTED'),
          ('QAB1', 'ORDER Q FIX A + B1. PRICED, NOT ADOPTED'),
          ('QAB2', 'ORDER Q FIX A + B2. PRICED, NOT ADOPTED')]
...
json.dump(OUT, open(os.path.join(HERE, 'BANDS_Q.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'BANDS_Q_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote BANDS_Q.json / BANDS_Q_out.txt')
