#!/usr/bin/env python3
"""ORDER Q — THE STANDING TWO-SIDED NO-ARB SUITE, in the owner's standing format.
...
LABELS = [l for l in ('OKRULED', 'PBUILT', 'QA', 'QB1', 'QB2', 'QAB1', 'QAB2')
          if os.path.exists(N36 + '/table_EXT_%s.json' % l)]
NICE = {'O35FINAL': 'THE LANDING CANDIDATE 1f176444 (Order D)',
        'QA': 'ORDER Q FIX A — the pedigree leg MONOTONISED in entry price. PRICED, NOT ADOPTED',
        'QB1': 'ORDER Q FIX B1 — the age-24 gate DELETED. PRICED, NOT ADOPTED',
        'QB2': 'ORDER Q FIX B2 — the charge RAMPED OUT across ages 23-26, invented endpoint. PRICED, NOT ADOPTED',
        'QAB1': 'ORDER Q FIX A + B1 together. PRICED, NOT ADOPTED',
        'QAB2': 'ORDER Q FIX A + B2 together. PRICED, NOT ADOPTED',
...
json.dump(OUT, open(os.path.join(HERE, 'STANDING_TABLES_Q.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'STANDING_TABLES_Q_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote STANDING_TABLES_Q.json / STANDING_TABLES_Q_out.txt')
