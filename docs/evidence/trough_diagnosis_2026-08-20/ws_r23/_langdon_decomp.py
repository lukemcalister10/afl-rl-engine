...
def fut(gdef, mid): return [["SD", float(gdef)], ["MID", float(mid)]]
...
L['_pos_now'] = 'SF'; v_probe = ev(L)
L['_pos_now'] = saved['_pos_now']; L['_fut'] = saved['_fut']; v_restore = ev(L)
print("PROBE _pos_now->SF:", v_probe, " | restored:", v_restore,
      "(round-trip OK)" if v_restore == base else "(!! MEMO/STATE ISSUE)")
print()

# ---- 2x2 ----
# columns: pos_now in {SF(old), SD(new)}; rows: _fut in {50/50(old), 70/30(new)}
grid = {}
for pos_now in ['SF', 'SD']:
...
OLD  = grid[('SF', (50, 50))]   # anchor -> expect 1122
NEW  = grid[('SD', (70, 30))]   # anchor -> expect 593
BARo = grid[('SD', (50, 50))]   # _pos_now toggled alone
POLo = grid[('SF', (70, 30))]   # _fut toggled alone

print("2x2 grid (EV):")
print("                _fut 50/50 (OLD)   _fut 70/30 (NEW)")
print(f"  _pos_now SF(OLD)   {OLD:>8}          {POLo:>8}")
print(f"  _pos_now SD(NEW)   {BARo:>8}          {NEW:>8}")
print()
...
print(f"  BAR  (_pos_now SF->SD) alone from OLD: {dBar1:+d}")
