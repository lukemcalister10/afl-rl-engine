"""#334 stage B / STAGE 4 AMENDMENT 1 — THE PROBES (b) MRAZ, (c) THE PEDIGREE PAIR, (d) THE BOUNDARY DiD.
...
REPO = os.environ.get('RL_REPO'); WORKDIR = os.environ.get('RL_WORKDIR', '/home/claude/amend1_ws/rl_after')
...
say('=' * 100)
say('#334 stage B / STAGE 4 AMENDMENT 1 PROBES   build tag: %s   PED_BAR=%s   SUR_W=%s' % (TAG, G.get('PED_BAR','n/a'), G.get('SUR_W','n/a (pre-amendment engine)')))
say('=' * 100)
...
if 'SUR_W' in G:
    anchor_full = R * G['entry_anchor'](MRAZ)
    gp = min(gy / fe, 6.0)
    s_sur = abs(float(np.log(efull / anchor_full)))
    u_un = 1.0 - G['_rho_res'](gp) / G['_RHO_SIT_BAR']
    dem = G['SUR_W'] * s_sur * u_un
    q2 = G['_ped_prior'](MRAZ, Y, fe, tau, cls, pk); e4 = 1.0 + G['PED_BAR'] * (1.0 - q2)
    say('  --- AMENDMENT 1: THE SURPRISE CHAIN ---')
    say('  anchor_full       : R x entry_anchor = %.5f x %.2f = %.2f   (the PRIOR-IMPLIED FULL PRICE)' % (R, G['entry_anchor'](MRAZ), anchor_full))
    say('  the claim         : e_full / anchor_full = %.2f / %.2f = %.3fx' % (efull, anchor_full, efull / anchor_full))
    say('  surprise s        : |log(%.3f)| = %.6f nats' % (efull / anchor_full, s_sur))
    say('  resolution        : %.4f games at pace -> rho(gp)/rho(6) = %.6f resolved  ->  u = %.6f UNRESOLVED' % (gp, 1.0 - u_un, u_un))
    say('  surprise demand   : SUR_W %.4g x s %.6f x u %.6f = %.6f extra passes of the lam ramp' % (G['SUR_W'], s_sur, u_un, dem))
    say('  exponent          : stage 4 %.6f  +  amendment 1 %.6f  =  %.6f' % (e4, dem, e4 + dem))
    say('  lam               : %.6f raw  ->  %.6f (stage 4)  ->  %.6f (amended)' % (lam0, lam0 ** e4, lam0 ** (e4 + dem)))
...
say('')
say('    NO-NEW-CLIFF SWEEP, raw games 1..10 on three probes. The amendment can only add a cliff by making')
say('    the exponent discontinuous in g; it is a ratio of polynomials in g composed with min(g/fE,6), so it')
say('    cannot. This measures it anyway. Reported: the price at each g, the successive step, and the LARGEST')
say('    single-game step as a share of the price it steps from. A cliff would show as one step dominating.')
say('    ' + '-' * 96)
sweep = {}
for k in PROBE_KEYS[:3]:
    b = next((x for x in MA.data if x.get('key') == k), None)
    if b is None: continue
    vals = [price(variant(b, games=g, avg=84.25, season=Y, name='CLIFF %s %dg' % (k, g)))[0] for g in range(1, 11)]
    steps = [vals[i + 1] - vals[i] for i in range(len(vals) - 1)]
    relsteps = [abs(steps[i]) / max(vals[i], 1) for i in range(len(steps))]
    j = int(np.argmax(relsteps))
    sweep[k] = dict(vals=vals, steps=steps, max_rel_step=relsteps[j], at_g=j + 1)
    say('    %-22s %s' % (b.get('player')[:22], '  '.join('%2d:%-6d' % (g, v) for g, v in zip(range(1, 11), vals))))
    say('    %-22s steps: %s' % ('', '  '.join('%+6d' % x for x in steps)))
    say('    %-22s LARGEST single-game step %.2f%% of the price it steps from, at g=%d->%d  (monotone rising: %s)'
        % ('', 100 * relsteps[j], j + 1, j + 2, all(steps[i] >= 0 for i in range(len(steps)))))


json.dump(dict(tag=TAG, ped_bar=G.get('PED_BAR'), sur_w=G.get('SUR_W'), sweep=sweep, mraz=dict(ev=ev_m, v0=v0_m, lam=lam0, efull=efull, R=R),
