"""#334 stage B / STAGE 6 — THE PICK/PLAYER SEAM (directive gate 6), AT ONE RUNG.
...
W = sys.argv[1]
MATRICES = [('STAGE 5 (landed book)    ', os.path.join(EV, 'stage5/noarb/per_entrant_338_stage5.json')),
            ('STAGE 6 rung %-5s        ' % W, os.path.join(HERE, 'noarb/per_entrant_338_rung%s.json' % W))]
...
say('=' * 112)
say('#334 stage B / STAGE 6 — THE PICK/PLAYER SEAM (implied ladder re-emitted from the post-change book)')
say('=' * 112)
...
a = OUTS['STAGE 5 (landed book)']; b = OUTS[('STAGE 6 rung %-5s' % W).strip()]
...
say('     max |per-pick move| attributable to stage 6    : %.6f%%  (tolerance +-2.00%%, seam tolerance, labeled)'
    % max(abs(x) for x in ddp))
say('     VERDICT: the seam gate                                  : %s'
...
say('  is 0.0 before and after AT ANY PRICE. That is why the established-leg reprice moves the')
say('  implied ladder by at most %.4f%%, inside but NOT far inside the +-2.00%% seam tolerance.' % max(abs(x) for x in ddp))
say('')
say('  THE FENCE AND THE SEAM: Mraz is on the SIT-OUT arm and is BYTE-IDENTICAL at every rung, so')
say('  stage 6 contributes NOTHING to his price. Pick 35\'s implied ladder value nevertheless moves')
say('  %+.4f%% at this rung, through the established-leg vpaths the lane reads. The coupling is real,' % ddp[34])
say('  not vacuous, and it is the established leg\'s, not the sit-out leg\'s.')
say('')
say('  %-6s %10s %10s %10s %12s %12s' % ('pick', 'installed', 'base book', 'landed', 'dev landed%', 'stage6 move%'))
say('  ' + '-' * 66)
for i in range(64):
    say('  %-6d %10d %10d %10d %+11.4f%% %+11.4f%%' % (i + 1, LAD0[i], a['ladder'][i], b['ladder'][i],
                                                       b['dev'][i], ddp[i]))
open(os.path.join(HERE, 'LADDER_SEAM_rung%s.txt' % W), 'w').write('\n'.join(L) + '\n')
json.dump(dict(installed=LAD0, baseline=a['ladder'], landed=b['ladder'],
               dev_landed_pct=b['dev'], dev_baseline_pct=a['dev'], stage6_move_pct=ddp,
               identical=bool(same), max_stage6_move_pct=float(max(abs(x) for x in ddp))),
          open(os.path.join(HERE, 'ladder_seam_rung%s.json' % W), 'w'), indent=1)
