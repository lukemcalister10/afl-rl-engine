"""#334 stage B / STAGE 5 — THE ENTRY-YEAR RIDE TABLES (gate restored by Addendum 2 from ruling 5214157997).
...
say('=' * 112)
say('#334 stage B / STAGE 5 — ENTRY-YEAR RIDE, ALL THREE TABLES AT THEIR OWN PEAK YEAR (gate: PRINTED ALWAYS)')
say('=' * 112)
RES = {}
for nm, f in BANDS:
    ...
    say('')
    say('  %s   n=%d' % (nm, len(ks)))
    say('     BASELINE own peak year %d, ratio %.6f    |    LANDED own peak year %d, ratio %.6f'
        % (pa, ta[pa]['ratio'], pb, tb[pb]['ratio']))
    say('     %-6s %10s %10s %10s %12s %12s' % ('entry N', 'n', 'BASE ratio', 'LAND ratio', 'BASE ride/yr', 'LAND ride/yr'))
    say('     ' + '-' * 66)
    rows = []
    for n in sorted(tb):
...
open(os.path.join(S5, 'RIDES.txt'), 'w').write('\n'.join(L) + '\n')
json.dump(RES, open(os.path.join(S5, 'rides.json'), 'w'), indent=1, default=float)
