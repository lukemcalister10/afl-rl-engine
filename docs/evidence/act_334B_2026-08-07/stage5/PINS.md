# STAGE 5 — PINS: what WOULD have moved, measured from the built artifacts

**NOTHING IN THIS TABLE IS LANDED.** Stage 5 STOPPED on its landing floor (`README.md`), so no product
file is committed. Every identity below was **measured from the artifact that carries it**, after that
artifact was built, in this seat's own worktree — none is typed from an expectation. They are recorded so
the frontier is reproducible and so a future pass knows exactly what it is re-stamping.

Baseline: `c05f214` (stage 4 amendment 1, board `b56bbdde`).

## data/expected_boot.json — the moves that were made and then NOT committed

| pin | old | would become | why |
|---|---|---|---|
...
| `board` | `b56bbddea15fd48e35b5794b1b5e9e23` | **`bad1961efad4c938aaf34eb6ee10036a`** | the rebuilt board, stamped AFTER the build from the built artifact. |
...
## A NEW COMMITTED ARTIFACT (also not landed)

| file | md5 | what |
|---|---|---|
| `engine/rl_after/g5_table.json` | **`1dc66750a51d04eb9b35b33685960feb`** | the taught surface. Loaded read-only at build, never fitted — the `lti_return_table.json` / `ycred_table.json` precedent. `bootstrap.sh` seeds it automatically (it copies `engine/rl_after` verbatim). A copy is filed beside this note as `stage5/g5_table.json`. |

## data/rl_build/rl_app_data.json + its sidecar

| field | old | would become |
|---|---|---|
| board file md5 | `b56bbddea15fd48e35b5794b1b5e9e23` | **`bad1961efad4c938aaf34eb6ee10036a`** |
| `.srcmd5` `own_md5` | `b56bbddea15fd48e35b5794b1b5e9e23` | **`bad1961efad4c938aaf34eb6ee10036a`** |
...
| artifact | md5 |
|---|---|
| `stage5/noarb/per_entrant_338_stage5.json` (walk-forward matrix, post-change) | `3d3420dd…` (banner: `store=37ced3ce engine=98ed7070 v0surf=3e8e50de51030297c99cf367161c161f frozen=True`) |
...
* **`side_by_side/board_before_after.xlsx` and `SIDE_BY_SIDE.md`** — deliberately untouched. They carry the
  FIVE landed stage columns and that remains correct. The sixth column is built and verified separately as
  `stage5/board_before_after_STAGE5_CANDIDATE.xlsx` (804/804 per-row identity, `verify_xlsx_stage5.py`).
