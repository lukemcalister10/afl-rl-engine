...
WHAT THIS DOES
  Teaches a per-pick re-anchor factor f(p) from the stage-1-basis walk-forward matrix and applies
  it to the shipped ladder in engine/rl_after/pvc_curve_v2.json. The construction is:
      ...
      R_s(p)   = locality-smoothed R_raw along the PICK axis (Gaussian kernel, n(p)-weighted,
                 bandwidth chosen by leave-one-out CV -- NO binning, NO bands, NO bucket steps)
...
def main():
    ...
    ap.add_argument('--matrix', default=os.path.join(HERE, 'per_entrant_338_stage1basis.json'))
    ap.add_argument('--ladder', default=os.path.join(HERE, 'pvc_curve_v2_PRE_stage2.json'))
    ...
    print("=" * 100)
    print("334 STAGE B / STAGE 2 -- PER-PICK RE-ANCHOR DERIVATION")
    print("=" * 100)
    ...
    print("  engine head     : %s   (stage-1 engine)" % meta.get('engine_head', 'n/a'))
    ...
    ratios = whole_cohort_year_ratios(ND)
    print("-" * 100)
    print("STEP 1 CONTROL -- whole-cohort year ratios (recomputed in-script; must match the no-arb table)")
    print("-" * 100)
    print("  yrN   n_incl     mean_yrN     mean_yr0      ratio")
    for N in sorted(ratios):
        n, mN, m0, r = ratios[N]
        print("  %-4d %6d %12.1f %12.1f %10.3f" % (N, n, mN, m0, r))
    ...
    # ---- the 64-row table ----------------------------------------------------------------------------
    lines = []
    lines.append("PER-PICK RE-ANCHOR -- 64 rows, no buckets, no bands")
    lines.append("peak year = %d   target residual = %.2f   kernel = Gaussian   bandwidth h = %.2f picks (LOO)"
                 % (PEAK, TARGET_RESIDUAL, h))
    lines.append("")
    ...
    js = dict(
        ...
        year_ratios={str(N): dict(n_incl=ratios[N][0], mean_yrN=ratios[N][1],
                                  mean_yr0=ratios[N][2], ratio=ratios[N][3]) for N in sorted(ratios)},
