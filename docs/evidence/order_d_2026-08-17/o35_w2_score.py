...
SUBS = [
    ("CAND_P = SP + '/per_entrant_O31FFINAL.json'", "CAND_P = SP + '/per_entrant_O35FINAL.json'"),
    ("assert md5f(CAND_P) == 'd97f1aee4161ebcf785cd635ed095038', 'matrix md5 mismatch'",
     "assert md5f(CAND_P) == 'e029bc9f646ed0a360a3043df18eba08', 'matrix md5 mismatch'"),
    ("assert A['meta']['engine_head'] == '71d9949a', 'engine head mismatch'",
     "assert A['meta']['engine_head'] == '5189e4ae', 'engine head mismatch'"),
    ("print('identity OK: matrix md5 d97f1aee store cb38ef11 head 71d9949a n=2648')",
     "print('identity OK: matrix md5 e029bc9f store cb38ef11 head 5189e4ae n=2648')"),
    ("with open(os.path.join(HERE, 'RESULTS_W2.json'), 'w') as f:",
     "with open(os.path.join(%r, 'W2_D_RESULTS.json'), 'w') as f:" % HERE),
]
_run = _txt
for a, b in SUBS:
...
open(os.path.join(HERE, 'W2_D_console.txt'), 'w').write(buf.getvalue())

import numpy as np
R = json.load(open(os.path.join(HERE, 'W2_D_RESULTS.json')))
...
A = json.load(open(SP + '/per_entrant_O35FINAL.json'))
...
json.dump(dict(flat=dict(mean_0515=mean_0515, min_class=mn, max_class=mx, slope=slope, W=W_flat,
          ...
          open(os.path.join(HERE, 'W2_D_SCORECARD.json'), 'w'), indent=1, sort_keys=True, default=float)
open(os.path.join(HERE, 'W2_D_out.txt'), 'w').write('\n'.join(OUT) + '\n')
print('written: W2_D_RESULTS.json / W2_D_SCORECARD.json / W2_D_out.txt')
