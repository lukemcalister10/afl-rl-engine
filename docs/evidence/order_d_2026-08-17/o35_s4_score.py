...
C32_P = SP + '/per_entrant_O35FINAL.json'
...
SUBS = [
    ...
    ("with open(os.path.join(HERE, 'RESULTS_S4.json'), 'w') as f:",
     "with open(%r, 'w') as f:" % os.path.join(HERE, 'RESULTS_S4_D.json')),
]
_run = _txt
for a, b in SUBS:
...
open(os.path.join(HERE, 'S4_D_console.txt'), 'w').write(buf.getvalue())
P('  scorer ran clean; console S4_D_console.txt (%d lines)' % buf.getvalue().count('\n'))

NEW = json.load(open(os.path.join(HERE, 'RESULTS_S4_D.json')))
...
json.dump(dict(order='ORDER A / Candidate 32 — S4 mid-career rescore (A6, report-only)',
          ...
          open(os.path.join(HERE, 'S4_D_RECOVERY.json'), 'w'), indent=1, sort_keys=True, default=float)
open(os.path.join(HERE, 'S4_D_out.txt'), 'w').write('\n'.join(OUT) + '\n')
print('\nwritten: RESULTS_S4_D.json / S4_D_RECOVERY.json / S4_D_out.txt')
