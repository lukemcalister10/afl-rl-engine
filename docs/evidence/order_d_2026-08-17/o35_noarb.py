...
EXT = {}
for tag, mx in (('C32R', SP + '/per_entrant_O32RFINAL.json'), ('D', SP + '/per_entrant_O35FINAL.json')):
    ...
    open(os.path.join(HERE, 't338D_%s_console.txt' % tag), 'w').write(r.stdout)
...
PREV = json.load(open(os.path.join(EV, 'order_a_2026-08-17', 'NOARB_32R.json')))
dev = 0
for g, tb in PREV['five_band'].items():
    rowsc = {r['N']: r['ratio_meanN_over_mean0'] for r in EXT['C32R']['groups'][g]['rows']}
    a = rowsc[1] / rowsc[0] - 1.0
    if abs(a - tb['r32']) > 1e-9:
        dev += 1
P('CONTROL: the C32R extended rerun reproduces the repair packet\'s own five-band yr0->1 numbers — %s'
  % ('EXACT' if dev == 0 else 'FAIL %d deviations' % dev))
assert dev == 0
...
ARMP = {'C32R': arm_paths(SP + '/per_entrant_O32RFINAL.json'),
        'D': arm_paths(SP + '/per_entrant_O35FINAL.json')}
...
P('')
P('=' * 100)
P('TWO-SIDED FIVE-BAND TABLE (ND, draft clock, extended 338 instrument) — the repaired C32 vs ORDER D (the landing candidate)')
P('=' * 100)
P('A group is fairly priced if it appreciates BETWEEN 0%% and +14%% over its first year.')
P('%-16s | %10s %10s %10s | %10s %10s %10s' % ('band', 'C32R yr0-1', 'sell', 'buy', 'D yr0-1', 'sell', 'buy'))
BANDS5 = ['picks 1-10', 'picks 11-20', 'picks 21-30', 'picks 31-40', 'picks 41-64']
TB = {}
for g in ['ALL picks 1-64', 'picks 1-20', 'picks 21-64'] + BANDS5:
    rows31 = {r['N']: r['ratio_meanN_over_mean0'] for r in EXT['C32R']['groups'][g]['rows']}
    rows32 = {r['N']: r['ratio_meanN_over_mean0'] for r in EXT['D']['groups'][g]['rows']}
    a31 = rows31[1] / rows31[0] - 1.0
    a32 = rows32[1] / rows32[0] - 1.0
    s31, b31 = flag(a31); s32, b32 = flag(a32)
    TB[g] = dict(c31=a31, r32=a32, path31=rows31, path32=rows32)  # c31 slot now carries C32R; r32 carries Order-D
...
P('five-band spread (widest minus narrowest): C32R %.1f points -> Order-D %.1f points  [DIAGNOSTIC — reported, not targeted]'
  % (100 * sp31, 100 * sp32))
...
SC = json.load(open(os.path.join(HERE, 'W2_D_SCORECARD.json')))
...
P('  yr-1-vantage spread on the repaired C32: k=1 %.3f  k=2 %.3f  k=3 %.3f  k=4 %.3f'
...
json.dump(dict(order='ORDER A REPAIR R3 — two-sided five-band + pool-arm no-arb + vantage matrix (diagnostic)',
          ...
          open(os.path.join(HERE, 'NOARB_D.json'), 'w'), indent=1, sort_keys=True, default=float)
open(os.path.join(HERE, 'NOARB_D_out.txt'), 'w').write('\n'.join(OUT) + '\n')
print('\nwritten: NOARB_D.json / NOARB_D_out.txt / t338_ext_{31F,32R}_console.txt')
