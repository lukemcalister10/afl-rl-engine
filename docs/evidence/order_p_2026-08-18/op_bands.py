...
P('=' * 118)
P('ORDER P — ND BAND TABLES, BOTH WINDOWS. ORDER N/M/L file, only the board list changed.')
P('=' * 118)
...
LABELS = [('PDERIV', 'ORDER P — THE PEDIGREE-CONDITIONAL CHARGE, PRICED OFFLINE (ESTIMATE, NOT A BUILD)'),
          ('NVARB', 'ORDER N — the age-only-bar charge at its frontier point (ESTIMATE, NOT A BUILD)'),
...
json.dump(OUT, open(os.path.join(HERE, 'BANDS_P.json'), 'w'), indent=1, default=float)
open(os.path.join(HERE, 'BANDS_P_out.txt'), 'w').write('\n'.join(L) + '\n')
print('\nwrote BANDS_P.json / BANDS_P_out.txt')
