...
1. The +1/+2 projection view (vP1/vP2) regenerates from the live model + live store inside the
   ...
   the item-5 pattern extended, Lens Projection law satisfied continuously.
2. Build-and-compare only; the owner views the first regenerated forward board before its pins
   bake; negative controls added for the new sibling (tamper → HALT, proven rc≠0).
3. Cold review before merge (a bare seat; implementer ≠ reviewer), because this modifies the
   production advance path.
...
2. Mechanical gates before anything scores: REF-FIXTURE bit-exact (App. A + B fixtures),
   REF-CANARY (every planted leak HALTs), REF-NULLCAL (null calibration within nominal),
   REF-MANIFEST complete. Scorecards identity-stamped per §9.3.
3. Deliverable: harness + claims note; then ONE cold blind review of the harness against the
   frozen protocol (§9.2) before the first official run; then round 0 (C0-smooth vs C0-banded,
