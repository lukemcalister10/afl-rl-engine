...
past, present and future — the pricing. Very separate." The forecast core is being settled by the
frozen referee (ITEM 410). THIS seat designs the pricing layer that sits on top. It never touches
the referee, the race, or the store.

## FOUNDING STRUCTURE (all owner-ruled; cite the register, do not re-litigate)
- v438/v439: PVC covers ND picks 1–64 — published scale FIXED at 1–64 (product ruling), curve
...
1. PRICING VALIDATION DESIGN — what evidence validates a pricing layer: internal consistency
   (G-MONO, CONSERVATION, L-CAPTAIN, LENS-PROJECTION, G-Y0, G-COHORT bind HERE), owner-preference
   elicitation protocol, external anchors if any (e.g., realized trade behaviour), and the
   bar/convexity experiment protocol for the owner's workbook work.
2. STRUCTURE SPEC — the v438/v439 architecture made exact: curve family and pool construction,
   era handling per R-ERA, observation-lane mechanics, boundary-measurement method.
3. ASSEMBLY NOTE — how the ruled pricing layer composes with the race's winning core into the
   new model (the only place Tracks A and C join), and what the adoption release must show the
   owner.

## FENCES
Read-only; bare inputs = this charter, the register header, the ITEM 409 memo, the frozen
protocol's scope lines; no contact with the race, the 408 branch, or the store; provenance tags
throughout; anything touching rulebook law returns as a drafted owner word, never a seat decision.
