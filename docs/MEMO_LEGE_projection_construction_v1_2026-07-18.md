# MEMO — LEG E: PROJECTION CONSTRUCTION — **v1.0 OPERATIVE** · 2026-07-18 · supersedes the v0.1 DRAFT
### v1.0 (item 341, the §8 finalization gate satisfied): Leg D + five-migration RETURNED AND
### PRESCREENED (base `a90052a`, board 06d8af60 @ RL_PVC2=1). Re-pinned: LENS dial `rl_model.py:414`,
### consumption `:442`/`:473`. Curve of record: `pvc_curve_v2.json` (payload `89c14729`) — the lens's
### pick input reads the v2 band evaluation. Construction content UNCHANGED from v0.1; [OWNER] slots
### remain his to rule at the movers report. Issued with DIRECTIVE_LEGE_2026-07-18.
...
`rl_model.py:414` on the LEG-E BASE `a90052a` — **RE-PINNED at v1.0 (was :397 groundwork; the register's `:317` is stale)**, consumed `:442`/`:473`). Contender and rebuilder DO NOT EXIST in code (rider job-6 grep:
