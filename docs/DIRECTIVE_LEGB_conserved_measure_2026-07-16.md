...
### sincerity story is PRE-SIM ONLY (memo: s=0.55 ⇒ y 1.100/1.154/1.113 — never measured on a
### real board; seg-5 halted before any ON board existed). This job measures it. NO SELECTION.
### It also measures the one thing the chapter OWES regardless: whether conserved-low-s pulls the
### baseline y4/y5 BREACH (1.3017/1.3160, the expired-waiver Leg-A condition) into the band.
...
- **Engine base — STRICT:** branch `claude/legb-unfunded-measurement-1f7vym` at **`b7a776c`**
  (the toggle + the proven harness live here; the toggle stays UNSET — conserved = shipped C).
  NEW branch; commits ONLY under `session_2026-07-16/legb_conserved_measure/`.
- **Docs base:** main AT OR AFTER the SHA in the owner's fire paste; diff docs/-only.

## FEED
Register items 256/257/258/259/260 (the sincerity law · the band 1.08–1.30 ideal 1.15–1.25 ·
the unfunded verdict) · docs/MEMO_LEGB_functional_form (seal cf6c0080) · acceptance_v1_20
...
E/B vs 1.75 WITH both numerators/denominators printed (the item-260 lesson: a ratio that
"passes" by denominator collapse is a failure to flag, not a pass) · position-pool Δ totals ·
net board ΣΔ · the all-804 SINCERITY ledger (ΔSCAR/Δ%/rank_off/rank_on/Δrank; Bontempelli the
named row; SCAR-up-rank-down counted; top-20 rank gainers/losers). Also: the OFF row restated
for the band verdict (known BREACH y4/y5 — the chapter's standing obligation).
