# 00_MANIFEST — v4.22 · 2026-07-15 · supersedes v4.21
### The pointer file. Read it first. It tells you what is current and what to read WHEN.
### Maintained by the FABLE seat (sitting 2026-07-15). This filing: the PRE-LADDER pack refresh —
### CONSTRAINTS -> v1.14 + acceptance v1.14 (A9 EXPECTED-FAIL-BY-LAW machine-readable; the AXIS RULE at
### PENDING-OWNER; L-CAPTAIN prerequisite status) · DECISIONS -> v102 · the v4.18 SELF-ROW TYPO in the
### pack table FIXED (carried since v4.19 in error). CORE/HANDOVER/SSI unchanged (HANDOVER refreshes at
### the rotation seam, not per-filing).
...
|---|---|---|---|
| **00_MANIFEST** | **v4.22** | 0 | first, always |
| **CORE** | **v2.8** | 0 | in full — BINDING. |
| **HANDOVER** | **rev139** | 0 | state at the 2026-07-15 rotation seam (refreshes at the NEXT seam). |
| **DECISIONS** | **v102** | 0 | rulings + the owner's CURRENT to-dos. Read v98 itself for verbatim. |
| **OPEN_ITEMS_REGISTER** | read its OWN header | 1 | THE DURABLE LOG (v122+; items 118–137 are this sitting). Repo-only. |
| **CONSTRAINTS** | **v1.14** | 1 | REPO-ONLY (P1) — fetch + grep when judging a board. A9 status · AXIS RULE (PENDING-OWNER) · L-CAPTAIN prereqs. |
| **acceptance_v1_14.json** | **v1.14** | 1 | REPO-ONLY (P1) — `standing_fails` + `audit_rule` are the auditor's scoring frame. |
