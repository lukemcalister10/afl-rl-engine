# OPEN ITEMS REGISTER — the single durable list · maintained by the supervisor pen · v376 2026-07-17 · PEN: ITEM 403: PRE-FIRE DIRECTIVE AUDIT (owner-requested) — caught+fixed 3 HALT hazards before firing: (1) store-write would HALT on the stale boot pins on base 3055ea5 (added GREEN-BOOT pre-step 0); (2) bake could run bootstrap at entry before the re-stamp (added entry-order banner); (3) bake Step 3 chased the stale BAKE_RUNBOOK (rewrote — reproducing head IS the bake head, acceptance/INFRA_ALLOW deferred). Fixed+coherent, main 43ce9d1; same launch blocks fetch the corrected directives · prior: ITEM 402
