# OPEN ITEMS REGISTER — the single durable list · maintained by the supervisor pen · v372 2026-07-17 · PEN: ITEM 399: dispatch-pin build HALTED on a STALE-BOOT-PIN bug (NOT the gremlin) — expected_boot.json rl_model/engine_head pins lag the VERIFIED-canonical engine cc626d7d/904722cd (missed re-stamp at legs E/F5/F6); build_board.sh bypasses them (guard hole); dispatch pin still UNPROVEN (clean container, toggle no-op). REC: (1) re-stamp+close-bypass small build; (2) REFRAME — bake via the gate now (doesn't need the pin), move dispatch/weekly-loop determinism to pre-go-live. Owner authorization pending · prior: ITEM 398
