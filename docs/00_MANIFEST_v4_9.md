# AFL RL ENGINE — PACK MANIFEST — v4.9 · 2026-07-13 · supersedes v4.8
### STATUS: OFFICIAL. Canonical pack home = repo `docs/` (truth). Project knowledge = THE SEAM
### PACK (fast start) — owner-ruled 2026-07-13. This manifest is a pointer card: NO operative
### content; on conflict its text yields to the role documents. Changelog: archive/CHANGELOG.
### CHANGELOG v4.9 (2026-07-13, owner-ruled housekeeping): Tier-0 table self-reference corrected
### (v4.8's rows still read "v4.7"/"CORE_v2.3" — stale carry from the v4.7 base; no pointer was
### operatively wrong elsewhere). Superseded docs swept from docs/ root to docs/archive/ in the
### same push (list in the commit). ALSO reconciled: the STANDING DOC RULES line still read
### "PROJECT KNOWLEDGE = TIER-0 ONLY / stale-copy hazard" — contradicting the seam-pack rule v4.8
### itself introduced; now worded to match. NO other content changed.
...
| role | file |
|---|---|
| Manifest | 00_MANIFEST_v4.9.md (this file) |
...
docs/-only token pen, SHA cited same turn) · PROJECT KNOWLEDGE = THE SEAM PACK (synced by the
owner at each seam; the freshness check makes the copies safe) — fast-moving docs stay repo-only ·
verbatim returns to docs/returns/ (WD-2) · doc jobs pin their base · every directive carries the
FULL-URL ls-remote base-verification line.
